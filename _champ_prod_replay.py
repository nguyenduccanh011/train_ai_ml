...
SRC = REPO / "_champ_src"
SERVING_DUCK = os.environ.get("REPLAY_DUCK", "C:/Users/DUC CANH PC/Desktop/stock-serving/market_data/market.duckdb")
...
px = cx.execute("SELECT symbol,date,low,close,high,volume FROM ohlcv WHERE timeframe='1D' AND date>='2018-06-01' ORDER BY symbol,date").fetchdf(); cx.close()
px["date"] = pd.to_datetime(px["date"]); CLO = {}; HI = {}; LO = {}; DIDX = {}; INV = {}; parts = []
...
def meta_features(syms):
    d = duckdb.connect(SERVING_DUCK, read_only=True); ph = ",".join("?" * len(syms))
    q = d.execute(f"select symbol,date,open,high,low,close,volume from ohlcv where timeframe='1D' and symbol in ({ph}) order by symbol,date", syms).fetchdf()
    d.close(); q['date'] = pd.to_datetime(q['date']); out = []
    ...
    df = pd.concat(out)
    df['rs_mom20'] = df.groupby('date')['ret20'].rank(pct=True); df['rs_mom60'] = df.groupby('date')['ret60'].rank(pct=True)
    return df
...
def build_tr_df(trades, signals, feat):
    ...
    sig = signals[(signals.signal == 1) & signals.score.notna()][["symbol", "date", "score", "exit_score"]].copy()
    ...
    sig['date'] = pd.to_datetime(sig['date']); parts = []
    ...
    tr['edkey'] = tr.entry_date.dt.strftime('%Y-%m-%d'); tr['t_pnl'] = tr['pnl']
    return tr
...
def rewrite(cvtr):
    ...
    o = cvtr.copy(); o["exit_date"] = pd.to_datetime(ned); o["exit_price"] = nep; o["exit_reason"] = nreason
    return o
...
def _entries(sim, r5map):
    ...
    for t in sim.trades:
        if t["conv"] < _skip_for(t["entry_date"]): continue
        v = r5map.get((t["symbol"], t["entry_date"]), np.nan)
        if not np.isnan(v) and v < R5THR: continue
        if OSMAP is not None:
            ov = OSMAP.get((t["symbol"], t["entry_date"]))
            if ov is not None and ov > OSTHR: continue
...
def prun_metric(sim, pm, cm, r5map, mu, sd, tplus, advance_fee=0.0008):
    ...
    for t in sim.trades:  # T+2 min-hold extend
        be, bx = t["i0"], t["i1"]; sc0 = sim.sym_close[t["symbol"]]
        if tplus and (bx - be) < tplus:
            nb = min(be + tplus, len(sc0) - 1); t["i1"] = nb; t["x_raw"] = sc0[nb]
    _prep(sim, pm, cm, mu, sd); entries = _entries(sim, r5map)
    ...
    nav = d["nav"]; fin = float(nav.iloc[-1]); yrs = (d["date"].iloc[-1] - d["date"].iloc[0]).days / 365.25
...
mu = statistics.mean(cv); sd_ = statistics.pstdev(cv) or 1.0
...
if _skip_gain <= 0 or _skip_mode == "off":
    ...
    print(f"[replay] OSTHR={OSTHR:.4f} conv_mu={mu:.4f} SKIP={SKIP:.4f} (mode=fixed gain={_skip_gain})", flush=True)
else:  # causal
    ...
    print(f"[replay] OSTHR={OSTHR:.4f} conv_mu={mu:.4f} SKIP=causal-per-year {_rng} "
          f"({ {y: round(s,3) for y,s in SKIP_BY_YEAR.items()} }) gain={_skip_gain}", flush=True)
...
print(f"  T+0: NAV x{fin0:.2f}  CAGR {100*cg0:.1f}%  DD {100*dd0:.1f}%  ({yrs:.2f} yrs)", flush=True)
print(f"  T+2: NAV x{fin2:.2f}  CAGR {100*cg2:.1f}%  DD {100*dd2:.1f}%", flush=True)
print(f"  (registered champion DuckDB reference: T+2 CAGR 132.1% / DD -13.8%)", flush=True)
# per-year NAV growth (T+2)
nav2["y"] = nav2["date"].dt.year
yr = nav2.groupby("y")["nav"].agg(["first", "last"]); yr["ret%"] = 100 * (yr["last"] / yr["first"] - 1)
