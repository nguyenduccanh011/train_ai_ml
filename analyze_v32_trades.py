...
DATA_DIR = (
    "C:/Users/DUC CANH PC/Desktop/train_ai_ml/portable_data/vn_stock_ai_dataset_cleaned/all_symbols"
)
...
def analyze():
    ...
    for _, row in trades.iterrows():
        ...
        rise_before_entry = (
            pct_diff(entry_price, price_10d_before) if not pd.isna(price_10d_before) else np.nan
        )
        drop_after_entry = (
            pct_diff(price_3d_after, entry_price) if not pd.isna(price_3d_after) else np.nan
        )
        ...
        max_before10 = (
            prices.iloc[max(0, idx_exit - 10) : idx_exit].max() if idx_exit > 0 else np.nan
        )
        ...
        exit_vs_m5 = pct_diff(
            exit_m5, exit_price
        )  # negative → earlier was cheaper → missed selling high
        ...
        missed_before5 = pct_diff(
            max_before5, exit_price
        )  # positive → could have sold higher 5d ago
    ...
    for col, label in [
        ("entry_vs_min5", "Min-5d"),
        ("entry_vs_min10", "Min-10d"),
        ("entry_vs_min20", "Min-20d"),
    ]:
        sub = losing[losing[col].notna()]
        pct_better = (sub[col] < -2).mean() * 100
        avg_save = sub[sub[col] < -2][col].mean()
        print(
            f"  {label:<14} {pct_better:.0f}% trades có giá thấp hơn ≥2%  avg saving={avg_save:+.2f}%"
        )
    ...
    print(
        f"  {'Symbol':<8} {'Entry date':<14} {'PnL':>8} {'Rise before':>12} {'Drop after':>12} {'Pattern':<20}"
    )
    for _, r in fomo.iterrows():
        print(
            f"  {r['entry_symbol']:<8} {str(r['entry_date'])[:10]:<14} {r['pnl_pct']:>+7.2f}%  "
    ...
    print(
        f"     WR={wr_late:.1f}%  avg={avg_late:+.2f}%  (vs tổng WR={len(wins) / total * 100:.1f}%)"
    )
    ...
    for col, label in [("missed_after5", "+5d"), ("missed_after10", "+10d")]:
        ...
        print(
            f"    {label}: {missed_sig:.1f}% trades giá tăng >5% sau bán  avg missed={avg_missed:+.2f}%"
        )
    ...
    for col, label in [("missed_before5", "-5d"), ("missed_before10", "-10d")]:
        ...
        avg_missed = sub[sub[col] > 5][col].mean()
        print(
            f"    {label}: {missed_sig:.1f}% trades đỉnh cao hơn điểm bán >5%  avg missed={avg_missed:+.2f}%"
        )

    # ── 3c: Worst exit timing — sold too late (big loss that had profit before)
    late_exit_loss = df[(df["pnl_pct"] < -5) & (df["max_profit_pct"] > 5)].sort_values("pnl_pct")
    print(f"\n  >> {len(late_exit_loss)} lệnh: max_profit>5% nhưng kết thúc lỗ >5% (bán muộn)")
    if len(late_exit_loss) > 0:
        print(
            f"  {'Symbol':<8} {'Entry':>12} {'Exit':>12} {'MaxProfit':>10} {'PnL':>8} {'ExitReason':<25}"
        )
        for _, r in late_exit_loss.head(15).iterrows():
            print(
                f"  {r['entry_symbol']:<8} {str(r['entry_date'])[:10]:>12} {str(r['exit_date'])[:10]:>12} "
                f"{r['max_profit_pct']:>+9.2f}% {r['pnl_pct']:>+7.2f}% {r['exit_reason']:<25}"
            )

    # ── 3d: Sold too early — missed big upside
    early_exit_win = df[(df["pnl_pct"] > 0) & (df["missed_after10"].fillna(0) > 10)].sort_values(
        "missed_after10", ascending=False
    )
    print(f"\n  >> {len(early_exit_win)} lệnh lời nhưng giá tăng >10% sau 10 ngày bán (bán sớm):")
    if len(early_exit_win) > 0:
        print(
            f"  {'Symbol':<8} {'Entry':>12} {'Exit':>12} {'PnL':>8} {'After10d':>10} {'ExitReason':<25}"
        )
    ...
    sold_at_dip = df[
        (df["missed_after5"].fillna(0) > 8) & (df["pnl_pct"] < 5)  # not a big winner when sold
    ...
    if len(sold_at_dip) > 0:
        ...
        for _, r in sold_at_dip.head(10).iterrows():
            print(
                f"  {r['entry_symbol']:<8} {str(r['exit_date'])[:10]:>12} "
    ...
    for ver in ["v30", "v31", "v32"]:
        try:
            t = pd.read_csv(
                f"C:/Users/DUC CANH PC/Desktop/train_ai_ml/stock_ml/results/trades_{ver}.csv"
            )
            n = len(t)
            ...
            print(
                f"  {ver.upper()}  trades={n}  WR={wr:.1f}%  avg_win={avg_w:+.2f}%  avg_loss={avg_l:+.2f}%  "
    ...
    df.loc[
        (df["rise_before_entry"].fillna(0) > 10) & (df["drop_after_entry"].fillna(0) < -3),
        "entry_quality",
    ] = "bought_at_peak"
    ...
    print(
        f"    Bán muộn (đỉnh đã qua 5d trước >5%): {len(late_sell)} lệnh  "
        f"avg PnL={late_sell['pnl_pct'].mean():+.2f}%"
    )
    print(
        f"    Bán sớm (giá tăng >5% trong 5d sau): {len(early_sell)} lệnh  "
    ...
    print(
        f"     WR={(hc['pnl_pct'] > 0).mean() * 100:.0f}%  avg={hc['pnl_pct'].mean():+.2f}%  "
        f"total={hc['pnl_pct'].sum():+.1f}%"
    )
    print(
        f"     max_profit>5% rồi đóng âm: "
        f"{((hc['max_profit_pct'] > 5) & (hc['pnl_pct'] < 0)).sum()} trades"
    )
    print(
        f"     max_profit>10% rồi đóng âm: "
    ...
    print(
        f"     WR={(hap['pnl_pct'] > 0).mean() * 100:.0f}%  avg={hap['pnl_pct'].mean():+.2f}%  "
