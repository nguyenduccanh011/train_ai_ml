...
### Submit Template for Backtest

**POST** `/api/v1/templates/{template_id}/submit`

Queue a backtest run for this template.

---

### Get Template Runs

**GET** `/api/v1/templates/{template_id}/runs`

List all backtest runs for a template.

---

### Export Template to YAML

**POST** `/api/v1/templates/{template_id}/export-yaml`

Export template configuration as YAML file.
...
Get all available feature sets catalog.
...
[
  {
    "id": "leading_v4",
    "name": "Leading Indicators v4",
    "description": "MACD, RSI, Bollinger Bands",
    "lookback_period": 20
  }
]
```
...
**GET** `/api/v1/model-library/feature-sets/{feature_set_id}`
...
Get all target definitions (what the model predicts).

---

### Create Target

**POST** `/api/v1/model-library/targets`
...
**PUT** `/api/v1/model-library/targets/{target_id}`
...
**DELETE** `/api/v1/model-library/targets/{target_id}`
...
List all reusable ML and rule components.

**Query Parameters**:
- `component_type` (optional): Filter by type (`ml` or `rule`)
- `role` (optional): Filter by role (`entry` or `exit`)
...
[
  {
    ...
    "type": "rule",
    "role": "entry",
    "description": "MACD crossover MA20"
  }
]
```
...
Register a new component (ML model or rule).

---

### Get Component Details

**GET** `/api/v1/model-library/components/{component_id}`

Get full component configuration and metadata.

---

### Update Component

**PUT** `/api/v1/model-library/components/{component_id}`
...
**DELETE** `/api/v1/model-library/components/{component_id}`

Remove a component from library.
...
**POST** `/api/v1/universes/{slug}/symbols`

Add one or more symbols to a universe.

---

### Remove Symbol from Universe

**DELETE** `/api/v1/universes/{slug}/symbols/{symbol}`
...
Create a copy of existing universe with new slug.
...
### List Symbols by Market

**GET** `/api/v1/markets/{market}/symbols`

Get all tradeable symbols in a market.
...
**GET** `/api/v1/experiments/pending`

List queued backtest experiments waiting to run.
...
[
  {
    ...
    "template_id": 1,
    "queue_time": "2026-05-30T14:32:15",
    "status": "queued"
  }
]
```
...
Queue a new backtest experiment (deprecated: use `/api/v1/templates/{template_id}/submit` instead).

**Note**: This endpoint returns 410 Gone. Use template submission instead.
