...
### 4.6 Universe Management Operations

Universe management allows organizing test universes for controlled experimentation. Use the API endpoints below for universe lifecycle.

#### 4.6.1 Create a New Universe

```bash
# Create a universe via API
...
#### 4.6.2 View Universe Versions and History

Every symbol addition/removal creates a new version automatically. Query version history:

```bash
# List all versions of a universe
curl http://localhost:8000/api/v1/universes/vn_tech/versions | jq '.'

# Get specific version
curl http://localhost:8000/api/v1/universes/vn_tech/versions/2 | jq '.symbols'

# Compare two versions
curl http://localhost:8000/api/v1/universes/vn_tech/versions/1 > v1.json
curl http://localhost:8000/api/v1/universes/vn_tech/versions/2 > v2.json
diff <(jq '.symbols[].symbol' v1.json | sort) <(jq '.symbols[].symbol' v2.json | sort)
```

#### 4.6.3 Modify Universe (Add/Remove Symbols)

```bash
# Add symbols to universe
curl -X POST http://localhost:8000/api/v1/universes/vn_tech/symbols \
  -H "Content-Type: application/json" \
  -d '{
    "symbols": [
      {"symbol": "VNM", "group": "pharma"}
    ]
  }'

# Remove symbol
curl -X DELETE http://localhost:8000/api/v1/universes/vn_tech/symbols/MWG

# After modification, version auto-increments (e.g., v1 → v2)
# Old backtests still use v1, new runs will use v2
```

#### 4.6.4 Query Leaderboard Universe Coverage

Check which experiments used which universes:

```bash
# View all experiments with universe tracking
curl http://localhost:8000/api/v1/leaderboard | jq '.summary.universe_coverage'
# Output: "15/20"  (15 out of 20 runs have universe info recorded)

# Find experiments using specific universe
curl 'http://localhost:8000/api/v1/leaderboard' | \
  jq '.models[] | select(.universe_slug=="vn_tech") | {run_name, universe_version, n_symbols, composite_score}'

# Find experiments NOT tracked (universe_slug=NULL)
curl 'http://localhost:8000/api/v1/leaderboard' | \
  jq '.models[] | select(.universe_slug==null) | {run_name, date_run}'
```

#### 4.6.5 Backfill Universe Info for Existing Runs

If you enabled universe tracking after existing experiments, backfill their universe info:

```bash
# Dry-run preview (shows what would be updated)
...
SELECT COUNT(*) as universe_tracked FROM leaderboard_runs WHERE universe_slug IS NOT NULL;
SQL
```

#### 4.6.6 Detect Universe-Related Issues

```bash
# Check if fairness groups properly separate universes
sqlite3 results/leaderboard.db << 'SQL'
SELECT 
  universe_slug, 
  COUNT(*) as count,
  COUNT(DISTINCT fairness_group_key) as distinct_fairness_groups
FROM leaderboard_runs
WHERE universe_slug IS NOT NULL
GROUP BY universe_slug;
SQL

# Expected: Different universes should have different fairness_group_key values
# If same fairness_group_key → old run data mixed with new universe (risk!)
```
