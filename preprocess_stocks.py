...
def load_symbols_metadata():
    """Load symbols.csv"""
    symbols = {}
    with open(os.path.join(METADATA_DIR, "symbols.csv"), encoding="utf-8") as f:
...
def load_coverage_metadata():
    ...
    """
    coverage = {}
    with open(os.path.join(METADATA_DIR, "coverage.csv"), encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            symbol = row["symbol"]
...
def main():
    ...
    for sym in all_symbols:
        ...
        if os.path.exists(data_path):
            with open(data_path, encoding="utf-8") as f:
                ...
                if (
                    row
                    and row.get("asset_type") in ("index", "derivative", "futures")
                    or sym in KNOWN_NON_STOCK
                ):
    ...
    print("  → Sẽ giữ lại làm CONTEXT FEATURES (chỉ số thị trường)")
    ...
    for i, sym in enumerate(candidates):
        ...
        if reasons:
            removed["data_anomaly"].append(f"{sym} ({', '.join(reasons)})")
            anomaly_removed.append(sym)
