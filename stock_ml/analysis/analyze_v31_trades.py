...
print(
    f"Total: {len(sig)} | WR: {(sig['pnl_pct'] > 0).mean() * 100:.1f}% | Avg pnl: {sig['pnl_pct'].mean():.2f}%"
)
...
for trend in ["strong", "moderate", "weak"]:
    ...
    if len(sub):
        ...
        print(
            f"  {trend:<12}: {len(sub):>4} trades | WR {wr:.1f}% | avg pnl {sub['pnl_pct'].mean():.2f}%"
        )
...
for val in sorted(sig["exit_above_sma20"].unique()):
    ...
    wr = (sub["pnl_pct"] > 0).mean() * 100
    print(
        f"  exit_above_sma20={val:.0f}: {len(sub):>4} trades | WR {wr:.1f}% | avg pnl {sub['pnl_pct'].mean():.2f}%"
    )
...
print(
    f"Count: {len(hap)} | WR: {(hap['pnl_pct'] > 0).mean() * 100:.1f}% | Avg pnl: {hap['pnl_pct'].mean():.2f}%"
)
...
print(
    f"WR:      {(peak['pnl_pct'] > 0).mean() * 100:.1f}%  vs all {(df['pnl_pct'] > 0).mean() * 100:.1f}%"
)
...
print("\n--- pnl stats ---")
print(
    f"Min: {ql['pnl_pct'].min():.2f}% | P25: {ql['pnl_pct'].quantile(0.25):.2f}% | Med: {ql['pnl_pct'].median():.2f}%"
)
...
print(
    f"price_max_profit_pct > 0%:  {(ll['price_max_profit_pct'] > 0).sum()} ({(ll['price_max_profit_pct'] > 0).mean() * 100:.1f}%)"
)
...
print(
    f"quick_reentry=True:  {len(re_df):4d} trades | WR: {(re_df['pnl_pct'] > 0).mean() * 100:.1f}% | Avg pnl: {re_df['pnl_pct'].mean():.2f}% | Total: {re_df['pnl_pct'].sum():.1f}%"
)
print(
    f"quick_reentry=False: {len(nre_df):4d} trades | WR: {(nre_df['pnl_pct'] > 0).mean() * 100:.1f}% | Avg pnl: {nre_df['pnl_pct'].mean():.2f}% | Total: {nre_df['pnl_pct'].sum():.1f}%"
)
print("\n--- Reentry exit reasons ---")
print(re_df["exit_reason"].value_counts().to_string())
print("\n--- Reentry pnl distribution ---")
re_df["pnl_bin"] = pd.cut(
    re_df["pnl_pct"],
    bins=[-999, -10, -5, 0, 5, 10, 20, 999],
    labels=["<-10", "-10~-5", "-5~0", "0~5", "5~10", "10~20", ">20"],
)
...
print("=" * 60)
ep_all = (
    df.groupby("entry_profile")
    .agg(trades=("pnl_pct", "count"), avg_pnl=("pnl_pct", "mean"))
    .round(2)
)
ep_hc = (
    hc.groupby("entry_profile")
    .agg(hc_trades=("pnl_pct", "count"), hc_avg_pnl=("pnl_pct", "mean"))
    .round(2)
)
