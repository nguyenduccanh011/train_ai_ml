...
# --- INSTRUMENTATION (analysis-only copy; trade semantics identical to stock_ml engine) ---
# INSTR_EVENTS rows: (symbol, kind, bar_idx, limit_price_or_nan, aux_idx)
#   kind in {"skip_market_weak", "skip_market_chop", "skip_cooldown", "missed", "filled"}
#   aux_idx: for "missed" -> j_end (last scanned bar); for "filled" -> fill bar idx; else -1
INSTR_EVENTS: list = []
INSTR_DEPTH: dict = {}   # symbol -> np.ndarray effective pullback depth per bar
INSTR_DATES: dict = {}   # symbol -> np.ndarray of bar dates
...
def _run_symbol(
    ...
    if cfg.entry_pullback_conv_scale and cfg.entry_pullback_pct is not None and n > 1:
        ...
        if cfg.entry_pullback_conv_use_combo:
            ...
            if _hw > 0 or _mw > 0 or _vw > 0 or _ww > 0 or _sw > 0 or _rw > 0 or _lw > 0:
                _blend = (1.0 - _hw - _mw - _vw - _ww - _sw - _rw - _lw) * _strength
                if _hw > 0:
    ...
    # --- INSTRUMENTATION: record per-bar effective pullback depth + bar dates ---
    if cfg.entry_pullback_pct is not None:
        _dep_arr = np.full(n, float(cfg.entry_pullback_pct))
        if pb_conv_mult is not None:
            _dep_arr = _dep_arr * pb_conv_mult
        INSTR_DEPTH[sym] = _dep_arr.copy()
        INSTR_DATES[sym] = dates.copy()
        ...
        if not in_pos:
            if sig > 0:
                ...
                if (
                    ...
                    INSTR_EVENTS.append((sym, "skip_market_weak", i, float("nan"), -1))
                    i += 1
                    continue
                ...
                if (
                    ...
                    INSTR_EVENTS.append((sym, "skip_market_chop", i, float("nan"), -1))
                    i += 1
                    continue
                ...
                if cfg.reentry_cooldown_bars > 0 and i - last_loss_exit_idx < cfg.reentry_cooldown_bars:
                    INSTR_EVENTS.append((sym, "skip_cooldown", i, float("nan"), -1))
                    i += 1
                    continue
                ...
                if cfg.entry_pullback_pct is not None and not _skip_pb:
                    ...
                    if fill_j < 0:
                        ...
                        if (cfg.entry_pullback_fill_if_missed and j_end > i
                            ...
                            i = entry_idx
                            continue
                        INSTR_EVENTS.append((sym, "missed", i, float(limit), int(j_end)))
                        i += 1  # missed — never pulled back; skip this signal
                        continue
                    entry_idx = fill_j
                    INSTR_EVENTS.append((sym, "filled", int(entry_signal_idx), float(limit), int(fill_j)))
...
def run_backtest(
    ...
    use_bot = (cfg.bot_ride_k is not None or cfg.bot_deepen_k is not None) and "score6" in sig.columns
    trades: list[Trade] = []
