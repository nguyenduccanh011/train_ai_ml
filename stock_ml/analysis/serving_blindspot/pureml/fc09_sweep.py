# -*- coding: utf-8 -*-
"""FAMILY_CHAMPIONS step 9: sweep WINNER-RIDING cho fc_rule2 (gap that = winner bi chem som, uplift +415).
...
BASE = "fc_rule2"
VARIANTS = [
    ("fc_r2_ox14", {"overext_pct": 0.14}),
    ("fc_r2_ox18", {"overext_pct": 0.18}),
    ("fc_r2_ox20", {"overext_pct": 0.20}),
]
...
def main():
    ...
    for name, _ in VARIANTS:
        ...
        row = cur.fetchone()
        print(f"FC9_RESULT {name} s42 comp={row[0]:.1f} d_base={row[0]-568.3:+.1f} pnl={row[1]:.1f} "
              f"pf={row[2]:.2f} mdd={row[3]:.3f} tr={row[4]} wr={row[5]:.3f} hold={row[6]:.1f} "
              f"run_id={r.get('run_id')}", flush=True)
        import pandas as pd
        con2 = psycopg2.connect(**PG)
        tdf = pd.read_sql("select symbol, entry_date, exit_date, entry_price, exit_price, "
                          ...
                          con2, params=(r.get("run_id"),))
        con2.close()
        tdf.to_csv(Path(__file__).parent / f"{name}_s42_trades.csv", index=False)
        print(f"dumped {len(tdf)} -> {name}_s42_trades.csv", flush=True)
    con.close()
    print("FC09_DONE")
