# -*- coding: utf-8 -*-
"""pureml step 4: clone t1058 (pure-ML champ) -> pm_sxrr25, chay seeds 42/7/99/555/123.

Khong dong den 2646/2730/2783. Pattern copy tu sv_probe.py + gb_multiseed.py.
"""
...
async def make_clone() -> int:
    ...
    async with Session() as s:
        ...
        t = await repo.create(
            ...
            description="PURE-ML champion autopsy clone of t1058 n2_sx_rr_h10_thr2p5 (no engine change)",
            hypothesis="best pure-ML (no force gate/suppress/trailing) template on leaderboard; "
                       "multi-seed + trade dump for gap diagnosis vs gb_x08",
...
def main():
    ...
    for sd in SEEDS:
        ...
        print(f"PM_RESULT seed={sd} comp={comps[sd]} "
              f"dChamp={comps[sd]-CHAMP[sd]:+.1f} dGBX08={comps[sd]-GBX08[sd]:+.1f} "
              f"pnl={row[1]:.1f} pf={row[2]:.2f} mdd={row[3]:.3f} tr={row[4]} wr={row[5]:.3f} "
              f"hold={row[6]:.1f} run_id={r.get('run_id')}", flush=True)
    cv = [v for v in comps.values() if v is not None]
    print(f"\n== {NAME} MEAN={statistics.mean(cv):.2f} seeds={comps}")
    print(f"== champ2646 MEAN={statistics.mean(CHAMP.values()):.2f} d={statistics.mean(cv)-statistics.mean(CHAMP.values()):+.2f}")
