# -*- coding: utf-8 -*-
"""R2 line NAV sim — BAN SAO Y HET khung chuan pr2_04_navsim.py (BASE mode dl_01_dual_sim,
size = NAV/K, 100% NAV, mark-to-market daily) — chi doi cho ghi output sang r2line.

Usage: python r2_nav.py <csv> <label> [K] [date_lo]
date_lo (optional): cat NAV-tu-<date> de chong ghost NAV (mac dinh 2020-01-01).
"""
import sqlite3
import sys
...
DATE_LO = sys.argv[4] if len(sys.argv) > 4 else "2020-01-01"

ta = pd.read_csv(CSV)
# NAV-tu-DATE_LO: chi lay trades entry >= DATE_LO
...
ns.to_csv(BASE_DIR + f"/r2line/r2_nav_{LABEL}.csv", index=False)
