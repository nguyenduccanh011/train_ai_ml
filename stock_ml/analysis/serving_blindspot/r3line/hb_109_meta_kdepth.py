...
def main():
    ...
    for sd in SEEDS:
        ...
        cv = HERE / f"_k109_s{sd}.csv"; cvtr.to_csv(cv, index=False)
        if feat is None: feat = features(cvtr.symbol.unique().tolist())
        tr = build_tr(con, rid, feat); seed_pm[sd] = meta_preds(tr); seed_cv[sd] = str(cv)
    con.close()
    print("META (v1 day-rank) x concentration-K depth — 3-seed mean CAGR/DD:", flush=True)
    print("  K  | CAGR%   DD%", flush=True)
    for Kv in (20, 16, 14, 12):
        cg_s, dd_s = [], []
        for sd in SEEDS:
            _, cg, dd = prun(NavSim2(seed_cv[sd], date_lo="2020-01-01"), seed_pm[sd], Kv=Kv); cg_s.append(cg); dd_s.append(dd)
        print(f"  {Kv:2d} | {statistics.mean(cg_s)*100:5.1f}  {statistics.mean(dd_s)*100:5.1f}", flush=True)
    print("HB_109_DONE", flush=True)
