...
def build_tr(con, rid, feat):
    ...
    hold = (tr.exit_date - tr.entry_date).dt.days.clip(lower=1)
    tr['t_pnl'] = tr['pnl']                                # raw pnl
    tr['t_vel'] = tr['pnl'] / np.sqrt(hold)               # profit velocity (per sqrt-time, free slot faster)
    tr['t_log'] = np.log1p(tr['pnl'].clip(lower=-0.9))    # log (tame outliers)
    tr['t_win'] = (tr['pnl'] > 0.02).astype(float)        # win classification
    return tr
...
def main():
    ...
    for sd in SEEDS:
        ...
        cv = HERE / f"_k112_s{sd}.csv"; cvtr.to_csv(cv, index=False)
        if feat is None: feat = features(cvtr.symbol.unique().tolist())
        seed_tr[sd] = build_tr(con, rid, feat); seed_cv[sd] = str(cv)
    con.close()
    print("META TARGET variants (same trade-set, multi-seed K16):", flush=True)
    for tg in TGTS:
        cg_s, dd_s, nv = [], [], []
        for sd in SEEDS:
            pm = meta_preds(seed_tr[sd], tgt=tg)
            n, cg, dd = prun(NavSim2(seed_cv[sd], date_lo="2020-01-01"), pm); nv.append(n); cg_s.append(cg); dd_s.append(dd)
        print(f"  target={tg:6s} NAV x{statistics.mean(nv):5.2f} CAGR {statistics.mean(cg_s)*100:.1f}% DD {statistics.mean(dd_s)*100:.1f}%", flush=True)
    con2 = psycopg2.connect(**PG)  # score-priority ref
    sc_nv = []
    for sd in SEEDS:
        sm = {(x.symbol, x.edkey): (float(x.score) if pd.notna(x.score) else 0.0) for _, x in seed_tr[sd].iterrows()}
        n, _, _ = prun(NavSim2(seed_cv[sd], date_lo="2020-01-01"), sm); sc_nv.append(n)
    con2.close()
    print(f"  [ref] score-priority NAV x{statistics.mean(sc_nv):.2f}", flush=True)
    print("HB_112_DONE", flush=True)
