# -*- coding: utf-8 -*-
"""hb_30: truc mh cua hybrid t2943 — clone stack t2943 (gb - T + head csrank dyn e3)
voi max_hold 16/20 (t2943 dang 25): hb_2943_mh16 / hb_2943_mh20, seed 42,
nh_nav2 2 che do x full/f22/f23/f25 — plateau check.
PHAI chay cwd=repo root (head csrank/xsec). KHONG dung canonical, KHONG commit.
Usage: python hb_30_plateau.py mh16 mh20"""
...
VARIANTS = {"mh16": 16, "mh20": 20}


async def make_clone(variant: str) -> int:
    mh = VARIANTS[variant]
    name = f"hb_2943_{variant}"
    ...
    async with Session() as s:
        ...
        t = await repo.create(
            ...
            description=f"hb_30: stack t2943 (gb-T + head csrank dyn e3) + max_hold {mh}",
            hypothesis="hb_30: plateau mh cua hybrid — mh25 co phai dinh co lap?",
...
def run_variant(variant: str):
    ...
    csv_path = HERE / f"hb_2943_{variant}_s42_trades.csv"
    if not csv_path.exists():
        ...
        row = cur.fetchone()
        print(f"HB30RUN hb_2943_{variant} s42 comp={row[0]:.1f} pnl={row[1]:.1f} pf={row[2]:.2f} "
    ...
    for lo, tag in (("2020-01-01", "full"), ("2022-01-01", "f22"),
        ...
        print(f"HB30NAV hb_2943_{variant} {tag}: adv x{a['mean']:.2f}±{a['sd']:.2f} "
...
def main():
    for v in sys.argv[1:]:
        ...
        try:
            ...
            print(f"HB30FAIL {v}: {e!r}", flush=True)
    print("HB_30_DONE", flush=True)
