# -*- coding: utf-8 -*-
"""Follow-up: filter "bỏ fill ngày thị trường sập" thắng +6.7pp/3-seed nhưng dùng return-ngày-khớp (close)
=> nghi look-ahead. Test bản CAUSAL:
  - lag1: thị trường sập HÔM QUA (return ngày trước fill, biết chắc trước khi khớp)
  - r2_lag1: return 2 phiên KẾT THÚC hôm qua
So với same-day (tham chiếu). Sweep ngưỡng + per-year. Nếu causal 3/3 => sạch, sang stack+register.
Base K10/cs5_ma50+ret7, T+2, seeds[42,21,123]."""
...
for s, g in px.groupby("symbol"):
    ...
    tr_ = (c - c.shift()).abs(); g["atrpct"] = tr_.rolling(14).mean() / c
...
# ---- robust market breadth index + market-day features (causal lags) ----
piv = px.pivot_table(index="date", columns="symbol", values="close").sort_index()
ewd = piv.pct_change().clip(-0.15, 0.15).median(axis=1)
idx_s = (1.0 + ewd.fillna(0.0)).cumprod()
r2d = idx_s / idx_s.shift(2) - 1.0
same = {d.strftime("%Y-%m-%d"): (float(v) if pd.notna(v) else np.nan) for d, v in ewd.items()}          # ret ngày khớp (look-ahead)
lag1 = {d.strftime("%Y-%m-%d"): (float(v) if pd.notna(v) else np.nan) for d, v in ewd.shift(1).items()}  # ret hôm qua (causal)
r2l1 = {d.strftime("%Y-%m-%d"): (float(v) if pd.notna(v) else np.nan) for d, v in r2d.shift(1).items()}  # ret 2p kết thúc hôm qua
...
for sd in SEEDS:
    ...
    cvf = HERE / f"_cf2_s{sd}.csv"; cvtr[["symbol", "entry_date", "exit_date", "entry_price", "exit_price"]].to_csv(cvf, index=False); cv_c[sd] = str(cvf)
    if feat is None:
...
# market-day maps keyed (sym, ed) per seed
SAME = {sd: {(sym, ed): same.get(ed, np.nan) for sym, ed, _ in key_c[sd]} for sd in SEEDS}
LAG1 = {sd: {(sym, ed): lag1.get(ed, np.nan) for sym, ed, _ in key_c[sd]} for sd in SEEDS}
R2L1 = {sd: {(sym, ed): r2l1.get(ed, np.nan) for sym, ed, _ in key_c[sd]} for sd in SEEDS}
...
def row(lab, cg, dd, sv):
    ...
    print(f"  {lab:46s} | {100*cg:6.1f} {100*dd:6.1f} | ΔCAGR {100*(cg-bcg):+5.1f}{mk} | {sgn}/3 | seeds {[round(100*x) for x in sv]}", flush=True)


print("--- same-day (LOOK-AHEAD, tham chiếu) ---", flush=True)
for thr in (-0.02, -0.025):
    cg, dd, sv = ev(SAME, fthr=thr); row(f"skip mkt(ngày khớp) < {thr:+.3f}", cg, dd, sv)
print("--- lag1: thị trường sập HÔM QUA (CAUSAL) ---", flush=True)
for thr in (-0.015, -0.02, -0.025, -0.03):
    cg, dd, sv = ev(LAG1, fthr=thr); row(f"skip mkt(hôm qua) < {thr:+.3f}", cg, dd, sv)
print("--- r2_lag1: 2 phiên kết thúc HÔM QUA (CAUSAL) ---", flush=True)
for thr in (-0.03, -0.04, -0.05, -0.06):
    cg, dd, sv = ev(R2L1, fthr=thr); row(f"skip mkt2p(→hôm qua) < {thr:+.3f}", cg, dd, sv)

# per-year cho filter causal tốt nhất (chọn sau khi xem, mặc định lag1<-0.02) + same-day để so
print("\n=== PER-YEAR: base vs lag1<-0.02 vs same<-0.02 ===", flush=True)


def peryear(rr):
    # rr: list per seed of (cg,dd,navdf); trả về dict year->CAGR-trung-bình-3-seed (year return)
    yd = defaultdict(list)
    for cg, dd, d in rr:
...
yl = peryear(ev(LAG1, fthr=-0.02, want_nav=True))
ys = peryear(ev(SAME, fthr=-0.02, want_nav=True))
print(f"  {'year':6s} {'base':>8s} {'lag1':>8s} {'same':>8s}", flush=True)
for y in sorted(yb):
    print(f"  {y:<6d} {100*yb[y]:+7.1f}% {100*yl.get(y,0):+7.1f}% {100*ys.get(y,0):+7.1f}%", flush=True)
print("CF2_DONE", flush=True)
