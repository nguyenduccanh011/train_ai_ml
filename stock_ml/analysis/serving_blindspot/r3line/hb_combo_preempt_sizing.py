# -*- coding: utf-8 -*-
"""STACK the two top execution levers (orthogonal?): PREEMPT (meta-prio walk-forward fill + R2 slot-
eviction, equal-weight, 89.3% CAGR) + cs-SIZING (fair mean-1 conviction weight by cross-sectional cs4
magnitude at signal-date, +sizing champion 84.8%). Priority/eviction stays meta-prio (preempt's engine);
each position sized by cs4 tilt. If orthogonal, stacks above 89.3%. 3-seed [42,21,123], K16, causal
(meta OOS + cs4 at signal-date), fair (total<=1). Baseline preempt_eq (w=1) should ~64.02.
"""
...
def prun_sized(sim, pm, cm, k_conv, prio_mode="meta", loss_thr=-0.05, margin=MARGIN, advance_fee=0.0008, roundtrip=0.006):
    """meta-prio fill + R2 eviction (preempt), each position sized by fair cs4 tilt (k_conv).
    prio_mode: 'meta' = priority by meta-pred only; 'mix' = z(meta)+z(cs4) (cs4 also drives churn)."""
    ...
    mv = [t["meta"] for t in sim.trades]; mmu = statistics.mean(mv); msd = statistics.pstdev(mv) or 1.0
    raw = []
    for t in sim.trades:
        z = (t["conv"] - mu) / sd
        t["_w"] = min(max(1.0 + k_conv * z, 0.4), 1.8); raw.append(t["_w"])
        t["prio"] = t["meta"] if prio_mode == "meta" else (t["meta"] - mmu) / msd + z
    ...
    for di, dt in enumerate(cal):
        ...
        for t in entries.get(dt, ()):
            size = (nav_now / K) * t["w"]
            if cash + 1e-12 >= size:                            # match hb_115: no len<K cap (cash self-caps)
                cash -= size; leg = mk(t, size); legs.append(leg); exits[t["exit_date"]].append(leg)
            elif legs:                                          # cash-constrained -> R2 prio-swap eviction
...
for sd in SEEDS:
    ...
    # cs4 keyed by (symbol, entry_date-str) via the SIGNAL date -> causal
    cvtr["sigd"] = pd.to_datetime(cvtr["entry_signal_date"])
    cm = {}
    for r in cvtr.itertuples():
        ek = (r.symbol, str(pd.to_datetime(r.entry_date).date()))
        cm[ek] = CS.get((r.symbol, str(r.sigd.date())), 0.5)
    cm_c[sd] = cm
    cv = HERE / f"_combo_s{sd}.csv"
...
print("STACK preempt + cs-sizing: k-sweep + cs4-in-priority, 3-seed [42,21,123] K16:", flush=True)
print("  variant                  | NAV     CAGR%   DD%   | vs preempt_eq(3seed)", flush=True)
base = None
CONFIGS = [("preempt_eq (k=0)", 0.0, "meta"), ("meta-prio +cs k1.5", 1.5, "meta"),
           ("meta-prio +cs k2.0", 2.0, "meta"), ("meta-prio +cs k2.5", 2.5, "meta"),
           ("meta-prio +cs k3.0", 3.0, "meta"), ("MIX-prio  +cs k1.5", 1.5, "mix"),
           ("MIX-prio  +cs k2.0", 2.0, "mix")]
for name, kc, pmode in CONFIGS:
    nv, cg, dd = [], [], []
    for sd in SEEDS:
        f, c, d = prun_sized(NavSim2(cv_c[sd], date_lo="2020-01-01"), pm[sd], cm_c[sd], k_conv=kc, prio_mode=pmode)
        nv.append(f); cg.append(c); dd.append(d)
    mn = statistics.mean(nv)
    if base is None:
        base = [nv[i] for i in range(len(SEEDS))]
    delt = [nv[i] - base[i] for i in range(len(SEEDS))]
    signs = "".join("+" if x > 0 else "-" for x in delt)
    print(f"  {name:24s} | x{mn:5.2f}  {statistics.mean(cg)*100:5.1f}  {statistics.mean(dd)*100:5.1f}  | "
