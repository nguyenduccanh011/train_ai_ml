# -*- coding: utf-8 -*-
"""#3 SKIP low-conviction (Q1) entries in the combo. Q1 cs4 legs contribute ~+0.25 (near-zero) with 477
legs = turnover for little P&L. Skip candidates with cs4 < threshold -> free that slot/capital for the
meta-prio churn to redeploy into better trades. Offline (base t3185 trades), combo preempt R2m01 + cs
k1.5, 3-seed [42,21,123] K16. Sweep skip {0(base),0.3,0.4,0.5}. WIN = a skip beats base 3/3."""
...
def prun(sim, pm, cm, skip=0.0, k_conv=KCONV, margin=MARGIN, advance_fee=0.0008, roundtrip=0.006):
    ...
    for t in sim.trades:
        if t["conv"] >= skip:                       # SKIP low-conviction entries
...
print("CONV-SKIP low-Q entries (combo preempt+cs k1.5), 3-seed K16:", flush=True)
print("  skip cs4< | NAV     CAGR%   DD%   | vs base(skip0)", flush=True)
store = {}
for skv in [0.0, 0.3, 0.4, 0.5]:
    nv, cg, dd = [], [], []
    for sd in SEEDS:
        f, c, d = prun(NavSim2(cv_c[sd], date_lo="2020-01-01"), pm_c[sd], cm_c[sd], skip=skv)
        nv.append(f); cg.append(c); dd.append(d)
    store[skv] = (nv, statistics.mean(cg), statistics.mean(dd))
base = store[0.0][0]
for skv in [0.0, 0.3, 0.4, 0.5]:
    nv, cg, dd = store[skv]
    delt = [nv[i] - base[i] for i in range(len(SEEDS))]; signs = "".join("+" if x > 0 else "-" for x in delt)
    print(f"  {skv:.2f}      | x{statistics.mean(nv):5.2f}  {cg*100:5.1f}  {dd*100:5.1f}  | "
