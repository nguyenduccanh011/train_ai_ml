# -*- coding: utf-8 -*-
"""Validate cs5_ma50 (cs4+atrpct+dist_ma50) before registering — from a 7-way search (FDR risk).
Cross-config: does it beat cs5 3/3 at K16/K12/K10/K8 own configs? + per-year detail at K10. Robust across
configs = real; K10-only = op-specific. Register at K10 if K10 robust + per-year consistent."""
...
CONFIGS = [("K16", 16, 0.01, 1.5), ("K12", 12, 0.01, 2.0), ("K10", 10, 0.005, 2.0), ("K8", 8, 0.01, 2.5)]
...
P["cs5"] = P[b4 + ["atrpct_r"]].mean(axis=1)
P["cs5_ma50"] = P[b4 + ["atrpct_r", "dist_ma50_r"]].mean(axis=1)
CS = {sig: {(r.symbol, str(r.date.date())): (getattr(r, sig) if pd.notna(getattr(r, sig)) else 0.5) for r in P.itertuples()} for sig in ("cs5", "cs5_ma50")}
...
for sd in SEEDS:
    ...
    cvf = HERE / f"_cm_s{sd}.csv"; cvtr[["symbol", "entry_date", "exit_date", "entry_price", "exit_price"]].to_csv(cvf, index=False); cv_c[sd] = str(cvf)
    if feat is None:
...
def evalc(sig, K, m, kc):
    navs = [prun(NavSim2(cv_c[sd], date_lo="2020-01-01"), pm[sd], mapfor(sd, sig), K, m, kc) for sd in SEEDS]
    ...
    py = {}
    for y in YEARS:
        vs = [((lambda s: s[s.index.year == y])(d.set_index("date")["nav"])) for d in navs]
        py[y] = statistics.mean([g.iloc[-1] / g.iloc[0] - 1 for g in vs if len(g) > 2])
    return fin, cg, dd, py


print("=== cs5 -> cs5_ma50 cross-config (own configs), 3-seed ===", flush=True)
print(f"  {'cfg':5s} | cs5 CAGR/DD | cs5_ma50 CAGR/DD | Δpp | 3/3?", flush=True)
k10 = None
for lab, K, m, kc in CONFIGS:
    f5, c5, d5, _ = evalc("cs5", K, m, kc); fm, cm, dm, pym = evalc("cs5_ma50", K, m, kc)
    rob = all(fm[i] > f5[i] for i in range(len(SEEDS)))
    print(f"  {lab:5s} | {100*c5:5.1f}/{100*d5:5.1f} | {100*cm:5.1f}/{100*dm:5.1f} | {100*(cm-c5):+4.1f} | {'3/3' if rob else 'x'}", flush=True)
    if lab == "K10":
        k10 = (fm, cm, dm, pym, rob)
        _, c5b, _, py5b = evalc("cs5", K, m, kc); py5_k10 = py5b
fm, cm, dm, pym, rob10 = k10
wins = sum(1 for y in YEARS if pym[y] >= py5_k10[y] - 0.005)
print(f"\n  K10 cs5_ma50 per-year vs cs5: {wins}/7 >= ; 3/3={rob10}", flush=True)
...
async def make_tmpl(name, desc):
    ...
    async with S2() as s:
        ...
        t = await repo.create(name=name, market=base.market, strategy=base.strategy, feature_set_id=base.feature_set_id,
            ...
            validation_config=base.validation_config, seed=42, description=desc, hypothesis="cs5+dist_ma50 (long-trend tilt)",
            universe_slug=base.universe_slug, model_mode="ml_only")
        await s.commit(); return t.id


if rob10 and wins >= 5:
    navm = statistics.mean(fm)
    name = "x2_struct_to_k10_cs5ma50"
    desc = (f"[K10 + preempt m0.005 + cs5ma50-conv k2.0 (cs4+ATRPCT+dist_MA50) + conv-skip0.40 — TONG VON<=1] "
            f"Free-exploration extension cua cs5: them dist_ma50 (trend dai 50-phien) = tilt trend-following. "
            f"CAGR ~{100*cm:.0f}%/DD{100*dm:.1f} vs cs5 (+{100*(cm-c5):.1f}pp, 3/3 seed, per-year {wins}/7, DD tot hon). "
            f"CAVEAT: tu 7-way search (FDR cao hon cs5 forensic-derived) — passed 3/3+per-year nhung provenance yeu hon, "
            f"nen validate them OOS. Fair same-cond raw CAGR.")
    ...
    ch = hashlib.md5(f"{r2}_cs5ma50".encode()).hexdigest()[:16]
    cur.execute("""insert into leaderboard_nav (run_id,nav_adv,nav_noadv,cagr_adv,cagr_noadv,maxdd_nav,nav_f22_adv,years,n_trades_sim,config_hash,computed_at)
                ...
                (r2, navm, navm, cm, cm, dm, navm, 6.51, 0, ch))
    cur.execute("update leaderboard_runs set state='trained', superseded=false where run_id=%s", (r2,))
    con2.commit(); con2.close()
    print(f"REGISTERED {name}: CAGR {100*cm:.1f}% DD {100*dm:.1f}% NAV x{navm:.1f}", flush=True)
else:
    print(f"NOT registered (rob10={rob10} wins={wins}/7)", flush=True)
print("CSMA50_VALID_DONE", flush=True)
