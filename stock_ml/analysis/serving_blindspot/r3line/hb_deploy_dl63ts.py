...
# time-stop + hard stop-loss levers (applied in the exit-rewrite below). Forensic-derived:
# TIMESTOP=(N, mfe_min): if by session N in-trade mfe < mfe_min -> sell N+1 (cut dead-money early,
#   free the slot). Wins on blue-chip 61-sym (+2.7pp CAGR 3/3 seed) but does NOT generalize to
#   the wide top-400 universe (mid/small consolidate longer before running). STOPLOSS: sell next
#   close once close<=entry*(1-SL) (defensive, trims DD ~1pp).
TIMESTOP = None; STOPLOSS = None
...
raw_c = {}  # cache RAW base trades per seed (before exit-rewrite) so rewrite can be re-run per variant
base = sigdf = bt = None
for sd in SEEDS:
    rid = run_template_experiment(template_id=3185, seed=sd).get("run_id")
    cvtr0 = pd.read_sql("select symbol,entry_date,exit_date,entry_price,exit_price,entry_signal_date,exit_reason,pnl_pct from run_trades where run_id=%s and exit_date is not null", con, params=(rid,))
    cvtr0["sigd"] = pd.to_datetime(cvtr0["entry_signal_date"]); cvtr0["ed"] = cvtr0["entry_date"].astype(str)
    raw_c[sd] = cvtr0
    if feat is None:
        feat = M.features(cvtr0.symbol.unique().tolist())
...
def build_seeds():
    """Rewrite RAW base per seed under current GT/TIMESTOP/STOPLOSS globals, rebuild cv/cm/r5/dl63
    maps + seed42 base/bt for portfolio. Re-runnable per variant (levers act at exit-rewrite)."""
    global base, bt
    GT = 0.08
    for sd in SEEDS:
        cvtr = raw_c[sd].copy(); ned, nep, nreason = [], [], []
        for r in cvtr.itertuples():
            ...
            if xi > ei + 3 and c[ei + 2] / r.entry_price - 1.0 < 0.0:      # red@s2 -> early-cut phien3
                base_k, base_p, base_r = ei + 3, float(c[ei + 3]), "early_cut"
            else:                                                          # green@s2 -> green-trail 8%
                base_k = xi; base_p = r.exit_price; base_r = r.exit_reason; peak = c[ei]
                for b in range(ei + 2, xi + 1):
                    peak = max(peak, c[b])
                    if c[b] <= peak * (1 - GT) and b >= ei + 2:
                        base_k, base_p = (b + 1, float(c[b + 1])) if b + 1 <= xi else (b, float(c[b]))
                        base_r = "green_trail"; break
            cands = [(base_k, base_p, base_r)]
            if STOPLOSS is not None:
                thr = r.entry_price * (1 - STOPLOSS)
                for b in range(ei + 1, xi + 1):
                    if c[b] <= thr:
                        sk = b + 1 if b + 1 <= xi else b; cands.append((sk, float(c[sk]), "stop_loss")); break
            if TIMESTOP is not None:
                N, mfe_min = TIMESTOP; bN = ei + N
                if bN < xi and (max(c[ei:bN + 1]) / r.entry_price - 1.0) < mfe_min:
                    tk = bN + 1 if bN + 1 <= xi else bN; cands.append((tk, float(c[tk]), "time_stop"))
            k, p, rs = min(cands, key=lambda x: x[0])
            ned.append(INV[r.symbol][k]); nep.append(p); nreason.append(rs)
...
# ============================================================================
DL63_HASH_SALT = "r7ec_gtrail_dl63_timestop"
...
# ---- 1 variant: dl63bal + forensic time-stop + stop-loss (blue-chip 61-sym only) ----
VARIANTS = [
    dict(name="x2_struct_to_k10_cs5ma50_r7ec_gt_dl63bal_ts", th=0.13, mode="skip", size=1.0,
         ts=(10, 0.05), sl=0.08,
         tag=("[DL63-BAL+TIMESTOP forensic] Champion gtrail + dl63bal SKIP(dist_l63<0.13) "
              "+ TIME-STOP (cat lenh chon-von: phien 10 chua dat mfe 5% -> ban) + STOP-LOSS 8%. "
              "Time-stop = phat hien forensic (cat lenh chet som de giai phong slot); +2.7pp CAGR 3/3 seed "
              "tren 61-ma blue-chip. 3-seed T+2 CAGR ~140%, DD~-12.8%, Calmar ~10.9. "
              "LUU Y: chi thang tren BLUE-CHIP 61-ma; KHONG generalize sang tap rong top-400 "
              "(ma mid/small tich luy lau hon truoc khi chay -> time-stop cat oan). Dung cho universe blue-chip.")),
]

cur = con.cursor()
for V in VARIANTS:
    globals()["DL63_TH"] = V["th"]; globals()["DL63_MODE"] = V["mode"]; globals()["DL63_SIZE"] = V["size"]
    globals()["TIMESTOP"] = V.get("ts"); globals()["STOPLOSS"] = V.get("sl")
    build_seeds()  # re-run exit-rewrite under this variant's time-stop/stop-loss
