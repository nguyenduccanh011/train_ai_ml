...
NAME = "x2_struct_to_k10_cs5ma50_r7earlycut"; PRE = "template/"; SUF = "-69338138"
...
for sd in SEEDS:
    ...
    for r in cvtr.itertuples():
        ...
        if ei is not None and xi is not None and xi > ei + 3 and CLO[r.symbol][ei + 2] / r.entry_price - 1.0 < 0.0:
            ned.append(INV[r.symbol][ei + 3]); nep.append(float(CLO[r.symbol][ei + 3])); nreason.append("early_cut")
        else:
            ned.append(r.exit_date); nep.append(r.exit_price); nreason.append(r.exit_reason)
...
# ---- 3-seed leaderboard metrics: BOTH T+0 (cagr_adv) and T+2 (cagr_t2), gated ----
def metrics(tplus, lo="2020-01-01"):
    globals()["TPLUS"] = tplus
    navs = [prun_metric(NavSim2(cv_c[sd], date_lo=lo), pm[sd], cm_c[sd], r5_c[sd]) for sd in SEEDS]
...
DESC = (f"[TOP: K10/cs5_ma50 + RET7-GATE + EARLY-CUT — CAGR(T+2) THUC TE VN] Ret7-gate ne entry momentum "
        f"7-phien yeu (ret7<{R5THR}) + EARLY-CUT: quan sat DO tai phien 2 (close<gia vao) -> BAN phien 3 "
        f"(khop quy uoc T+1-exit cua model, causal tuyet doi, T+2-hop le). Cot cagr_adv=T+0 ({100*CG0:.0f}%), "
        f"cagr_t2=T+2 thuc te ({100*CG5:.0f}%). Early-cut VERIFIED: +4.4pp vs ret7-only tren T+2 realistic, 3/3 "
        f"seed, 3/3 cross-K (K10/K12/K16), DD phang. Co che: do-som(e3<0)=20% win -> cat gan nhu khong mat "
        f"upside, chan bleed ~5 phien + giai phong slot som; cuu MANH nam momentum yeu (2022+14/2024+12). Day "
        f"la cach monetize capture-leak (khac take-profit cat-PHAI da fail). corr(ret-3-phien-dau,net)=+0.65. "
        f"Fair 61-sym, TONG VON<=1. Model manh nhat hien tai.")
...
ch = hashlib.md5(f"{RID}_r7earlycut".encode()).hexdigest()[:16]
