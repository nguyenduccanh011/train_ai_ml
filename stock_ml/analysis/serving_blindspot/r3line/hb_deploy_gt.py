...
NAME = "x2_struct_to_k10_cs5ma50_r7ec_gtrail"; PRE = "template/"; SUF = "-69338138"
...
DESC = (f"[TOP MANH NHAT: K10/cs5_ma50 + RET7 + EARLY-CUT + GREEN-TRAIL — CAGR(T+2)] Ret7-gate + exit 2-tang: "
        f"RED-early (do phien2) -> EARLY-CUT ban phien3 (chan bleed loser); GREEN-early (xanh phien2) -> GREEN-TRAIL "
        f"rong 8% tu dinh -> ban phien ke (bat nhom 'len-roi-crash' residual ma early-cut bo sot, khong whipsaw dip nho). "
        f"Causal T+1. cagr_adv=T+0 ({100*CG0:.0f}%), cagr_t2=T+2 ({100*CG5:.0f}%). Green-trail VERIFIED +3.9pp vs "
        f"r7earlycut: plateau 7-9% deu 3/3 seed, cross-K config-specific (K10 3/3, K12/K16 neutral); per-year cuu "
        f"2020+15/2023+7/2024+6/2025+9, hai 2021-19 (bull cat runner). Tach chinh sach: cat-TRAI nhanh (loser) + "
        f"trail-RONG (winner reversal). Tu insight user (nhom xanh-2-phien roi vao chuoi do). Fair 61-sym, TONG VON<=1.")
...
ch = hashlib.md5(f"{RID}_r7ec_gtrail".encode()).hexdigest()[:16]
