...
NAME = "x2_struct_to_k10_cs5ma50_r7ec_gt_s1"; PRE = "template/"; SUF = "-69338138"
...
for sd in SEEDS:
    ...
    for r in cvtr.itertuples():
        ...
        c = CLO[r.symbol]
        if xi > ei + 2 and c[ei + 1] / r.entry_price - 1.0 < 0.0:          # red@s1 -> early-cut phien2 (som nhat T+2)
            ned.append(INV[r.symbol][ei + 2]); nep.append(float(c[ei + 2])); nreason.append("early_cut")
...
DESC = (f"[DEFENSIVE best-Calmar: r7ec_gtrail nhung SPLIT@PHIEN1] Nhu r7ec_gtrail nhung phan loai red/green tai "
        f"PHIEN 1 (thay vi 2): red@phien1 -> early-cut ban phien2 (som nhat T+2, it bleed nhat); green@phien1 -> "
        f"green-trail 8%. cagr_adv=T+0 ({100*CG0:.0f}%), cagr_t2=T+2 ({100*CG5:.0f}%). So voi split@phien2 (131.7/DD-16.1): "
        f"CAGR nhinh thap ~1pp NHUNG DD -12.9% THAP NHAT board + Calmar 10.1 CAO NHAT (cat loser ngay phien2=san T+2 "
        f"-> chan bleed toi da). Diem manh rieng = DD/rui-ro thap (operating-point PHONG THU). session3 kem (qua tre). "
        f"Tu cau hoi user 'y nghia phien 1/2/3'. Fair 61-sym, TONG VON<=1.")
...
ch = hashlib.md5(f"{RID}_r7ec_gt_s1".encode()).hexdigest()[:16]
