...
NAME = "x2_struct_to_k10_cs5ma50_r7ec_osdef"; PRE = "template/"; SUF = "-69338138"
...
DESC = (f"[DEFENSIVE low-DD: r7earlycut + OVERSHOOT-FILTER — CAGR(T+2)] Ban phong thu cua r7earlycut: them "
        f"loc OVERSHOOT (bo entry co low tut sau duoi gia khop = falling-knife, overshoot>top10%; biet luc khop, "
        f"causal). Overshoot AUC 0.614 du bao thua. DD GIAM 3/3 seed (-16.5 -> ~-14.0), Calmar 7.7->9.5. CAGR "
        f"cagr_adv=T+0 ({100*CG0:.0f}%), cagr_t2=T+2 ({100*CG5:.0f}%) — CAGR mean cao hon r7earlycut nhung chi 2/3 "
        f"(khong robust), DIEM MANH RIENG = DD/rui-ro thap 3/3. Nen la operating-point PHONG THU. Nen tang: ret7-gate "
        f"+ early-cut (do-som phien2 ban phien3) + overshoot-filter. Fair 61-sym, TONG VON<=1. "
...
ch = hashlib.md5(f"{RID}_r7ec_osdef".encode()).hexdigest()[:16]
