...
K, MARGIN, KCONV, TPLUS, R5THR = 10, 0.005, 2.0, 2, 0.03; PB_PCT, PB_WIN = 0.045, 40
NAME = "x2_struct_to_k10_cs5ma50_t2ret5g"; PRE = "template/"; SUF = "-69338138"
...
# ---- 3-seed leaderboard metrics (T+2 + gate) ----
navs = [prun_metric(NavSim2(cv_c[sd], date_lo="2020-01-01"), pm[sd], cm_c[sd], r5_c[sd]) for sd in SEEDS]
fin = [float(d["nav"].iloc[-1]) for d in navs]; yrs = (navs[0]["date"].iloc[-1] - navs[0]["date"].iloc[0]).days / 365.25
NAV5 = statistics.mean(fin); CG5 = statistics.mean(f ** (1 / yrs) - 1 for f in fin)
DD5 = statistics.mean(float((d["nav"] / d["nav"].cummax() - 1).min()) for d in navs)
navs22 = [prun_metric(NavSim2(cv_c[sd], date_lo="2022-01-01"), pm[sd], cm_c[sd], r5_c[sd]) for sd in SEEDS]
NAVF22 = statistics.mean(float(d["nav"].iloc[-1]) for d in navs22)
print(f"3-seed T+2+ret5gate: CAGR {100*CG5:.1f}% DD {100*DD5:.1f}% NAV x{NAV5:.2f} f22 x{NAVF22:.2f}", flush=True)


# ---- register template + leaderboard row ----
DESC = (f"[TOP model K10/cs5_ma50 + RET5-GATE (ne entry momentum yeu) — DUOI T+2 THUC TE VN] "
        f"Ne cac entry co ret5<{R5THR} (co chua tang >={100*R5THR:.0f}%/5 phien) = nhom churn 3-5 phien lo. "
        f"CAGR o day tinh DUOI T+2 (khac cac dong T+0 ly thuyet). Baseline cung model duoi T+2 = 111%. "
        f"Gate PASSED: 3/3 seed (K10 & K12), per-year duong-hoac-trung-tinh MOI nam (khong hai nam nao), "
        f"config-specific (K16 champ that bai -> gate hop cho model tap trung). Loai ~29% book ~ dung cohort "
        f"short-hold-lo. exit_score tai ngay mua vo dung du bao churn (AUC 0.532) -> gate = momentum-yeu, "
        f"khong phai tin-hieu-ban. Multi-seed CAGR ~{100*CG5:.0f}%/DD{100*DD5:.1f}. Fair 61-sym, TONG VON<=1.")
...
async def make_tmpl():
    ...
    async with S2() as s:
        ...
        t = await repo.create(name=NAME, market=b.market, strategy=b.strategy, feature_set_id=b.feature_set_id,
            ...
            hypothesis="T+2 ret5 entry-momentum gate (avoid short-hold churn)", universe_slug=b.universe_slug, model_mode="ml_only")
...
ch = hashlib.md5(f"{RID}_t2ret5g".encode()).hexdigest()[:16]
cur.execute("""insert into leaderboard_nav (run_id,nav_adv,nav_noadv,cagr_adv,cagr_noadv,maxdd_nav,nav_f22_adv,years,n_trades_sim,config_hash,computed_at)
               ...
               cagr_adv=excluded.cagr_adv, maxdd_nav=excluded.maxdd_nav, nav_f22_adv=excluded.nav_f22_adv, config_hash=excluded.config_hash, computed_at=now()""",
            (RID, NAV5, NAV5, CG5, CG5, DD5, NAVF22, round(yrs, 2), 0, ch))
...
for r in bt.itertuples():
    ...
    if conv < SKIP:
        ...
        skipped.append((RID, r.symbol, str(r.sigd.date()) if pd.notna(r.sigd) else None, r.ed, float(r.pnl_pct or 0.0), float(conv), "ret5_gate"))
...
con.commit()
ng = sum(1 for x in skipped if x[6] == "ret5_gate")
print(f"PORTFOLIO {RID}: eq={len(equity)} hold={len(holdings)} trades={len(trades)} skipped={len(skipped)} (ret5_gate={ng}) pending={len(pending)}", flush=True)
con.close()
