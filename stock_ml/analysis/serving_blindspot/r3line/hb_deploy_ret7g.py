...
NAME = "x2_struct_to_k10_cs5ma50_t2ret7g"; PRE = "template/"; SUF = "-69338138"
...
DESC = (f"[TOP model K10/cs5_ma50 + RET7-GATE (ne entry momentum 7-phien yeu) — CAGR(T+2) THUC TE VN] "
        f"Ne cac entry co ret7<{R5THR} (co chua tang >={100*R5THR:.0f}%/7 phien) = nhom churn 3-5 phien lo. "
        f"Cot cagr_adv=T+0 ly thuyet ({100*CG0:.0f}%), cot cagr_t2=T+2 thuc te ({100*CG5:.0f}%). Baseline model "
        f"duoi T+2=111% -> gate +{100*(CG5-1.11):.0f}pp. Sweep window: ret7 = DINH cua ridge phang (N=4,5,7 deu "
        f"3/3), VUOT ret5<0.03 (121.3%). Gate PASSED: cross-K K10 3/3 moi thr + K12 3/3 moi thr (robust hon ret5), "
        f"K16 fail (gate=concentrated-only); per-year duong/trung-tinh gan moi nam (te nhat -1.3pp 2023, CUU them "
        f"2022 +29pp). Loai ~book churn short-hold-lo. Fair 61-sym, TONG VON<=1. Song song voi ban ret5.")
...
ch = hashlib.md5(f"{RID}_t2ret7g".encode()).hexdigest()[:16]
