# -*- coding: utf-8 -*-
"""#1 HOLD-EXTENSION x PREEMPTION. Profit engine = 21-40d holds (+8.6%); max_hold=14 caps it. Prior
max_hold-extension was NULL/neg due to SLOT-LOCKUP (hold longer -> fewer slots). But the combo has
PREEMPTION which frees slots by evicting laggards -> the blocker is relaxed. Clone t3185 with
max_hold_bars {20,25}, generate longer-hold trades, combo-score (preempt R2m01 + cs k1.5), 3-seed
[42,21,123]. WIN = a longer max_hold beats mh14 champion 3/3."""
...
BASE = 3185; SEEDS = [42, 21, 123]; K = 16; KCONV = 1.5; MARGIN = 0.01; CS4 = ["dist20low", "dist_ma20", "rsi14", "ret20"]
VARIANTS = {"minh5": 5, "minh8": 8}   # name -> min_hold_bars (suppress <5d whipsaw drag)
KNOB = "min_hold_bars"
    ...
    async with Session() as s:
        ...
        for nm, mh in VARIANTS.items():
            ...
            t = await repo.create(name=nm, market=base.market, strategy=base.strategy, feature_set_id=base.feature_set_id,
                                  ...
                                  split_config=base.split_config, engine_config=ec, validation_config=base.validation_config,
                                  seed=base.seed, description=f"frontier + {KNOB} {mh}",
                                  hypothesis="min-hold suppresses <5d whipsaw drag (-1.6% bucket)",
...
ids = asyncio.run(make()); asyncio.run(async_engine.dispose())
TEMPLATES = [("mh14 (base)", BASE)] + [(nm, ids[nm]) for nm in VARIANTS]
...
print("\n=== HOLD-EXTENSION x PREEMPTION (combo), 3-seed [42,21,123], vs mh14 base, sign 3/3 ===", flush=True)
base = res["mh14 (base)"]
for nm, _ in TEMPLATES:
    navs = [res[nm][s][0] for s in SEEDS]; cg = statistics.mean([res[nm][s][1] for s in SEEDS])
    dd = statistics.mean([res[nm][s][2] for s in SEEDS])
    d = [res[nm][s][0] - base[s][0] for s in SEEDS]; signs = "".join("+" if x > 0 else "-" for x in d)
    print(f"  {nm:12s}: NAV x{statistics.mean(navs):5.2f}  CAGR {cg*100:5.1f}%  DD {dd*100:5.1f}%  | "
          f"Δ={[f'{x:+.1f}' for x in d]} {signs}", flush=True)
print("HOLDEXT_COMBO_DONE")
