# -*- coding: utf-8 -*-
"""Sweep K x conviction-k on champion combo to find higher-CAGR robust operating points to register.
Same combo (preempt R2m01 + cs4-conviction + conv-skip 0.40, total<=1). K in {8,10,12,16} x kconv in
{1.5,2.0,2.5}. 3-seed [42,21,123]. Report CAGR/DD/exposure + 3/3-robust-vs-K16k1.5. NOT new alpha —
extends the CAGR<->DD dial (deeper DD for higher CAGR)."""
...
# baseline K16 k1.5 per-seed (robustness ref)
ref = [prun(NavSim2(cv_c[sd], date_lo="2020-01-01"), pm[sd], cm_c[sd], 16, 1.5)[0] for sd in SEEDS]
print("SWEEP K x kconv (champion combo, 3-seed). ref=K16/k1.5 NAV", [f"{x:.0f}" for x in ref], flush=True)
print(f"  {'K/kconv':12s} | CAGR%  DD%   expo | 3seed-NAV        | robust-vs-K16k1.5", flush=True)
best = None
for K in (8, 10, 12):
    for kc in (1.5, 2.0, 2.5):
        nv, cg, dd, ex = [], [], [], []
        for sd in SEEDS:
            f, c, d, e = prun(NavSim2(cv_c[sd], date_lo="2020-01-01"), pm[sd], cm_c[sd], K, kc)
            nv.append(f); cg.append(c); dd.append(d); ex.append(e)
        rob = all(nv[i] > ref[i] for i in range(len(SEEDS)))
        mcg = statistics.mean(cg)
        print(f"  K{K}/k{kc:<4} | {mcg*100:5.1f} {statistics.mean(dd)*100:5.1f} {statistics.mean(ex):.2f} | "
              f"{[f'{x:.0f}' for x in nv]} | {'3/3' if rob else 'no '}", flush=True)
        if rob and (best is None or mcg > best[1]):
            best = (f"K{K}/k{kc}", mcg, statistics.mean(dd))
print(f"\nBEST robust max-CAGR: {best[0]} = {best[1]*100:.1f}% CAGR / DD {best[2]*100:.1f}%", flush=True)
