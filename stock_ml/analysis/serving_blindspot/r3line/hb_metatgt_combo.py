# -*- coding: utf-8 -*-
"""Churn-priority SIGNAL-QUALITY axis (analog to magnitude-signal winning for sizing): the meta-model
drives the preempt CHURN priority; it currently regresses t_pnl. Test t_vel (velocity = free slots
faster -> more compounding), t_log (tame outliers), t_win (classification) for the churn priority, in
the combo (preempt + cs-sizing k1.5). 3-seed [42,21,123] K16. WIN = a target beats t_pnl 3/3."""
...
SEEDS = [42, 21, 123]; K = 16; MARGIN = 0.01; KCONV = 1.5
CS4 = ["dist20low", "dist_ma20", "rsi14", "ret20"]; TGTS = ["t_pnl", "t_vel", "t_log", "t_win"]
...
print("CHURN-PRIORITY meta-target sweep (combo preempt + cs k1.5), 3-seed [42,21,123] K16:", flush=True)
print("  meta target | NAV     CAGR%   DD%   | vs t_pnl", flush=True)
base = None
for tg in TGTS:
    pm = {sd: M.meta_preds(tr_c[sd], tgt=tg) for sd in SEEDS}
    nv, cg, dd = [], [], []
    for sd in SEEDS:
        f, c, d = prun(NavSim2(cv_c[sd], date_lo="2020-01-01"), pm[sd], cm_c[sd])
    ...
    tag = " (baseline)" if tg == "t_pnl" else ""
    print(f"  {tg:9s}   | x{mn:5.2f}  {statistics.mean(cg)*100:5.1f}  {statistics.mean(dd)*100:5.1f}  | "
          f"{[f'{x:+.1f}' for x in delt]} {signs}{tag}", flush=True)
print("METATGT_DONE")
