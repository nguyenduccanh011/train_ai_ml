# -*- coding: utf-8 -*-
"""Populate 'Danh muc' (run_equity + run_portfolio_daily + run_pending + run_skipped) for the NEW aggressive
K-variants so their detail page shows real daily holdings, letting the user compare portfolio behaviour vs
champion K16. SAME combo, only (K, conviction-k) differ per model. Faithful per-model (holdings/pending are
K-dependent). seed42."""
...
SPECS = [
    ("template/x2_struct_to_k8preempt_cssize-69338138", 8, 2.5, 0.01),
    ("template/x2_struct_to_k10preempt_cssize-69338138", 10, 1.5, 0.01),
    ("template/x2_struct_to_k12size2-69338138", 12, 2.0, 0.01),
]


def prun_daily(sim, pm, cm, K, k_conv, margin, RID, advance_fee=0.0008, roundtrip=0.006):
...
def compute_skipped(RID):
    out = []
    for r in bt.itertuples():
        conv = cm.get((r.symbol, str(pd.to_datetime(r.entry_date).date())), 0.5)
        if conv < SKIP:
            out.append((RID, r.symbol, str(r.sigd.date()) if pd.notna(r.sigd) else None,
...
def compute_pending(RID, caldates, held_by_date):
    bar_of = {d: i for i, d in enumerate(caldates)}; nD = len(caldates)
    low_arr, close_arr = {}, {}
    for s, g in px.groupby("symbol"):
    ...
    for sym, sbars in sig_bars.items():
        ...
        for d in range(nD):
            ...
            if _np.isnan(cD):
                continue
            pctL = limit / cD - 1.0; hi = min(S + PB_WIN, nD - 1); seg = la[S:hi + 1]
...
cur = con.cursor()
for RID, K, KCONV, MARGIN in SPECS:
    equity, holdings, held_by_date = prun_daily(NavSim2(str(cv), date_lo="2020-01-01"), pm, cm, K, KCONV, MARGIN, RID)
    caldates = [pd.to_datetime(e[1]) for e in equity]
    skipped = compute_skipped(RID); pending = compute_pending(RID, caldates, held_by_date)
    ...
    con.commit()
    navx = equity[-1][2]; avg_npos = statistics.mean(e[5] for e in equity); avg_exp = statistics.mean(e[4] for e in equity)
    print(f"  {RID.split('/')[-1]:34s} K{K}/k{KCONV}: equity={len(equity)} holdings={len(holdings)} "
          f"| NAVx{navx:.1f} avg_npos={avg_npos:.1f} avg_expo={avg_exp:.2f}", flush=True)
con.close()
print("PORTFOLIO_NEW_DONE", flush=True)
