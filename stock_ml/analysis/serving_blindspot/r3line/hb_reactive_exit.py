# -*- coding: utf-8 -*-
"""More POST-entry reactive-exit ideas on top of r7earlycut (127.8%). Attack the capture-leak (winners give
back ~66%) with REACTIVE exits (not fixed take-profit which failed): (1) TRAILING-from-peak (ride, exit on
X% drop from running peak); (2) TIME-STOP dead-money (held N sessions but still flat/below +minup -> cut,
free capital). All combined with early-cut, earliest-trigger, T+2-legal. K10/cs5_ma50+ret7, 3-seed."""
...
for sd in SEEDS:
    ...
    cvf = HERE / f"_re_s{sd}.csv"; cvtr[["symbol", "entry_date", "exit_date", "entry_price", "exit_price"]].to_csv(cvf, index=False); cv_c[sd] = str(cvf)
    cvtrs[sd] = cvtr
    if feat is None:
...
def make_csv(sd, trail=None, ts_n=None, ts_up=0.0, earlycut=True):
    cv = cvtrs[sd]; nd, npx = [], []
    for r in cv.itertuples():
        ...
        c = CLO[r.symbol]; exit_k = xi; exit_p = r.exit_price
        # early-cut (obs2/sell3)
        if earlycut and xi > ei + 3 and c[ei + 2] / r.entry_price - 1.0 < 0.0:
            exit_k = ei + 3; exit_p = float(c[ei + 3])
        # trailing-from-peak (>= session TPLUS)
        if trail is not None:
            peak = c[ei]
            for k in range(ei + 1, min(exit_k, xi) + 1):
                peak = max(peak, c[k])
                if k >= ei + TPLUS and c[k] <= peak * (1 - trail):
                    if k < exit_k:
                        exit_k = k; exit_p = float(c[k])
                    break
        # time-stop dead-money at session ts_n if still below +ts_up
        if ts_n is not None and xi > ei + ts_n and (ei + ts_n) < exit_k and c[ei + ts_n] / r.entry_price - 1.0 < ts_up:
            exit_k = ei + ts_n; exit_p = float(c[ei + ts_n])
        nd.append(INV[r.symbol][exit_k]); npx.append(exit_p)
    out = cv[["symbol", "entry_date", "exit_date", "entry_price", "exit_price"]].copy()
    out["exit_date"] = pd.to_datetime(nd); out["exit_price"] = npx
    f = HERE / f"_re_v_s{sd}.csv"; out.to_csv(f, index=False); return str(f)


def ev(**kw):
    rr = [prun(NavSim2(make_csv(sd, **kw), date_lo="2020-01-01"), pm[sd], CMS[sd], MW[sd]) for sd in SEEDS]
    return statistics.mean([c for c, d in rr]), statistics.mean([d for c, d in rr]), [c for c, d in rr]


ecg, edd, es = ev()  # r7earlycut baseline
print(f"=== REACTIVE-EXIT trên r7earlycut (T+2, K10/cs5_ma50+ret7) — base {100*ecg:.1f}%/DD{100*edd:.1f} ===", flush=True)
print(f"  {'variant':34s} | CAGR%  DD%  | vs ec | 3/3", flush=True)
def row(lab, cg, dd, sv):
    sgn = sum(1 for i in range(3) if sv[i] > es[i]); mk = "*" if (sgn == 3 and cg > ecg) else ("+" if sgn == 3 else " ")
    print(f"  {lab:34s} | {100*cg:5.1f} {100*dd:5.1f} | {100*(cg-ecg):+5.1f}{mk} | {sgn}/3", flush=True)
for tr in (0.08, 0.12, 0.15, 0.20):
    cg, dd, sv = ev(trail=tr); row(f"[trail-peak] {int(100*tr)}%", cg, dd, sv)
for n in (5, 8, 10):
    for up in (0.0, 0.02):
        cg, dd, sv = ev(ts_n=n, ts_up=up); row(f"[time-stop] phien{n} nếu<+{int(100*up)}%", cg, dd, sv)
print("REACTIVE_DONE", flush=True)
