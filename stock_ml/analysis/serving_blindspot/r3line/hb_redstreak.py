# -*- coding: utf-8 -*-
"""Residual case-2: trades GREEN at session 2 (early-cut misses) that later start a RED STREAK and crash.
Can a rolling 'cut on red-streak after initial rise' catch them WITHOUT whipsawing winners? Diagnostic
(green-early winners vs losers: do losers show earlier red-streak?) + NAV (early-cut + K-consecutive-red
cut) vs early-cut 127.8 / base 123.4. Operating K10/cs5_ma50+ret7 T+2, 3-seed."""
...
for sd in SEEDS:
    ...
    cvf = HERE / f"_rs_s{sd}.csv"; cvtr[["symbol", "entry_date", "exit_date", "entry_price", "exit_price"]].to_csv(cvf, index=False); cv_c[sd] = str(cvf)
    cvtrs[sd] = cvtr
    if feat is None:
...
# diagnostic seed42: green@s2 -> winner vs loser, khi nào chuỗi đỏ đầu tiên
cv = cvtrs[42]; g_rows = []
for r in cv.itertuples():
    di = DIDX.get(r.symbol, {}); ei = di.get(r.ed); xi = di.get(str(r.exit_date)[:10])
    if ei is None or xi is None or xi <= ei + 4:
        continue
    c = CLO[r.symbol]; ep = r.entry_price; e2 = c[ei + 2] / ep - 1
    if e2 <= 0:
        continue  # chỉ xét green@s2 (early-cut không bắt)
    net = r.exit_price / ep - 1
    # phiên đầu tiên có 2 nến đỏ liên tiếp (sau phiên 2)
    fs = 99
    for b in range(ei + 3, xi + 1):
        if b - 1 >= ei and c[b] < c[b - 1] and c[b - 1] < c[b - 2]:
            fs = b - ei; break
    g_rows.append(dict(net=net, win=1 if net > 0 else 0, streak_at=fs))
G = pd.DataFrame(g_rows)
print(f"=== Green@phiên2 (early-cut KHÔNG bắt): {len(G)} lệnh, thắng {100*G.win.mean():.0f}% ===", flush=True)
w = G[G.win == 1]; ll = G[G.win == 0]
print(f"  phiên có 2-nến-đỏ-liên-tiếp đầu tiên: WINNER TB {w.streak_at.replace(99,np.nan).mean():.1f} | LOSER TB {ll.streak_at.replace(99,np.nan).mean():.1f}", flush=True)
print(f"  % có chuỗi-đỏ SỚM (≤5 phiên): winner {100*(w.streak_at<=5).mean():.0f}% | loser {100*(ll.streak_at<=5).mean():.0f}%", flush=True)


def cut_csv(sd, streakK=None, drop_from_hi=None):
    cv = cvtrs[sd]; nd, npx = [], []
    for r in cv.itertuples():
        ...
        if xi > ei + 3 and c[ei + 2] / ep - 1.0 < 0.0:
            ...
            # red-streak cut cho green@s2: K nến đỏ liên tiếp sau phiên2 -> bán phiên kế
            if streakK is not None:
                for b in range(ei + 2 + streakK, min(ek, xi) + 1):
                    if all(c[b - j] < c[b - j - 1] for j in range(streakK)) and b >= ei + TPLUS:
                        if b + 1 <= xi and b + 1 < ek:
                            ek = b + 1; ep_ = float(c[b + 1])
                        break
            if drop_from_hi is not None:
                peak = c[ei]
                for b in range(ei + 2, min(ek, xi) + 1):
                    peak = max(peak, c[b])
                    if c[b] <= peak * (1 - drop_from_hi) and b >= ei + TPLUS:
                        if b + 1 <= xi and b + 1 < ek:
                            ek = b + 1; ep_ = float(c[b + 1])
                        break
    ...
    f = HERE / f"_rs_c_s{sd}.csv"; out.to_csv(f, index=False); return str(f)


def ev(**kw):
    rr = [prun(NavSim2(cut_csv(sd, **kw), date_lo="2020-01-01"), pm[sd], CMS[sd], MW[sd]) for sd in SEEDS]
    return statistics.mean([c for c, d in rr]), statistics.mean([d for c, d in rr]), [c for c, d in rr]


ecg, edd, es = ev()  # early-cut only
print(f"\n=== NAV: early-cut + red-streak-cut cho green-early (early-cut only = {100*ecg:.1f}%/DD{100*edd:.1f}) ===", flush=True)
def row(lab, cg, dd, sv):
    sgn = sum(1 for i in range(3) if sv[i] > es[i]); mk = "*" if (sgn == 3 and cg > ecg) else ("+" if sgn == 3 else " ")
    print(f"  {lab:30s} | CAGR {100*cg:5.1f}%{mk} ({100*(cg-ecg):+4.1f}, {sgn}/3) DD {100*dd:5.1f}%", flush=True)
for kk in (2, 3):
    cg, dd, sv = ev(streakK=kk); row(f"+ {kk} nến đỏ liên tiếp", cg, dd, sv)
for dh in (0.05, 0.08):
    cg, dd, sv = ev(drop_from_hi=dh); row(f"+ rớt {int(100*dh)}% từ đỉnh (green)", cg, dd, sv)
print("REDSTREAK_DONE", flush=True)
