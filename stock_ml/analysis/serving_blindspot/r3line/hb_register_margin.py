# -*- coding: utf-8 -*-
"""(1) REGISTER K10+conv-k2.0+margin0.005 (OOS-validated aggressive-eviction point).
(2) Floor sweep margin down to 0 and NEGATIVE (evict even when new signal slightly WORSE = max churn) +
per-year + walk-forward, to find the true optimum / floor before pushing further. 3-seed [42,21,123]."""
...
SEEDS = [42, 21, 123]; K = 10; KCONV = 2.0; SKIP = 0.40; CS4 = ["dist20low", "dist_ma20", "rsi14", "ret20"]


def prun(sim, pm, cm, margin, advance_fee=0.0008, roundtrip=0.006):
    ...
    for di, dt in enumerate(cal):
        ...
        for t in entries.get(dt, ()):
            ...
            if cash + 1e-12 >= size:
                ...
                if t["prio"] - c["prio"] > margin:                 # margin<0 -> evict even if new slightly worse
...
for sd in SEEDS:
    ...
    cm_c[sd] = cm
    cvf = HERE / f"_rm_s{sd}.csv"; cvtr[["symbol", "entry_date", "exit_date", "entry_price", "exit_price"]].to_csv(cvf, index=False); cv_c[sd] = str(cvf)
    if feat is None:
        feat = M.features(cvtr.symbol.unique().tolist())
    pm[sd] = M.meta_preds(M.build_tr(con, rid, feat), tgt='t_pnl')


def stats(margin, lo="2020-01-01"):
    navs = [prun(NavSim2(cv_c[sd], date_lo=lo), pm[sd], cm_c[sd], margin) for sd in SEEDS]
    fin = [float(d["nav"].iloc[-1]) for d in navs]
    yrs = (navs[0]["date"].iloc[-1] - navs[0]["date"].iloc[0]).days / 365.25
    cg = statistics.mean(f ** (1 / yrs) - 1 for f in fin)
    dd = statistics.mean(float((d["nav"] / d["nav"].cummax() - 1).min()) for d in navs)
    return statistics.mean(fin), cg, dd, navs


# ========== (1) REGISTER K10/convk2.0/margin0.005 ==========
async def make_tmpl(name, desc):
    ...
    async with S2() as s:
        ...
        t = await repo.create(name=name, market=base.market, strategy=base.strategy, feature_set_id=base.feature_set_id,
            ...
            hypothesis="aggressive preempt margin (OOS-validated)", universe_slug=base.universe_slug, model_mode="ml_only")
        await s.commit(); return t.id


nav5, cg5, dd5, _ = stats(0.005); f22 = stats(0.005, "2022-01-01")[1]
name = "x2_struct_to_k10c2m005"
desc = (f"[K10 + preempt R2 margin0.005 (aggressive-evict, OOS-VALIDATED) + cs4-conv k2.0 + conv-skip0.40 "
        f"— TONG VON<=1] Nhu champion combo nhung duoi TICH CUC hon (margin0.005 vs 0.01). Walk-forward audit: "
        f"chon margin theo qua khu -> OOS 99.8% ~ in-sample-best, VUOT fixed-0.01 +2.3pp -> KHONG overfit "
        f"(margin causal: prio=meta walk-forward). Multi-seed CAGR ~{100*cg5:.0f}%/DD{100*dd5:.1f} (3/3). "
        f"Champion margin0.01 hoi bao thu. Fair same-conditions raw CAGR.")
...
ch = hashlib.md5(f"{r2}_m005".encode()).hexdigest()[:16]
cur.execute("""insert into leaderboard_nav (run_id,nav_adv,nav_noadv,cagr_adv,cagr_noadv,maxdd_nav,nav_f22_adv,years,n_trades_sim,config_hash,computed_at)
            ...
            (r2, nav5, nav5, cg5, cg5, dd5, nav5, 6.51, 0, ch))
cur.execute("update leaderboard_runs set state='trained', superseded=false where run_id=%s", (r2,))
con2.commit(); con2.close()
print(f"REGISTERED {name}: CAGR {100*cg5:.1f}% DD {100*dd5:.1f}% NAV x{nav5:.1f}", flush=True)

# ========== (2) FLOOR sweep down to 0 and NEGATIVE + per-year + walk-forward ==========
MARGINS = [0.01, 0.005, 0.003, 0.001, 0.0, -0.005, -0.01, -0.02]
YEARS = list(range(2020, 2027))
yr = {}; summ = {}
print("\n=== FLOOR sweep margin (K10/convk2.0), per-year + full ===", flush=True)
print("  margin  |" + "".join(f" {y} " for y in YEARS) + "| CAGR%  DD%", flush=True)
for m in MARGINS:
    navs = [prun(NavSim2(cv_c[sd], date_lo="2020-01-01"), pm[sd], cm_c[sd], m) for sd in SEEDS]
    yv = {}
    for y in YEARS:
        vals = []
        for d in navs:
            dd = d.set_index("date")["nav"]; g = dd[dd.index.year == y]
            if len(g) > 2:
                vals.append(g.iloc[-1] / g.iloc[0] - 1)
        yv[y] = statistics.mean(vals) if vals else 0.0
    yr[m] = yv
    fin = statistics.mean(float(d["nav"].iloc[-1]) for d in navs)
    yrs = (navs[0]["date"].iloc[-1] - navs[0]["date"].iloc[0]).days / 365.25
    cg = fin ** (1 / yrs) - 1; dd = statistics.mean(float((d["nav"] / d["nav"].cummax() - 1).min()) for d in navs)
    summ[m] = (cg, dd)
    print(f"  {m:<7} |" + "".join(f" {100*yv[y]:+4.0f}" for y in YEARS) + f" | {100*cg:5.1f} {100*dd:5.1f}", flush=True)

# walk-forward over the full floor set
wf, fixed01 = 1.0, 1.0; picks = []
for i, y in enumerate(YEARS):
    pick = 0.01 if i == 0 else max(MARGINS, key=lambda m: sum(yr[m][py] for py in YEARS[:i]))
    picks.append((y, pick)); wf *= (1 + yr[pick][y]); fixed01 *= (1 + yr[0.01][y])
n = len(YEARS)
print(f"\n  WALK-FORWARD picks: {dict(picks)}", flush=True)
print(f"  WF-selected  CAGR {100*(wf**(1/n)-1):5.1f}%  vs  fixed-0.01 {100*(fixed01**(1/n)-1):5.1f}%", flush=True)
best = max(MARGINS, key=lambda m: summ[m][0])
print(f"  in-sample best margin = {best} ({100*summ[best][0]:.1f}%/DD{100*summ[best][1]:.1f})", flush=True)
print("  (neg margin better => 'freshness' beats meta-prio slightly; but check WF picks + fee realism)", flush=True)
print("MARGIN_FLOOR_DONE", flush=True)
