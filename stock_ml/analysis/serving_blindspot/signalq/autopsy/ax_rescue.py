"""AUTOPSY rescue probes: clone candidate 2730 (xq_snr_t08_g27), override ONLY snr-extend knobs.
Never touches templates 2646/2730. Usage: python ax_rescue.py <probe_id|tmpl_id> [seed]
Pattern copied from st_probe.py / sv_probe.py.
"""
...
PROBES = {
    "sx_g35": {"exit_snr_min_gain": 0.35},
    "sx_w40": {"exit_snr_extend_window": 40},
    "sx_w60": {"exit_snr_extend_window": 60},
    "sx_g35_w40": {"exit_snr_min_gain": 0.35, "exit_snr_extend_window": 40},
}
...
async def make_clone(name: str, eng_updates: dict) -> int:
    ...
    async with Session() as s:
        ...
        t = await repo.create(
            ...
            description=f"SNR-EXTEND AUTOPSY rescue over candidate 2730: eng={eng_updates}",
            hypothesis="Autopsy seed-42: 51 deferred trades, 41% worse (-1.48u), all gain funded by "
                       "2 mega-runners (+2.49u); knock-on -1.59u. Test whether min_gain/window shifts "
                       "trim the bad defers (near-peak deferrals) without cutting the mega tail.",
...
def main():
    ...
    if row and row[0] is not None:
        ...
        print(f"SX_RESULT {pid} seed={seed} tmpl={tid} comp={row[0]} dChamp={dch} dCand={dcd} "
              f"pnl={row[1]:.2f} pf={row[2]:.3f} mdd={row[3]:.5f} tr={row[4]} "
              f"run_id={r.get('run_id')}", flush=True)
    else:
        print(f"SX_RESULT {pid} seed={seed} tmpl={tid} NO_ROW run_id={r.get('run_id')}", flush=True)
