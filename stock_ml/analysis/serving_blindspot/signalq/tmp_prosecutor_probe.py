"""PROSECUTOR plateau probe: clone champion 2646 + neighbor lever combos, run seeds.
Pattern copied from st_probe.py. Never touches templates 2646/2760/2761.
Usage: python tmp_prosecutor_probe.py <probe_id|tmpl_id> [seed]
"""
...
PROBES = {
    "pr_dsb50_snr08": {"downleg_skip_bull": {"ma_win": 50, "persist": 3}, **SNR08},
}
...
async def make_clone(name: str, eng_updates: dict) -> int:
    ...
    async with Session() as s:
        ...
        t = await repo.create(
            ...
            description=f"PROSECUTOR plateau neighbor over champion 2646: eng={eng_updates}",
            hypothesis="plateau check for st_dsb60_snr08 promotion audit: neighbor config to detect winner's curse spike.",
...
def main():
    ...
    if row and row[0] is not None:
        ...
        print(f"PR_RESULT {pid} seed={seed} tmpl={tid} comp={row[0]} d={d} "
              f"pnl={row[1]:.2f} pf={row[2]:.3f} mdd={row[3]:.5f} tr={row[4]}", flush=True)
    else:
        print(f"PR_RESULT {pid} seed={seed} tmpl={tid} NO_ROW run_id={r.get('run_id')}", flush=True)
