...
def main():
    ...
    losers_pnl = (
        merged[(merged["n_v34"] > 0) & (merged["n_rule"] > 0)].sort_values("pnl_diff").head(30)
    )
    ...
    print(
        f"  V34 wins: {(both['comp_diff'] > 0).sum()}  ({100 * (both['comp_diff'] > 0).mean():.1f}%)"
    )
    print(
        f"  Rule wins: {(both['comp_diff'] < 0).sum()}  ({100 * (both['comp_diff'] < 0).mean():.1f}%)"
    )
