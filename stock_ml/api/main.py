...
# Fix module path: add stock_ml root so 'src' imports work
_stock_ml_root = Path(__file__).resolve().parent.parent
if str(_stock_ml_root) not in sys.path:
    sys.path.insert(0, str(_stock_ml_root))  # noqa: E402
...
from .middleware.error_handler import error_exception_handler  # noqa: E402
from .middleware.logging_middleware import LoggingMiddleware  # noqa: E402
from .middleware.rate_limiter import limiter, rate_limit_error_handler  # noqa: E402
...
app.include_router(routes.models.router, tags=["models"])
app.include_router(routes.leaderboard.router, tags=["leaderboard"])
app.include_router(routes.runs.router, tags=["runs"])
app.include_router(routes.ohlcv.router, tags=["ohlcv"])
app.include_router(routes.jobs.router, tags=["jobs"])
app.include_router(routes.experiments.router, tags=["experiments"])
app.include_router(routes.universes.router, tags=["universes"])
app.include_router(routes.model_components.router, tags=["model-library"])
app.include_router(routes.templates.router, tags=["templates"])
app.include_router(routes.cache.router, tags=["cache"])
app.include_router(routes.features.router, tags=["features"])
