"""
Export V11 ML vs V10 ML vs Rule-Based comparison data for visualization.
...
def make_markers(trades, method="v11"):
    ...
    for t in trades:
        ...
        if exit_date:
            ...
            if reason == "stop_loss":
                ...
                reason_tag = " HS"
            elif reason == "zombie_exit":
                reason_tag = " ZE"
            elif reason == "hybrid_exit":
                reason_tag = " HX"
...
def main():
    ...
    for sym in symbols:
        ...
        sym_json = {
            ...
            "markers": v11_markers + rule_markers,
            "v11_trades": [
                {
                    k: v
                    for k, v in t.items()
                    if k in ("entry_date", "exit_date", "pnl_pct", "holding_days", "exit_reason", "entry_trend", "quick_reentry", "breakout_entry")
                }
                for t in v11_trades
            ],
            "v10_trades": [
                {k: v for k, v in t.items() if k in ("entry_date", "exit_date", "pnl_pct", "holding_days", "exit_reason", "entry_trend", "quick_reentry")}
                for t in v10_trades
            ],
            "rule_trades": [
                {k: v for k, v in t.items() if k in ("entry_date", "exit_date", "pnl_pct", "holding_days", "exit_reason")}
    ...
    index_entries.sort(key=lambda x: x["v11_pnl"], reverse=True)

    index_json = {
        "symbols": index_entries,
        "version": "V11_vs_V10_vs_Rule",
