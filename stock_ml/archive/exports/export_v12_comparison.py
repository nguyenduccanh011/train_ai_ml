"""
Export V12 ML vs V11 ML vs V10 ML vs Rule-Based comparison data for visualization.
...
def make_markers(trades, method="v12"):
    """Create chart markers from trade list."""
    markers = []
    colors = {
        "v12": ("#00BCD4", "#4caf50", "#f44336", "arrowUp", "arrowDown", "V12"),
        "v11": ("#9C27B0", "#4caf50", "#f44336", "arrowUp", "arrowDown", "V11"),
        "v10": ("#2196F3", "#4caf50", "#f44336", "arrowUp", "arrowDown", "V10"),
...
def main():
    ...
    for fold_idx, (_, train_df, test_df) in enumerate(splitter.split(df)):
        ...
        for sym in test_df["symbol"].unique():
            ...
            for t in r10["trades"]:
                t["symbol"] = sym
            v12_by_sym[sym].extend(r12["trades"])
    ...
    for sym in symbols:
        ...
        trade_keys = ("entry_date", "exit_date", "pnl_pct", "holding_days", "exit_reason", "entry_trend", "quick_reentry", "breakout_entry")

        sym_json = {
            "symbol": sym,
            "ohlcv": ohlcv,
            "v12_markers": v12_markers,
            "v11_markers": v11_markers,
            "v10_markers": v10_markers,
            "rule_markers": rule_markers,
            "markers": v12_markers + v11_markers + rule_markers,
            "v12_trades": [{k: v for k, v in t.items() if k in trade_keys} for t in v12_trades],
            "v11_trades": [{k: v for k, v in t.items() if k in trade_keys} for t in v11_trades],
            "v10_trades": [{k: v for k, v in t.items() if k in ("entry_date", "exit_date", "pnl_pct", "holding_days", "exit_reason", "entry_trend", "quick_reentry")} for t in v10_trades],
        ...
        index_entries.append({
            "symbol": sym,
            "v12_trades": v12_stats["total_trades"],
            "v12_wr": v12_stats["win_rate"],
            "v12_pnl": v12_stats["total_pnl_pct"],
    ...
    index_entries.sort(key=lambda x: x["v12_pnl"], reverse=True)

    index_json = {
        "symbols": index_entries,
        "version": "V12_vs_V11_vs_V10_vs_Rule",
