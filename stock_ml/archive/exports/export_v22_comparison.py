"""
Export V22 Final + V19.1 + V19.3 + Rule trades for web visualization.
Generates JSON files per symbol with OHLCV, markers, trades, stats.
V22 = V19.3 + smart fast_exit + adaptive hard_cap (std_thresh=-6%).

Training: ALL viable symbols (>= 2000 rows).
...
def main():
    ...
    viz_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "visualization_v22", "data")
    ...
    v22_by, v19_1_by, v19_3_by = (defaultdict(list) for _ in range(3))
    ...
    for window, train_df, test_df in splitter.split(df):
        ...
        for sym in test_syms:
            ...
            r22 = backtest_v22_wrapper(y_pred, rets, sym_test, feature_cols,
                                       mod_a=True, mod_b=True, mod_c=False, mod_d=False,
                                       mod_e=True, mod_f=True, mod_g=True, mod_h=True,
                                       mod_i=True, mod_j=True)
            r191 = backtest_v19_1(y_pred, rets, sym_test, feature_cols,
                                  mod_a=True, mod_b=True, mod_c=False, mod_d=False,
                                  mod_e=True, mod_f=True, mod_g=True, mod_h=True,
                                  mod_i=True, mod_j=True)
            r193 = backtest_v19_3(y_pred, rets, sym_test, feature_cols,
                                  mod_a=True, mod_b=True, mod_c=False, mod_d=False,
                                  mod_e=True, mod_f=True, mod_g=True, mod_h=True,
                                  mod_i=True, mod_j=True)

            for r, by in [(r22, v22_by), (r191, v19_1_by), (r193, v19_3_by)]:
                for t in r["trades"]:
                    t["symbol"] = sym
    ...
    for sym in all_symbols:
        if sym not in v22_by and sym not in rule_by:
            continue
        ...
        v22t = enrich_trades_with_prices(v22_by.get(sym, []), close_by_date)
        ...
        s22 = compute_stats(v22t, "V22 Final")
        ...
        index_entries.append({
            "symbol": sym,
            "v22_trades": s22["total_trades"], "v22_pnl": s22["total_pnl_pct"], "v22_wr": s22["win_rate"],
    ...
    index_entries.sort(key=lambda x: x["v22_pnl"], reverse=True)
    with open(os.path.join(viz_dir, "index.json"), "w", encoding="utf-8") as f:
        json.dump({
            "symbols": index_entries,
            "version": "V22_Final_vs_V19.1_vs_V19.3_vs_Rule",
    ...
    total_v22_pnl = sum(e["v22_pnl"] for e in index_entries)
    total_v191_pnl = sum(e["v19_1_pnl"] for e in index_entries)
    total_v193_pnl = sum(e["v19_3_pnl"] for e in index_entries)
    total_rule_pnl = sum(e["rule_pnl"] for e in index_entries)
    positive = sum(1 for e in index_entries if e["v22_pnl"] > 0)
    print(f"\nAggregate PnL across {len(index_entries)} symbols:")
    print(f"  V22 Final: {total_v22_pnl:+.1f}% | {positive}/{len(index_entries)} profitable")
    print(f"  V19.1:     {total_v191_pnl:+.1f}%")
    print(f"  V19.3:     {total_v193_pnl:+.1f}%")
    print(f"  Rule:      {total_rule_pnl:+.1f}%")
    print(f"\nOpen visualization_v22/index.html (via http server)")
