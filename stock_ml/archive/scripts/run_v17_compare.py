...
if __name__ == "__main__":
    ...
    print("=" * 132)
    print(f"V17 COMPARISON (V11/V14/V15/V16/V17 + Rule) - Symbols: {args.pick}")
    print("=" * 132)
    ...
    # a, b, c, d, e, f, g, h, i, j
    configs = [
        ("V11 baseline               ", False, False, False, False, False, False, False, False, False, False),
        ("V14 (A+B)                  ", True,  True,  False, False, False, False, False, False, False, False),
        ("V15 best (A+B+E)           ", True,  True,  False, False, True,  False, False, False, False, False),
        ("V16 best (A+B+E+F+G)       ", True,  True,  False, False, True,  True,  True,  False, False, False),
        ("V17 full (A+B+E+F+G+H+I+J) ", True,  True,  False, False, True,  True,  True,  True,  True,  True),
    ]
    ...
    results = {}
    for name, ma, mb, mc, md, me, mf, mg, mh, mi, mj in configs:
        trades = run_test(args.pick, ma, mb, mc, md, me, mf, mg, mh, mi, mj)
    ...
    print("\n" + "=" * 132)
    print("PER-SYMBOL: V16-best vs V17-full vs Rule")
    print("=" * 132)

    t16 = results["V16 best (A+B+E+F+G)       "][1]
    t17 = results["V17 full (A+B+E+F+G+H+I+J) "][1]
    trule = results["Rule-based                 "][1]
    b16, b17, br = defaultdict(list), defaultdict(list), defaultdict(list)
    for t in t16: b16[t["symbol"]].append(t)
    for t in t17: b17[t["symbol"]].append(t)
    for t in trule: br[t["symbol"]].append(t)

    print(f"{'Sym':<6}| {'V16 Tot':>9} {'V16 PF':>6} | {'V17 Tot':>9} {'V17 PF':>6} | "
          f"{'Rule Tot':>9} {'Rule PF':>7} | {'V17-V16':>8} {'V17-Rule':>9}")
    print("-" * 132)
    T16 = T17 = TR = 0
    for sym in sorted(set(b16.keys()) | set(b17.keys()) | set(br.keys())):
        m16 = calc_metrics(b16.get(sym, []))
        m17 = calc_metrics(b17.get(sym, []))
        mr = calc_metrics(br.get(sym, []))
        T16 += m16["total_pnl"]; T17 += m17["total_pnl"]; TR += mr["total_pnl"]
        print(f"{sym:<6}| {m16['total_pnl']:>+8.1f}% {m16['pf']:>5.2f} | "
              f"{m17['total_pnl']:>+8.1f}% {m17['pf']:>5.2f} | "
              f"{mr['total_pnl']:>+8.1f}% {mr['pf']:>6.2f} | "
              f"{m17['total_pnl']-m16['total_pnl']:>+7.1f}% {m17['total_pnl']-mr['total_pnl']:>+8.1f}%")
    print("-" * 132)
    print(f"{'TOT':<6}| {T16:>+8.1f}% {'':>6} | {T17:>+8.1f}% {'':>6} | "
          f"{TR:>+8.1f}% {'':>7} | {T17-T16:>+7.1f}% {T17-TR:>+8.1f}%")
