...
def check_data():
    ...
    if os.path.isdir(DATA_DIR):
        ...
        if os.path.isdir(all_sym_dir):
            n_symbols = len([d for d in os.listdir(all_sym_dir) if d.startswith("symbol=")])
        ...
        print("  1. On local machine, compress your data:")
        print("     cd portable_data")
        print("     tar -czf vn_stock_ai_dataset_cleaned.tar.gz vn_stock_ai_dataset_cleaned/")
        print("  2. Upload vn_stock_ai_dataset_cleaned.tar.gz to Google Drive:")
        print("     MyDrive/stock_ml_hub/portable_data/")
        print("  3. Then run this cell to extract:")
        print(
            f"     !cd {os.path.join(DRIVE_BASE, 'portable_data')} && "
...
def clone_or_pull_repo():
    ...
    if os.path.isdir(os.path.join(REPO_DIR, ".git")):
        ...
        subprocess.run(["git", "clone", REPO_URL, REPO_DIR], capture_output=True, text=True)
...
def print_env_info():
    ...
    print("  !python run_pipeline.py --all          # All active models")
    print("  !python run_pipeline.py --version v27  # Single model")
    print("=" * 60)
