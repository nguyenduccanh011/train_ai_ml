...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["parent_run_id"], ["leaderboard_runs.run_id"], ondelete="SET NULL", name="fk_runs_parent"
        ),
    )

    # Indexes for leaderboard_runs
    op.create_index("idx_runs_run_id", "leaderboard_runs", ["run_id"], unique=True)
    op.create_index("idx_runs_score_state", "leaderboard_runs", ["composite_score", "state", "superseded"])
    ...
    op.create_index("idx_runs_bundle_name", "leaderboard_runs", ["bundle", "run_name", "generated_at"])
    op.create_index(
