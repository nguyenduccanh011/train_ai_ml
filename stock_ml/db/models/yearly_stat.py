"""Yearly statistics model — per-year breakdown of run performance."""
...
class RunYearlyStatModel(Base):
    __tablename__ = "run_yearly_stats"

    __table_args__ = (UniqueConstraint("run_id", "year", name="uq_run_yearly"),)
    ...
    max_drawdown: Mapped[float | None] = mapped_column(Double(), nullable=True)
    ...
    run: Mapped[LeaderboardRunModel] = relationship(
        "LeaderboardRunModel", back_populates="yearly_stats_list"
    )
