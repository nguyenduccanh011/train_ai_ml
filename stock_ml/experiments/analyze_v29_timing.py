...
DATA_DIR = os.path.normpath(
    os.path.join(ROOT, "..", "portable_data", "vn_stock_ai_dataset_cleaned", "all_symbols")
)
...
def analyze(trade_csv, label):
    ...
    print(
        f"  Profit factor: {df.loc[df['pnl_pct'] > 0, 'pnl_pct'].sum() / -df.loc[df['pnl_pct'] < 0, 'pnl_pct'].sum():.2f}"
    )
        ...
        rows.append(
            {
                ...
                "sell_5d_earlier_pct": (x_minus5 - exit_close)
                / exit_close
                * 100,  # >0 means earlier price higher
                "sell_10d_earlier_pct": (x_minus10 - exit_close) / exit_close * 100,
                "sell_5d_later_pct": (x_plus5 - exit_close)
                / exit_close
                * 100,  # >0 means later price higher
    ...
    print(
        f"    Mean rally_5d  : {losers['rally_5d'].mean():+.2f}%   (winners: {winners['rally_5d'].mean():+.2f}%)"
    )
    print(
        f"    Mean rally_10d : {losers['rally_10d'].mean():+.2f}%  (winners: {winners['rally_10d'].mean():+.2f}%)"
    )
    print(
        f"    Mean rally_20d : {losers['rally_20d'].mean():+.2f}%  (winners: {winners['rally_20d'].mean():+.2f}%)"
    )
    print(
        f"    Mean dist_from_20d_high: {losers['dist_from_high20'].mean():+.2f}%  (winners: {winners['dist_from_high20'].mean():+.2f}%)"
    )
    ...
    print(
        f"    Loser BUYS AFTER STRONG RALLY (rally_10d>12%): {len(rallied_strong)} ({len(rallied_strong) / max(len(losers), 1) * 100:.1f}%), "
    ...
    print(
        f"    Losers where 10d-earlier price was ≥5% LOWER : {len(losers_better_10d)}/{len(losers)} ({len(losers_better_10d) / max(len(losers), 1) * 100:.1f}%) "
        f"avg cheaper by {losers_better_10d['buy_10d_earlier_pct'].mean():.2f}%"
    )
    print(
        f"    Losers where 20d-earlier price was ≥8% LOWER : {len(losers_better_20d)}/{len(losers)} ({len(losers_better_20d) / max(len(losers), 1) * 100:.1f}%) "
    ...
    print(
        f"    Trades where price went +5% within 20d AFTER exit (sold too early): "
        f"{len(sold_too_early)}/{len(a)} ({len(sold_too_early) / len(a) * 100:.1f}%), avg lost upside {sold_too_early['fwd20_max_pct'].mean():.2f}%"
    )
    print(
        f"    Trades where price went +10% within 20d AFTER exit             : "
    ...
    print(
        f"    Mean sell_5d_earlier: {a['sell_5d_earlier_pct'].mean():+.2f}%  (positive = SHOULD have sold earlier)"
    )
    ...
    worst = losers.nsmallest(15, "pnl_pct")[
        [
            "symbol",
            "entry_date",
            "pnl_pct",
    ...
    early = a.nlargest(15, "fwd20_max_pct")[
        [
            "symbol",
            "entry_date",
            "exit_date",
...
if __name__ == "__main__":
    ...
    for col in [
        ...
        "sell_20d_later_pct",
        "sell_5d_earlier_pct",
        "sell_10d_earlier_pct",
    ]:
        print(
            f"    {col:<26} V29={a29[col].mean():+.2f}%  V28={a28[col].mean():+.2f}%   Δ={a29[col].mean() - a28[col].mean():+.2f}"
        )
