...
Usage:
  ...
  python run_feature_ablation.py --symbols ACB,FPT,HPG
"""
...
from src.config_loader import get_pipeline_symbols, get_training_device, load_config
...
def resolve_symbols(symbols_arg="", min_rows=2000):
    return get_pipeline_symbols(symbols_arg=symbols_arg, min_rows_override=min_rows)


def run_experiment(symbols, extra_groups=None, device="cpu", model_type="lightgbm"):
    ...
    if str(config["target"].get("type", "trend_regime")).lower() == "return_regression":
        raise ValueError("Feature ablation currently supports classification targets only.")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Comma-separated symbols. Empty=resolve from pipeline config",
    )
    ...
    print("\nResolving symbols...")
    symbols = resolve_symbols(symbols_arg=args.symbols, min_rows=args.min_rows)
    ...
    for extra_groups, label in experiments:
        ...
        trades, importance, n_features = run_experiment(
            symbols, extra_groups=extra_groups, device=device, model_type=model_type
        )
        dt = time.time() - t0
        ...
        print(
            f"    Trades={metrics['trades']}, WR={metrics['wr']:.1f}%, "
        ...
        if importance:
            ...
            for i, (fname, imp) in enumerate(list(importance.items())[:15]):
                marker = (
                    " *NEW*"
                    if extra_groups
                    and any(fname.startswith(p) for p in _get_group_prefixes(extra_groups))
                    else ""
                )
    ...
    header = (
        f"  {'Experiment':<40s} | {'#Feat':>5} {'#Trd':>5} {'WR':>6} "
    ...
    for key, r in results.items():
        ...
        print(
            f"  {r['label']:<40s} | {r['n_features']:>5} {r['trades']:>5} "
    ...
    print(f"\n{'─' * 80}")
    if improved_groups:
        print("  GROUPS THAT IMPROVED (score + TotalPnL > baseline):")
        ...
        if len(best_combo) > 1 and not args.group:
            ...
            t0 = time.time()
            trades, importance, n_features = run_experiment(
                symbols, extra_groups=best_combo, device=device, model_type=model_type
            )
            dt = time.time() - t0
            metrics = calc_metrics(trades)
            score = comp_score(metrics)
            ...
            print(
                f"    Composite Score: {score:.0f} (Baseline: {bl_score:.0f}, "
            ...
            if importance:
                ...
                for i, (fname, imp) in enumerate(list(importance.items())[:20]):
                    marker = (
                        " *NEW*"
                        if any(fname.startswith(p) for p in _get_group_prefixes(best_combo))
                        else ""
                    )
                    print(f"      {i + 1:>2}. {fname:<35s} {imp:>8.0f}{marker}")
    else:
        print("  NO GROUP IMPROVED over baseline. Current features are already well-tuned.")
    ...
    for exp_key, imp_dict in all_importances.items():
        for fname, imp_val in imp_dict.items():
            imp_rows.append(
                {"experiment": exp_key, "feature": fname, "importance": round(imp_val, 1)}
            )
    if imp_rows:
