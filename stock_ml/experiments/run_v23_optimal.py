...
if __name__ == "__main__":
    ...
    from src.strategies.legacy import backtest_v19_1, backtest_v19_3
    ...
    t_v22 = run_test_base(
        SYMBOLS, True, True, False, False, True, True, True, True, True, True, backtest_fn=v22_fn
    )
    ...
    configs = [
        ...
        (
            "V23-fe_s10+pp12+hcw8",
            {
                "fast_exit_strong": -0.10,
        ...
        (
            "V23-fe_s9+fem5+pp12",
            {
                "fast_exit_strong": -0.09,
                "fast_exit_moderate": -0.05,
                "peak_protect_strong_threshold": 0.12,
            },
        ),
        (
    ...
    for lbl, m in [("V19.1", m_v191), ("V19.3", m_v193), ("V22-Final", m_v22), ("Rule", m_rule)]:
        print(
            f"  {lbl:<30} | {m['trades']:>4} {m['wr']:>5.1f}% {m['avg_pnl']:>+7.2f}% "
    ...
    for label, params in configs:
        ...
        print(
            f"\r  {label:<30} | {m['trades']:>4} {m['wr']:>5.1f}% {m['avg_pnl']:>+7.2f}% "
            f"{m['total_pnl']:>+9.1f}% {m['pf']:>5.2f} {m['max_loss']:>+7.1f}% {m['avg_hold']:>6.1f}d | "
    ...
    print(
        f"  {'ref':>4} {'V19.3':<30} {m_v193['total_pnl']:>+9.1f}% {m_v193['pf']:>5.2f} {m_v193['wr']:>5.1f}% "
        f"{m_v193['max_loss']:>+7.1f}% {s_v193:>7.1f}"
    )
    print(
        f"  {'ref':>4} {'V22-Final':<30} {m_v22['total_pnl']:>+9.1f}% {m_v22['pf']:>5.2f} {m_v22['wr']:>5.1f}% "
    ...
    for rank, (score, name, m, _) in enumerate(scored[:10], 1):
        ...
        print(
            f"  {rank:>4} {name:<30} {m['total_pnl']:>+9.1f}% {m['pf']:>5.2f} {m['wr']:>5.1f}% "
    ...
    for sym in syms:
        ...
        t_all["v191"] += m1["total_pnl"]
        t_all["v193"] += m3["total_pnl"]
        t_all["v22"] += m22["total_pnl"]
        t_all["v23"] += mb["total_pnl"]
        t_all["rule"] += mr["total_pnl"]
        print(
            f"  {sym:<6}| {m1['total_pnl']:>+8.1f}% | {m3['total_pnl']:>+8.1f}% | "
    ...
    print("  " + "-" * 120)
    print(
        f"  {'TOTAL':<6}| {t_all['v191']:>+8.1f}% | {t_all['v193']:>+8.1f}% | "
    ...
    for reason, grp in df_b.groupby("exit_reason"):
        ...
        print(
            f"    {reason:<25}: {len(grp):>4} ({wins}W), WR={wins / len(grp) * 100:>5.1f}%, "
    ...
    for reason in sorted(all_reasons):
        ...
        print(
            f"    {reason:<25}: {tots[0]:>+9.1f}% {tots[1]:>+9.1f}% {tots[2]:>+9.1f}% {tots[3]:>+9.1f}%"
        )

    # By year
    print("\n  BY YEAR:")
    df_bc = df_b.copy()
    df_bc["year"] = df_bc["entry_date"].astype(str).str[:4]
    df_191c = df_191.copy()
    df_191c["year"] = df_191c["entry_date"].astype(str).str[:4]
    df_193c = df_193.copy()
    df_193c["year"] = df_193c["entry_date"].astype(str).str[:4]
    df_22c = df_22.copy()
    df_22c["year"] = df_22c["entry_date"].astype(str).str[:4]
    for yr in sorted(df_bc["year"].unique()):
        ...
        print(
            f"    {yr}: V19.1={m191y['total_pnl']:>+8.1f}% V19.3={m193y['total_pnl']:>+8.1f}% "
    ...
    for trend in ["strong", "moderate", "weak"]:
        for lbl, df in [("V19.1", df_191), ("V19.3", df_193), ("V22", df_22), ("V23", df_b)]:
            ...
            print(
                f"    {lbl:<6} {trend:<10}: #{m['trades']:>3} WR={m['wr']:>5.1f}% avg={m['avg_pnl']:>+6.2f}% "
    ...
    for lbl, df in [("V19.3", df_193), ("V22", df_22), ("V23", df_b)]:
        ...
        print(
            f"    {lbl}: {len(fel)} trades, total={fel['pnl_pct'].sum():>+8.1f}%, avg={fel['pnl_pct'].mean():>+6.2f}%"
            if len(fel) > 0
            else f"    {lbl}: 0 fast_exit_loss trades"
        )
    ...
    for lbl, df in [("V19.1", df_191), ("V19.3", df_193), ("V22", df_22), ("V23", df_b)]:
        ...
        print(
            f"    {lbl}: {len(pp)} trades, total={pp['pnl_pct'].sum():>+8.1f}%, avg={pp['pnl_pct'].mean():>+6.2f}%"
            if len(pp) > 0
            else f"    {lbl}: 0 peak_protect trades"
        )
