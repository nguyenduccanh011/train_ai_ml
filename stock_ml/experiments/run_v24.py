...
if __name__ == "__main__":
    ...
    print(f"\n{'=' * 130}")
    print(
        f"  {'Model':<20} | {'#':>4} {'WR':>6} {'AvgPnL':>8} {'TotPnL':>10} {'PF':>6} {'MaxLoss':>8} {'AvgHold':>8}"
    )
    print("  " + "-" * 80)
    for lbl, m in [
        ("V19.1", m_v191),
        ...
        print(
            f"  {lbl:<20} | {m['trades']:>4} {m['wr']:>5.1f}% {m['avg_pnl']:>+7.2f}% "
    ...
    for lbl, m in [
        ...
        print(
            f"  {lbl:<20} | {m['total_pnl'] - m_v191['total_pnl']:>+9.1f}% "
    ...
    print(
        f"  {'Sym':<6}| {'V19.1':>9} | {'V22':>9} | {'V23':>9} | {'Rule':>9} | {'V24':>9} | {'V24-V23':>9} | {'V24-Rule':>9}"
    )
    print("  " + "-" * 100)
    for sym in sorted(df_v24["symbol"].unique() if "symbol" in df_v24.columns else []):
        ...
        def sp(df, s):
            return (
                calc_metrics(df[df["symbol"] == s].to_dict("records"))
        ...
        m4 = sp(df_v24, sym)
        print(
            f"  {sym:<6}| {m1['total_pnl']:>+8.1f}% | {m2['total_pnl']:>+8.1f}% | "
    ...
    print(
        "    Target: TotalPnL >= +2000%, MaxLoss <= -18%, signal_hard_cap improved, peak_protect increased"
    )
    print(
        f"    Actual: TotalPnL = {m_v24['total_pnl']:>+.1f}%, MaxLoss = {m_v24['max_loss']:>+.1f}%"
    )
