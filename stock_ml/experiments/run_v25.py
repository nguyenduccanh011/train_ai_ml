...
if __name__ == "__main__":
    ...
    def v23_best_fn(y_pred, returns, df_test, feature_cols, **kwargs):
        return backtest_v23(
            y_pred, returns, df_test, feature_cols, peak_protect_strong_threshold=0.12, **kwargs
        )
    ...
    ablation_configs = [
        ...
        (
            "N2: adaptive_hc",
            {**ALL_OFF, "patch_adaptive_hardcap": True, "patch_smart_hardcap": True},
        ),
    ...
    for lbl, m in [
        ("V19.1 (baseline)", m_v191),
        ("V22 (baseline)", m_v22),
        ("V23-best (baseline)", m_v23),
        ("Rule (baseline)", m_rule),
    ]:
        print(
    ...
    for rank, (sc, name, m, cfg) in enumerate(scored, 1):
        ...
        print(
            f"  #{rank:>2} {sc:>7.0f}  {name:<30} TotPnL={m['total_pnl']:>+9.1f}% "
    ...
    print(
        f"    TotalPnL:  {best_m['total_pnl']:+.1f}%  (vs V23: {best_m['total_pnl'] - m_v23['total_pnl']:+.1f}%,"
    ...
    print(f"\n{'=' * 140}")
    print("  PER-SYMBOL: V25-best vs baselines")
    print(f"{'=' * 140}")
    ...
    print(
        f"  {'Sym':<6}| {'V19.1':>9} | {'V23':>9} | {'Rule':>9} | {'V25':>9} | {'V25-V23':>9} | {'V25-Rule':>9}"
    )
    print("  " + "-" * 80)
