...
if __name__ == "__main__":
    ...
    print(
        f"    V25: {m_v25['trades']} trades, TotalPnL={m_v25['total_pnl']:+.1f}%, "
    ...
    print(f"\n{'=' * 120}")
    print(
        f"  {'Config':<20} | {'#':>5} {'WR':>6} {'AvgPnL':>8} {'TotPnL':>10} {'PF':>6} "
        f"{'MaxLoss':>8} {'AvgH':>6} | {'Comp':>7}"
    )
    print("  " + "-" * 100)
    print(
        f"  {'V25 Baseline':<20} | {m_v25['trades']:>5} {m_v25['wr']:>5.1f}% {m_v25['avg_pnl']:>+7.2f}% "
        f"{m_v25['total_pnl']:>+9.1f}% {m_v25['pf']:>5.2f} {m_v25['max_loss']:>+7.1f}% "
        f"{m_v25['avg_hold']:>5.1f}d | {cs25:>6.0f}"
    )
    print(
        f"  {'V26 B+C+E':<20} | {m_v26['trades']:>5} {m_v26['wr']:>5.1f}% {m_v26['avg_pnl']:>+7.2f}% "
    ...
    print(
        f"  Delta vs V25:  TotPnL={delta_pnl:+.1f}%, WR={m_v26['wr'] - m_v25['wr']:+.2f}%, "
