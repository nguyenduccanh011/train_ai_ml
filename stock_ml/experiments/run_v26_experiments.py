...
if __name__ == "__main__":
    ...
    print(
        f"    V25 Baseline: {m_v25['trades']} trades, TotalPnL={m_v25['total_pnl']:+.1f}%, "
    ...
    experiments = [
        ...
        (
            ...
            {
                **V26_OFF,
                "v26_wider_hardcap": True,
                "v26_skip_choppy": True,
                "v26_extended_hold": True,
            },
        ),
        (
            "A+C+H",
            {
                **V26_OFF,
                "v26_wider_hardcap": True,
                "v26_skip_choppy": True,
                "v26_hardcap_confirm_strong": True,
            },
        ),
        (
            "A+D+H",
    ...
    print(
        f"  {'V25-BASELINE':<30} | {m_v25['trades']:>5} {m_v25['wr']:>5.1f}% {m_v25['avg_pnl']:>+7.2f}% "
    ...
    for label, v26_cfg in experiments:
        ...
        print(
            f"\r  {label:<30} | {m['trades']:>5} {m['wr']:>5.1f}% {m['avg_pnl']:>+7.2f}% "
    ...
    print(f"\n  BEST: {best_name}")
    print(
        f"    Composite: {best_score:.0f} (V25 baseline: {cs25:.0f}, delta: {best_score - cs25:+.0f})"
    )
    print(
        f"    TotalPnL:  {best_m['total_pnl']:+.1f}% (vs V25: {best_m['total_pnl'] - m_v25['total_pnl']:+.1f}%)"
    )
    ...
    for name in [
        ...
        "C: Skip choppy",
        "D: Extended hold",
        "E: Strong rule ensemble",
        "F: Min position (0.28)",
        "G: Score-5 penalty",
        "H: HC confirm strong+1",
    ]:
        if name in results:
            m, _, _ = results[name]
            ...
            print(
                f"  {name:<30} dTotPnL={dt:>+8.1f}%  dWR={dwr:>+5.2f}%  dPF={dpf:>+5.3f}  => {verdict}"
            )
