...
def fmt(name, m, cs, base_cs=None):
    ...
    return (
        f"  {name:<42} | {m['trades']:>5} {m['wr']:>5.1f}% {m['avg_pnl']:>+7.2f}% "
...
if __name__ == "__main__":
    ...
    experiments = [
        ...
        (
            "A1a: peak_prox -2%/8%",
            dict(
                v30_peak_proximity_filter=True,
        ...
        (
            ...
            dict(
                v30_peak_proximity_filter=True,
                v30_peak_prox_dist_threshold=-0.02,
                v30_peak_prox_rally10_min=0.06,
            ),
        ),
        (
            "A1c: peak_prox -3%/8%",
            dict(
                v30_peak_proximity_filter=True,
                v30_peak_prox_dist_threshold=-0.03,
                v30_peak_prox_rally10_min=0.08,
            ),
        ),
        (
            "A2a: rally_ext 12%/18%",
            dict(
                v30_rally_extension_filter=True,
        ...
        (
            ...
            dict(
                v30_rally_extension_filter=True,
                v30_rally10_hard_block=0.10,
                v30_rally20_hard_block=0.15,
            ),
        ),
        (
            "A2c: rally_ext 15%/22%",
            dict(
                v30_rally_extension_filter=True,
                v30_rally10_hard_block=0.15,
                v30_rally20_hard_block=0.22,
            ),
        ),
        ("A3a: pullback_only 3%", dict(v30_pullback_only_entry=True, v30_pullback_min_pct=0.03)),
        ("A3b: pullback_only 2%", dict(v30_pullback_only_entry=True, v30_pullback_min_pct=0.02)),
        (
            "A4a: rally_scaling 5%/10%",
            dict(
                v30_rally_position_scaling=True, v30_rps_tier1_rally=0.05, v30_rps_tier2_rally=0.10
            ),
        ),
        (
            "A4b: rally_scaling 6%/12%",
            dict(
                v30_rally_position_scaling=True, v30_rps_tier1_rally=0.06, v30_rps_tier2_rally=0.12
            ),
        ),
        # ── EXIT improvements ──
        (
            "B1a: sig_defer 3bars/3%",
            dict(v30_signal_exit_defer=True, v30_sed_defer_bars=3, v30_sed_min_cum_ret=0.03),
        ),
        (
            "B1b: sig_defer 2bars/5%",
            dict(v30_signal_exit_defer=True, v30_sed_defer_bars=2, v30_sed_min_cum_ret=0.05),
        ),
        (
            "B2a: mom_hold 5%/RSI72",
            dict(v30_momentum_hold_override=True, v30_mho_min_profit=0.05, v30_mho_rsi_max=72),
        ),
        (
            "B2b: mom_hold 3%/RSI70",
            dict(v30_momentum_hold_override=True, v30_mho_min_profit=0.03, v30_mho_rsi_max=70),
        ),
        (
            "B2c: mom_hold 7%/RSI75",
            dict(v30_momentum_hold_override=True, v30_mho_min_profit=0.07, v30_mho_rsi_max=75),
        ),
        (
            "B3a: chandelier 3ATR/5%",
            dict(v30_chandelier_trail=True, v30_chand_atr_mult=3.0, v30_chand_profit_trigger=0.05),
        ),
        (
            "B3b: chandelier 2.5ATR/5%",
            dict(v30_chandelier_trail=True, v30_chand_atr_mult=2.5, v30_chand_profit_trigger=0.05),
        ),
        (
            "B3c: chandelier 3ATR/8%",
            dict(v30_chandelier_trail=True, v30_chand_atr_mult=3.0, v30_chand_profit_trigger=0.08),
        ),
        # ── HARD CAP fixes ──
        (
            "C1a: atr_hc 1.5x/8-20%",
            dict(
                v30_atr_aware_hardcap=True,
                v30_atr_hc_mult=1.5,
        ...
        (
            "C2a: hc2step -4%/-8%",
            dict(v30_hardcap_two_step_v2=True, v30_hc2_step1_loss=-0.04, v30_hc2_step2_loss=-0.08),
        ),
        (
            "C2b: hc2step -3%/-7%",
            dict(v30_hardcap_two_step_v2=True, v30_hc2_step1_loss=-0.03, v30_hc2_step2_loss=-0.07),
        ),
        (
            "C2c: hc2step -5%/-10%",
            dict(v30_hardcap_two_step_v2=True, v30_hc2_step1_loss=-0.05, v30_hc2_step2_loss=-0.10),
        ),
        (
            "C3a: regime_hc -5%/-12%",
            dict(
                v30_regime_aware_hardcap=True, v30_rah_choppy_cap=-0.05, v30_rah_trending_cap=-0.12
            ),
        ),
        (
            "C3b: regime_hc -4%/-10%",
            dict(
                v30_regime_aware_hardcap=True, v30_rah_choppy_cap=-0.04, v30_rah_trending_cap=-0.10
            ),
        ),
    ]
    ...
    for n, r in pos_sorted[:8]:
        print(
            f"    {n}: comp={r[3]:.0f} (+{r[3] - cs_v29:.0f})  tot={r[2]['total_pnl']:+.1f}%  pf={r[2]['pf']:.2f}  maxloss={r[2]['max_loss']:+.1f}%"
        )

    # Best per family
    def family(nm):
        return nm.split(":")[0].strip()[:2]
    ...
    if args.phase in ("all", "2", "3", "4"):
        ...
        for cname, (fl, _, m, cs) in combo_pos[:5]:
            print(
                f"    {cname}: comp={cs:.0f} (+{cs - cs_v29:.0f})  tot={m['total_pnl']:+.1f}%  pf={m['pf']:.2f}  maxloss={m['max_loss']:+.1f}%"
            )
    ...
    if args.phase in ("all", "4") and greedy_families:
        ...
        print(sep)
        print(
            fmt("Greedy baseline", calc_metrics(run(SYMBOLS, make_bt(**greedy_flags))), current_cs)
        )
        print(sep)
            ...
            for d, r in [(-0.02, 0.07), (-0.02, 0.09), (-0.025, 0.08), (-0.015, 0.08)]:
                finetune_variants.append(
                    (
                        ...
                        {
                            **greedy_flags,
                            "v30_peak_prox_dist_threshold": d,
                            "v30_peak_prox_rally10_min": r,
                        },
                    )
                )
        if "A2" in greedy_families:
            for r10, r20 in [(0.10, 0.16), (0.11, 0.17), (0.13, 0.20)]:
                finetune_variants.append(
                    (
                        f"A2 tune r10={r10} r20={r20}",
                        {
                            **greedy_flags,
                            "v30_rally10_hard_block": r10,
                            "v30_rally20_hard_block": r20,
                        },
                    )
                )
        ...
        for ftname, ftflags in finetune_variants:
            ...
            if cs > best_ft_cs:
                best_ft_cs = cs
                best_ft_flags = ftflags
                best_ft_trades = t_exp
        print(sep)
        if best_ft_cs > current_cs:
            print(
                f"  Fine-tune improved: {current_cs:.0f} -> {best_ft_cs:.0f} (+{best_ft_cs - current_cs:.0f})"
            )
            greedy_flags = best_ft_flags
            current_cs = best_ft_cs
        else:
