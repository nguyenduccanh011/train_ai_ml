...
if __name__ == "__main__":
    ...
    print(
        f"  {'V30 (signal_exit_defer)':<32} | {m_v30['trades']:>5} {m_v30['wr']:>5.1f}% {m_v30['avg_pnl']:>+7.2f}% {m_v30['total_pnl']:>+9.1f}% {m_v30['pf']:>5.2f} {m_v30['max_loss']:>+7.1f}% {m_v30['avg_hold']:>5.1f}d | {cs_v30:>6.0f}"
    )
    print(
        f"  {'V31 (shef+hap final)':<32} | {m_v31['trades']:>5} {m_v31['wr']:>5.1f}% {m_v31['avg_pnl']:>+7.2f}% {m_v31['total_pnl']:>+9.1f}% {m_v31['pf']:>5.2f} {m_v31['max_loss']:>+7.1f}% {m_v31['avg_hold']:>5.1f}d | {cs_v31:>6.0f}"
    )
    print(SEP)
    print(
        f"  Delta vs V30:  Comp={cs_v31 - cs_v30:+.0f}  WR={m_v31['wr'] - m_v30['wr']:+.2f}pp  "
    ...
    print(
        f"    Signal exits hold<18d: V30={len(sig_v30_short)} (WR={(sig_v30_short['pnl_pct'] > 0).mean() * 100:.1f}%)  V31={len(sig_v31_short)} (WR={(sig_v31_short['pnl_pct'] > 0).mean() * 100:.1f}%)"
    )
    print(
        f"    Signal exits hold>=18d: V30={len(sig_v30[sig_v30['holding_days'] >= 18])} WR={(sig_v30[sig_v30['holding_days'] >= 18]['pnl_pct'] > 0).mean() * 100:.1f}%  V31={len(sig_v31[sig_v31['holding_days'] >= 18])} WR={(sig_v31[sig_v31['holding_days'] >= 18]['pnl_pct'] > 0).mean() * 100:.1f}%"
    )
