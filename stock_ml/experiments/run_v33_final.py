...
if __name__ == "__main__":
    ...
    print(
        f"  {'V32 (hap_pre+weak_oversold)':<38} | {m_v32['trades']:>5} {m_v32['wr']:>5.1f}% {m_v32['avg_pnl']:>+7.2f}% {m_v32['total_pnl']:>+9.1f}% {m_v32['pf']:>5.2f} {m_v32['max_loss']:>+7.1f}% {m_v32['avg_hold']:>5.1f}d | {cs_v32:>6.0f}"
    )
    print(
        f"  {'V33 (rpf_12pct)':<38} | {m_v33['trades']:>5} {m_v33['wr']:>5.1f}% {m_v33['avg_pnl']:>+7.2f}% {m_v33['total_pnl']:>+9.1f}% {m_v33['pf']:>5.2f} {m_v33['max_loss']:>+7.1f}% {m_v33['avg_hold']:>5.1f}d | {cs_v33:>6.0f}"
    )
    print(SEP)
    print(
        f"  Delta vs V32:  Comp={cs_v33 - cs_v32:+.0f}  WR={m_v33['wr'] - m_v32['wr']:+.2f}pp  "
    ...
    for reason, grp in df_v33.groupby("exit_reason"):
        ...
        wr = (pnl > 0).mean() * 100
        print(
            f"    {reason:<30} n={len(grp):4d} WR={wr:5.1f}% avg={pnl.mean():+.2f}% tot={pnl.sum():+.0f}%"
        )
    ...
    print(
        f"    Signal exits: V32={len(sig_v32)} (WR={(sig_v32['pnl_pct'] > 0).mean() * 100:.1f}% avg={sig_v32['pnl_pct'].mean():+.2f}%)  "
    ...
    print(
        f"    Trades difference (entry filter): {m_v32['trades']} → {m_v33['trades']} "
    ...
    print(
        f"    Trades < -20%: {len(deep_loss)} (V32: {len(df_v32_saved[df_v32_saved['pnl_pct'] < -20])})"
    )
