...
def fmt_row(label, m, cs, delta=None):
    ...
    return (
        f"  {label:<60} | {m['trades']:>5} {m['wr']:>5.1f}% "
...
if __name__ == "__main__":
    ...
    for r in f_results[:5]:
        print(
            f"    {r[0]:<60} comp={r[4]:.0f} D={r[5]:+.0f} trades={r[2]['trades']} maxloss={r[2]['max_loss']:+.1f}%"
        )
    ...
    combos_2b = [
        ...
        ("F_hi+D_2d", {**V33_F_HI, **dict(v33_hap_consec_drop=True, v33_hcd_min_days=2)}),
        ("F_hi+E_30", {**V33_F_HI, **dict(v33_rsi_oversold_block=True, v33_rob_rsi_thresh=30.0)}),
    ]
    ...
    for r in (p2b_safe or p2b_results)[:5]:
        print(
            f"    {r[0]:<60} comp={r[4]:.0f} Dvs32={r[5]:+.0f} DvsF={r[6]:+.0f} "
    ...
    print(
        f"  comp={greedy_cs:.0f} D_vs_v32={greedy_cs - cs_v32:+.0f} D_vs_Fhi={greedy_cs - cs_fhi:+.0f}"
    )

    # ── Phase 4: Fine-tune F params on greedy stack ───────────────────────────
    print("\n== PHASE 4: Fine-tune F on greedy stack ==")
    greedy_no_f = {k: v for k, v in greedy_patches.items() if not k.startswith("v33_sce")}
    ...
    overall_best = (
        max(all_safe, key=lambda x: x[4]) if all_safe else max(all_candidates, key=lambda x: x[4])
    )
    ...
    print(
        f"  trades={overall_best[2]['trades']} WR={overall_best[2]['wr']:.1f}% "
