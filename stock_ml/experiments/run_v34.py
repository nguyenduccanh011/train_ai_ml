...
def fmt_row(label, m, cs, delta=None):
    ...
    return (
        f"  {label:<62} | {m['trades']:>5} {m['wr']:>5.1f}% "
...
if __name__ == "__main__":
    ...
    HDR = (
        f"  {'Config':<62} | {'#':>5} {'WR':>6} {'AvgPnL':>8} {'TotPnL':>10} "
    ...
    print("  Engine: V33 (= V32 + recovery_peak_filter)")
    print()
    ...
    if args.phase in (0, 1):
        ...
        for name, cs, d, m, _ in p1:
            print(
                f"    {name:<8} comp={cs:.0f} D={d:+.0f} WR={m['wr']:.1f}% "
        ...
        extra_patches_list = [
            (
                ...
                dict(
                    ...
                    v33_rpf_ret10_thresh=0.08,
                    v33_rpf_dist_sma20_thresh=0.005,
    ...
    print("=" * 170)
    print(
        f"  V33 baseline: comp={cs_v33:.0f} WR={m_v33['wr']:.1f}% avg={m_v33['avg_pnl']:+.2f}% "
    ...
    if all_results:
        ...
        for name, cs, d, m, t in all_results[:5]:
            print(
                f"    {name:<50} comp={cs:.0f} D={d:+.0f} trades={m['trades']} "
