"""
V14 — Built from V11 (NOT V12). Two new modules with toggle for ablation:
======================================================================
  Module A — V-SHAPE ENTRY: Detect confirmed V-bottom reversal,
             bypass `ret_5d > 5%` and falling-knife blockers for 5 bars
             after a bullish reversal candle with volume.
             Goal: catch ACB 2025-04-09 (17.5) instead of 2025-07-22 (22.7).

  Module B — PROFIT-PEAK PROTECTION: Independent exit layer that protects
             realized profit. Triggers:
               - max_profit >= 0.15 AND 2 consecutive close < EMA8  -> exit
               - max_profit >= 0.20 AND 1 close < SMA10 with vol>1.5x avg -> exit
             Does NOT use V12's time-decay trailing (proven harmful).
             Goal: ACB exit at ~27.85 (08-22) instead of 26.55 (09-09).

Ablation:
  V11 baseline (mod_a=False, mod_b=False)
  + A only
  + B only
  + A + B (V14 full)
"""
...
def run_test(symbols_str, mod_a, mod_b):
    ...
    for window, train_df, test_df in splitter.split(df):
        ...
        for sym in test_df["symbol"].unique():
            ...
            r = backtest_v14(y_pred, rets, sym_test, feature_cols, mod_a=mod_a, mod_b=mod_b)
            for t in r["trades"]:
                t["symbol"] = sym
...
if __name__ == "__main__":
    ...
    print("=" * 100)
    print(f"V14 ABLATION STUDY (built from V11) — Symbols: {args.pick}")
    print("=" * 100)
    print("Module A: V-shape entry override (bypass falling-knife & ret_5d for confirmed reversals)")
    print("Module B: Profit-peak protection (EMA8 break + distribution day)")
    print()

    configs = [
        ("V11 baseline (a=0,b=0)", False, False),
        ("V14 + A only         ", True, False),
        ("V14 + B only         ", False, True),
        ("V14 full (A+B)       ", True, True),
    ]

    print(f"{'Config':<26} | {'#':>4} {'WR':>6} {'AvgPnL':>8} {'TotPnL':>10} {'PF':>6} "
          f"{'MaxLoss':>8} {'AvgHold':>8}")
    print("-" * 100)

    results = {}
    for name, ma, mb in configs:
        trades = run_test(args.pick, ma, mb)
        m = calc_metrics(trades)
        results[name] = (m, trades)
        print(f"{name:<26} | {m['trades']:>4} {m['wr']:>5.1f}% {m['avg_pnl']:>+7.2f}% "
              f"{m['total_pnl']:>+9.1f}% {m['pf']:>5.2f} {m['max_loss']:>+7.1f}% {m['avg_hold']:>6.1f}d")

    # Module A entry breakdown
    print("\n" + "=" * 100)
    print("MODULE A — V-shape entry stats (in V14 full)")
    print("=" * 100)
    full_trades = results["V14 full (A+B)       "][1]
    vshape_trades = [t for t in full_trades if t.get("vshape_entry")]
    if vshape_trades:
        m = calc_metrics(vshape_trades)
        print(f"  V-shape entries: {m['trades']}, WR={m['wr']:.1f}%, "
              f"Avg={m['avg_pnl']:+.2f}%, Total={m['total_pnl']:+.1f}%, PF={m['pf']:.2f}")
    else:
        print("  No V-shape entries triggered.")

    # Module B exit breakdown
    print("\nMODULE B — Peak-protect exit stats (in V14 full)")
    pp_trades = [t for t in full_trades if t.get("exit_reason", "").startswith("peak_protect")]
    if pp_trades:
        m = calc_metrics(pp_trades)
        print(f"  Peak-protect exits: {m['trades']}, WR={m['wr']:.1f}%, "
              f"Avg={m['avg_pnl']:+.2f}%, Total={m['total_pnl']:+.1f}%")
    else:
        print("  No peak-protect exits triggered.")

    # Per-symbol comparison V11 vs V14 full
    print("\n" + "=" * 100)
    print("PER-SYMBOL: V11 baseline vs V14 full")
    print("=" * 100)
    v11_trades = results["V11 baseline (a=0,b=0)"][1]
    v14_trades = results["V14 full (A+B)       "][1]
    by_sym_v11 = defaultdict(list)
    by_sym_v14 = defaultdict(list)
    for t in v11_trades:
        by_sym_v11[t["symbol"]].append(t)
    for t in v14_trades:
        by_sym_v14[t["symbol"]].append(t)

    print(f"{'Symbol':<8}| {'V11 #':>5} {'V11 Tot':>9} {'V11 WR':>7} | {'V14 #':>5} {'V14 Tot':>9} {'V14 WR':>7} | {'dTot':>8}")
    print("-" * 90)
    syms = sorted(set(list(by_sym_v11.keys()) + list(by_sym_v14.keys())))
    for sym in syms:
        m11 = calc_metrics(by_sym_v11.get(sym, []))
        m14 = calc_metrics(by_sym_v14.get(sym, []))
        print(f"{sym:<8}| {m11['trades']:>5} {m11['total_pnl']:>+8.1f}% {m11['wr']:>6.1f}% | "
              f"{m14['trades']:>5} {m14['total_pnl']:>+8.1f}% {m14['wr']:>6.1f}% | "
              f"{m14['total_pnl']-m11['total_pnl']:>+7.1f}%")
