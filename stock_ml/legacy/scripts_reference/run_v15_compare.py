"""
V15 — Built from V14. Adds 3 new modules to attack signal-exit losses & whipsaw:
==============================================================================
  Module A — V-SHAPE ENTRY (kept from V14)
  Module B — PROFIT-PEAK PROTECTION (kept from V14)

  Module C — FAST LOSS CUT: In first 5 bars, if cum_ret < -3% AND macd_hist<0
             AND close<open, exit IMMEDIATELY (bypass MIN_HOLD).
             Goal: cut REE/TCB signal-exit losses (-7% to -13% trades).

  Module D — ADAPTIVE EXIT CONFIRMATION: Reduce EXIT_CONFIRM from 3 to 1
             when cum_ret < 0 OR cum_ret < max_profit*0.6.
             Keep 3 when in profit. Fast exit when bad, patient when good.

  Module E — SECONDARY BREAKOUT SCANNER: Loosen consolidation_breakout
             (range 8% -> 10%, vol 1.2x -> 1.1x if MA20>MA50, use max_high_5d).
             Goal: catch ACB May-Jul 2025 sideways breakout V11 missed.

Ablation:
  V14 baseline (A+B only)
  + C / + D / + E individually
  + C+D / + C+E / + D+E
  V15 full (A+B+C+D+E)
"""
...
def backtest_v15(y_pred, returns, df_test, feature_cols,
    ...
    for i in range(1, n):
        ...
        if mod_e and new_position == 0 and position == 0 and secondary_breakout[i]:
            new_position = 1
            breakout_entry = True
        ...
        if position == 1:
            ...
            # Exit confirmation (Module D adaptive)
            ...
            # Strong trend override (V11 original)
            if new_position == 0 and exit_reason == "signal":
...
def run_test(symbols_str, mod_a, mod_b, mod_c=False, mod_d=False, mod_e=False):
    ...
    for window, train_df, test_df in splitter.split(df):
        ...
        for sym in test_df["symbol"].unique():
            ...
            r = backtest_v15(y_pred, rets, sym_test, feature_cols,
...
if __name__ == "__main__":
    ...
    print("=" * 110)
    print(f"V15 ABLATION STUDY (built on V14 + 3 new modules) — Symbols: {args.pick}")
    print("=" * 110)
    print("A=V-shape entry   B=Peak-protect   C=Fast-loss-cut   D=Adaptive-confirm   E=Secondary-breakout")
    print()

    # a, b, c, d, e
    configs = [
        ("V14 base (A+B)        ", True, True, False, False, False),
        ("V14 + C only          ", True, True, True, False, False),
        ("V14 + D only          ", True, True, False, True, False),
        ("V14 + E only          ", True, True, False, False, True),
        ("V14 + C+D             ", True, True, True, True, False),
        ("V14 + C+E             ", True, True, True, False, True),
        ("V14 + D+E             ", True, True, False, True, True),
        ("V15 full (A+B+C+D+E)  ", True, True, True, True, True),
    ]

    print(f"{'Config':<24} | {'#':>4} {'WR':>6} {'AvgPnL':>8} {'TotPnL':>10} {'PF':>6} "
          f"{'MaxLoss':>8} {'AvgHold':>8}")
    print("-" * 110)

    results = {}
    for name, ma, mb, mc, md, me in configs:
        trades = run_test(args.pick, ma, mb, mc, md, me)
        m = calc_metrics(trades)
        results[name] = (m, trades)
        print(f"{name:<24} | {m['trades']:>4} {m['wr']:>5.1f}% {m['avg_pnl']:>+7.2f}% "
              f"{m['total_pnl']:>+9.1f}% {m['pf']:>5.2f} {m['max_loss']:>+7.1f}% {m['avg_hold']:>6.1f}d")

    # Module breakdowns in V15 full
    print("\n" + "=" * 110)
    print("MODULE BREAKDOWN (V15 full)")
    print("=" * 110)
    full_trades = results["V15 full (A+B+C+D+E)  "][1]

    vshape = [t for t in full_trades if t.get("vshape_entry")]
    if vshape:
        m = calc_metrics(vshape)
        print(f"  A V-shape entries : n={m['trades']} WR={m['wr']:.1f}% Avg={m['avg_pnl']:+.2f}% Tot={m['total_pnl']:+.1f}% PF={m['pf']:.2f}")

    pp = [t for t in full_trades if t.get("exit_reason", "").startswith("peak_protect")]
    if pp:
        m = calc_metrics(pp)
        print(f"  B Peak-protect    : n={m['trades']} Avg={m['avg_pnl']:+.2f}% Tot={m['total_pnl']:+.1f}%")

    flc = [t for t in full_trades if t.get("exit_reason") == "fast_loss_cut"]
    if flc:
        m = calc_metrics(flc)
        print(f"  C Fast loss cut   : n={m['trades']} Avg={m['avg_pnl']:+.2f}% Tot={m['total_pnl']:+.1f}%")

    # Per-symbol V14 vs V15
    print("\n" + "=" * 110)
    print("PER-SYMBOL: V14 base (A+B) vs V15 full (A+B+C+D+E)")
    print("=" * 110)
    v14_trades = results["V14 base (A+B)        "][1]
    v15_trades = results["V15 full (A+B+C+D+E)  "][1]
    by_sym_v14 = defaultdict(list)
    by_sym_v15 = defaultdict(list)
    for t in v14_trades:
        by_sym_v14[t["symbol"]].append(t)
    for t in v15_trades:
        by_sym_v15[t["symbol"]].append(t)

    print(f"{'Symbol':<7}| {'V14 #':>5} {'V14 Tot':>9} {'V14 WR':>7} {'V14 PF':>6} | "
          f"{'V15 #':>5} {'V15 Tot':>9} {'V15 WR':>7} {'V15 PF':>6} | {'dTot':>8}")
    print("-" * 110)
    syms = sorted(set(list(by_sym_v14.keys()) + list(by_sym_v15.keys())))
    tot14 = tot15 = 0
    for sym in syms:
        m14 = calc_metrics(by_sym_v14.get(sym, []))
        m15 = calc_metrics(by_sym_v15.get(sym, []))
        tot14 += m14["total_pnl"]
        tot15 += m15["total_pnl"]
        print(f"{sym:<7}| {m14['trades']:>5} {m14['total_pnl']:>+8.1f}% {m14['wr']:>6.1f}% {m14['pf']:>5.2f} | "
              f"{m15['trades']:>5} {m15['total_pnl']:>+8.1f}% {m15['wr']:>6.1f}% {m15['pf']:>5.2f} | "
              f"{m15['total_pnl']-m14['total_pnl']:>+7.1f}%")
    print("-" * 110)
    print(f"{'TOTAL':<7}| {'':>5} {tot14:>+8.1f}% {'':>7} {'':>6} | {'':>5} {tot15:>+8.1f}% {'':>7} {'':>6} | {tot15-tot14:>+7.1f}%")
