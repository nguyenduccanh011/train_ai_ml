"""
V16 — Built from V15 (A+B+E). Attack 2021-2022 bear losses + BO fakeouts.
=========================================================================
  Modules A/B/E kept as in V15 (proven positive).

  Module F — BO QUALITY FILTER: Breakout entries (consolidation & secondary)
             require MACD hist > 0 AND close > open AND volume > 1.5×.
             Goal: drop fakeout BOs at BID (2024-04), VNM (2020-07, 2023-12)
             that died in 2-8 bars.

  Module G — BEAR REGIME DEFENSE: Block ALL non-V-shape entries when
             60d_return < -10% AND close < SMA50 AND SMA20 < SMA50.
             Goal: avoid 2022 bear cluster (TCB -42.6%, BID -29.7%, VNM -22.3%).
"""
...
def run_test(symbols_str, mod_a, mod_b, mod_c=False, mod_d=False, mod_e=False,
    ...
    for window, train_df, test_df in splitter.split(df):
        ...
        for sym in test_df["symbol"].unique():
            ...
            r = backtest_v16(y_pred, rets, sym_test, feature_cols,
...
if __name__ == "__main__":
    ...
    print("=" * 110)
    print(f"V16 ABLATION STUDY (built on V15 A+B+E, testing F+G) - Symbols: {args.pick}")
    print("=" * 110)
    print("F=BO Quality Filter   G=Bear Regime Defense")
    print()

    # a, b, c, d, e, f, g
    configs = [
        ("V11 baseline         ", False, False, False, False, False, False, False),
        ("V15 best (A+B+E)     ", True,  True,  False, False, True,  False, False),
        ("V15 + F only         ", True,  True,  False, False, True,  True,  False),
        ("V15 + G only         ", True,  True,  False, False, True,  False, True),
        ("V16 full (A+B+E+F+G) ", True,  True,  False, False, True,  True,  True),
    ]
    ...
    results = {}
    for name, ma, mb, mc, md, me, mf, mg in configs:
        trades = run_test(args.pick, ma, mb, mc, md, me, mf, mg)
    ...
    # Per-symbol V15 vs V16 full
    print("\n" + "=" * 110)
    print("PER-SYMBOL: V11 vs V15-best vs V16-full")
    print("=" * 110)
    t11 = results["V11 baseline         "][1]
    t15 = results["V15 best (A+B+E)     "][1]
    t16 = results["V16 full (A+B+E+F+G) "][1]
    b11, b15, b16 = defaultdict(list), defaultdict(list), defaultdict(list)
    for t in t11: b11[t["symbol"]].append(t)
    for t in t15: b15[t["symbol"]].append(t)
    for t in t16: b16[t["symbol"]].append(t)

    print(f"{'Sym':<6}| {'V11 Tot':>9} {'V11 PF':>6} | {'V15 Tot':>9} {'V15 PF':>6} | "
          f"{'V16 Tot':>9} {'V16 PF':>6} | {'V16-V11':>8} {'V16-V15':>8}")
    print("-" * 110)
    T11=T15=T16=0
    for sym in sorted(set(b11.keys()) | set(b16.keys())):
        m11 = calc_metrics(b11.get(sym, []))
        m15 = calc_metrics(b15.get(sym, []))
        m16 = calc_metrics(b16.get(sym, []))
        T11 += m11["total_pnl"]; T15 += m15["total_pnl"]; T16 += m16["total_pnl"]
        print(f"{sym:<6}| {m11['total_pnl']:>+8.1f}% {m11['pf']:>5.2f} | "
              f"{m15['total_pnl']:>+8.1f}% {m15['pf']:>5.2f} | "
              f"{m16['total_pnl']:>+8.1f}% {m16['pf']:>5.2f} | "
              f"{m16['total_pnl']-m11['total_pnl']:>+7.1f}% {m16['total_pnl']-m15['total_pnl']:>+7.1f}%")
    print("-" * 110)
    print(f"{'TOT':<6}| {T11:>+8.1f}% {'':>6} | {T15:>+8.1f}% {'':>6} | {T16:>+8.1f}% {'':>6} | "
          f"{T16-T11:>+7.1f}% {T16-T15:>+7.1f}%")
