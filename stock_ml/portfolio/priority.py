...
def meta_features(q: pd.DataFrame) -> pd.DataFrame:
    ...
    q = q.copy()
    q['date'] = pd.to_datetime(q['date']); out = []
...
def meta_priority(base_trades: pd.DataFrame, signals: pd.DataFrame, ohlcv_frame: pd.DataFrame) -> dict:
    ...
    tr['t_pnl'] = tr.exit_price / tr.entry_price - 1.0; tr['yr'] = tr.entry_date.dt.year
    sig['date'] = pd.to_datetime(sig['date']); parts = []
    for s, tg in tr.groupby('symbol'):
        sg = sig[sig.symbol == s].sort_values('date')
        m = pd.merge_asof(tg.sort_values('entry_date'),
