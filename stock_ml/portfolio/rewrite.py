...
def rewrite(
    base_trades: pd.DataFrame, CLO, DIDX, INV, gt: float, ec_check_bar: int = 2
) -> pd.DataFrame:
    ecb = ec_check_bar
    ned, nep, nreason = [], [], []
    for r in base_trades.itertuples():
        ...
        c = CLO[r.symbol]
        if (
            xi > ei + ecb + 1 and c[ei + ecb] / r.entry_price - 1.0 < 0.0
        ):  # red@s{ecb} -> early-cut next bar
            ned.append(INV[r.symbol][ei + ecb + 1])
            nep.append(float(c[ei + ecb + 1]))
