    ...
    for i in range(1, n):
        ...
        if new_position == 1 and position == 0:
            ...
            if dist_sma20[i] > 0.05 and position_size > 0.5:
                position_size = 0.5
            # Keep strong breakout entries but downsize to avoid chasing risk.
...
if __name__ == "__main__":
    ...
    print("=" * 132)
    print(f"V18 COMPARISON (V11/V17/V18 + Rule) - Symbols: {args.pick}")
    print("=" * 132)
    ...
    print("\n" + "=" * 132)
    print("PER-SYMBOL: V11 vs V17 vs V18 vs Rule")
    print("=" * 132)
    ...
    b11, b17, b18, br = defaultdict(list), defaultdict(list), defaultdict(list), defaultdict(list)
    ...
    print(f"{'Sym':<6}| {'V11 Tot':>9} {'V11 PF':>6} | {'V17 Tot':>9} {'V17 PF':>6} | "
          f"{'V18 Tot':>9} {'V18 PF':>6} | {'Rule Tot':>9} {'Rule PF':>7} | {'V18-V17':>8} {'V18-Rule':>9}")
    print("-" * 132)
    T11 = T17 = T18 = TR = 0
    for sym in sorted(set(b11.keys()) | set(b17.keys()) | set(b18.keys()) | set(br.keys())):
        m11 = calc_metrics(b11.get(sym, []))
        m17 = calc_metrics(b17.get(sym, []))
        m18 = calc_metrics(b18.get(sym, []))
        mr = calc_metrics(br.get(sym, []))
        T11 += m11["total_pnl"]; T17 += m17["total_pnl"]; T18 += m18["total_pnl"]; TR += mr["total_pnl"]
        print(f"{sym:<6}| {m11['total_pnl']:>+8.1f}% {m11['pf']:>5.2f} | "
              ...
              f"{m18['total_pnl']-m17['total_pnl']:>+7.1f}% {m18['total_pnl']-mr['total_pnl']:>+8.1f}%")
    print("-" * 132)
    print(f"{'TOT':<6}| {T11:>+8.1f}% {'':>6} | {T17:>+8.1f}% {'':>6} | "
          f"{T18:>+8.1f}% {'':>6} | {TR:>+8.1f}% {'':>7} | {T18-T17:>+7.1f}% {T18-TR:>+8.1f}%")
