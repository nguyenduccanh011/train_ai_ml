"""
V19.3 Signal Exit + Binary Sizing.
===================================
V19.2 exit overhaul + position sizing overhaul:
1) Binary sizing: High(90-100%), Medium(50%), Low(30%) - eliminates weak 60-80% bucket
2) ATR-based size cap: ATR/price > 4% -> max 50%, > 5.5% -> max 35%
3) Weak trend max 40%, Moderate max 70%
"""
...
def backtest_v19_3(y_pred, returns, df_test, feature_cols,
    ...
    for i in range(1, n):
        ...
        if position == 1:
            ...
            # 0.5) V19.2: SIGNAL HARD CAP using price return
            elif price_cur_ret <= -SIGNAL_HARD_CAP:
                new_position = 0
                exit_reason = "signal_hard_cap"
                n_signal_hard_cap += 1

            # 0.6) V19.2: FAST EXIT when price drops > 5%
