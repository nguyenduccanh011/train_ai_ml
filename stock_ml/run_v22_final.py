...
def backtest_v22_final(y_pred, returns, df_test, feature_cols,
                       ...
                       # V22 params - tunable
                       fast_exit_skip_strong=True,     # Skip fast_exit when trend healthy
                       fast_exit_vol_confirm=True,     # Require volume confirm for fast exit
                       fast_exit_threshold_hb=-0.07,   # High beta threshold
                       fast_exit_threshold_std=-0.05,   # Standard threshold
                       adaptive_hard_cap=True,          # ATR-adaptive hard cap
                       hard_cap_mult_hb=3.0,            # High beta cap multiplier
                       hard_cap_mult_std=2.5,           # Standard cap multiplier
                       hard_cap_floor=0.12,             # Minimum hard cap
                       hard_cap_floor_hb=0.15,          # Minimum for high_beta
                       hybrid_rule_entry=False,         # Rule-based fallback entries
                       early_cycle_relax=False,         # Relax filters in early cycle
                       ):
    """V22 Final: Targeted fixes on V19.3 base."""
    n = len(y_pred)
    equity = np.zeros(n)
    ...
    def get_regime_adapter(i, trend):
        ...
        if atr_ratio > 0.055:
            params["size_mult"] *= 0.90; params["base_confirm_bars"] += 1; params["exit_score_threshold"] += 0.20
        if trend == "strong":
    ...
    for i in range(1, n):
        ...
        if position == 1:
            ...
            # 0.5) SIGNAL HARD CAP - V22: ATR-adaptive
            elif adaptive_hard_cap:
                if profile == "high_beta":
                    cap = max(hard_cap_floor_hb, hard_cap_mult_hb * atr_ratio_now)
                else:
                    cap = max(hard_cap_floor, hard_cap_mult_std * atr_ratio_now)
            ...
            # 0.6) FAST EXIT LOSS - V22: Smart version
            if new_position == 1:
                do_fast_exit = False
                ft = fast_exit_threshold_hb if profile == "high_beta" else fast_exit_threshold_std
                mt = ft + 0.02  # moderate threshold = fast - 2%

                # Check if trend is still healthy (for skip logic)
                trend_healthy = (fast_exit_skip_strong and strong_uptrend and
                                macd_line[i] > 0 and
                                not np.isnan(sma20[i]) and close[i] > sma20[i] * 0.97)

                # Volume-based confirm: high selling volume = real distribution
                vol_selling = (fast_exit_vol_confirm and
            ...
            if new_position == 0 and exit_reason == "signal" and mod_h:
                ...
                if hold_days > 15 and cum_ret < 0.03: score_threshold *= 0.60
                elif hold_days > 10 and cum_ret < 0.01: score_threshold *= 0.75
...
if __name__ == "__main__":
    ...
    configs = [
        ...
        ("V22-no_vol_confirm", {"fast_exit_vol_confirm": False}),
        ("V22-no_skip_strong", {"fast_exit_skip_strong": False}),
        ("V22-hb_thresh=-6%", {"fast_exit_threshold_hb": -0.06}),
        ("V22-hb_thresh=-8%", {"fast_exit_threshold_hb": -0.08}),
        ("V22-std_thresh=-6%", {"fast_exit_threshold_std": -0.06}),
        ("V22-std_thresh=-4%", {"fast_exit_threshold_std": -0.04}),
        ("V22-cap_mult_hb=2.5", {"hard_cap_mult_hb": 2.5}),
        ("V22-cap_mult_hb=3.5", {"hard_cap_mult_hb": 3.5}),
        ("V22-cap_floor_hb=0.18", {"hard_cap_floor_hb": 0.18}),
        ("V22-no_adaptive_cap", {"adaptive_hard_cap": False}),
        # Combo refinements
        ("V22-hb8+std6", {"fast_exit_threshold_hb": -0.08, "fast_exit_threshold_std": -0.06}),
        ("V22-hb8+std6+cap3.5", {"fast_exit_threshold_hb": -0.08, "fast_exit_threshold_std": -0.06, "hard_cap_mult_hb": 3.5}),
        ("V22-hb7+novol", {"fast_exit_vol_confirm": False, "fast_exit_threshold_hb": -0.07}),
        ("V22-hb8+novol", {"fast_exit_vol_confirm": False, "fast_exit_threshold_hb": -0.08}),
        ("V22-hb7+std6+novol", {"fast_exit_vol_confirm": False, "fast_exit_threshold_hb": -0.07, "fast_exit_threshold_std": -0.06}),
    ]
    ...
    for label, params in configs:
        ...
        def make_fn(p):
            def bt_fn(y_pred, returns, df_test, feature_cols, **kwargs):
                return backtest_v22_final(y_pred, returns, df_test, feature_cols, **{**kwargs, **p})
            return bt_fn
