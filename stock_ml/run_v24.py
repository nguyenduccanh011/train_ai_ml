...
def backtest_v24(y_pred, returns, df_test, feature_cols,
    ...
    # V24 PATCH 5.1: Smart hard_cap confirm state
    hard_cap_pending_bars = 0
    HARD_CAP_CONFIRM_STRONG = 1    # strong: need 2 bars (1 pending + 1 confirm)
    HARD_CAP_CONFIRM_MODERATE = 1  # moderate: also 2 bars
    HARD_CAP_CONFIRM_WEAK = 0      # weak: cut immediately (keep V23 logic)

    # V24 PATCH 5.2: Peak protect pending state
    pp_pending_bars = 0
    ...
    # V24 PATCH 5.3: Long-horizon carry pre-computation
    ...
    # V24 PATCH 5.5: Rule signal pre-computation
    ...
    def get_regime_adapter(i, trend):
        ...
        # V24 PATCH 5.4: Symbol-specific overrides
        if sym == "REE":
            params["exit_score_threshold"] += 0.6  # harder to exit via signal
        elif sym == "AAS":
            params["disable_profit_lock_in_strong"] = True  # big winners >+50%
        elif sym == "MBB":
            params["pp_sensitivity_bonus"] = 0.02  # lower pp threshold by 2%
        return params
    ...
    for i in range(1, n):
        ...
        if position == 1:
            ...
            # ═══ V24 PATCH 5.1: Smart hard_cap with confirm bars ═══
                ...
                if price_cur_ret <= -cap:
                    ...
                    if hard_cap_pending_bars >= 1 + HARD_CAP_CONFIRM_MODERATE:
                        ...
                        hard_cap_pending_bars = 0
                        counters["signal_hard_cap"] += 1
                    # else: don't exit yet, wait for confirm bar
                else:
                    hard_cap_pending_bars = 0  # price recovered, reset
            else:  # strong
                if profile == "high_beta":
                    cap = max(0.18, 3.5 * atr_ratio_now)  # V24: raised floor 0.15->0.18, mult 3.0->3.5
                else:
            ...
            # ═══ V24 PATCH 5.2: Peak protection — V19.1 style sensitivity ═══
            if new_position == 1 and mod_b:
                # Layer 1: aggressive lock when profit already large (>=25%)
                ...
                if price_max_profit >= pp_threshold_active:
                    ...
                    # V24: only need below_sma10 + confirm 1 bar (no 3-in-1)
            ...
            # Profit lock — V24 PATCH 5.4: disable in strong trend for AAS
            ...
            # ═══ V24 PATCH 5.3: Long-horizon carry — override exit in big momentum ═══
        ...
        if new_position != position:
            if new_position == 1:
                ...
                hard_cap_pending_bars = 0; pp_pending_bars = 0  # V24: reset states
...
def get_all_viable_symbols(data_dir, min_rows=DEFAULT_MIN_ROWS):
...
def resolve_symbols(symbols_arg="", use_all_clean=True, min_rows=DEFAULT_MIN_ROWS):
    ...
    if symbols_arg and symbols_arg.strip():
        ...
        return [s for s in pick if s in loader.symbols]

    if not use_all_clean:
        pick = [s.strip().upper() for s in DEFAULT_SYMBOLS.split(",") if s.strip()]
        return [s for s in pick if s in loader.symbols]

    viable = get_all_viable_symbols(data_dir, min_rows=min_rows)
    return [s for s in viable if s in loader.symbols]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--default14", action="store_true",
                        help="Use legacy 14 symbols instead of auto all clean symbols.")
    parser.add_argument("--min-rows", type=int, default=DEFAULT_MIN_ROWS,
                        help="Minimum rows per symbol for auto all-clean mode.")
    args = parser.parse_args()

    _sys.stdout = open(_sys.stdout.fileno(), mode='w', encoding='utf-8', buffering=1)

    OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "results")
    symbols_list = resolve_symbols(
        symbols_arg=args.symbols,
        use_all_clean=not args.default14,
    ...
    print("=" * 130)
    mode = "custom" if args.symbols else ("default14" if args.default14 else "all_clean")
    print(f"  Symbols: {len(symbols_list)} (mode={mode}, min_rows={args.min_rows})")
    ...
    df_23 = pd.DataFrame(t_v23); df_r = pd.DataFrame(t_rule); df_24 = pd.DataFrame(t_v24)
    print(f"  {'Sym':<6}| {'V19.1':>9} | {'V22':>9} | {'V23':>9} | {'Rule':>9} | {'V24':>9} | {'V24-V23':>9} | {'V24-Rule':>9}")
    print("  " + "-" * 100)
    for sym in sorted(df_24["symbol"].unique() if "symbol" in df_24.columns else []):
        def sp(df, s): return calc_metrics(df[df["symbol"]==s].to_dict("records")) if "symbol" in df.columns else calc_metrics([])
        m1=sp(df_191,sym); m2=sp(df_22,sym); m3=sp(df_23,sym); mr=sp(df_r,sym); m4=sp(df_24,sym)
    ...
    # V24 exit reasons
    if len(df_24) > 0 and "exit_reason" in df_24.columns:
        print(f"\n  V24 EXIT REASONS:")
        for reason, grp in df_24.groupby("exit_reason"):
    ...
    # V24 counters
    r0 = run_test_base(SYMBOLS.split(",")[0], True, True, False, False, True, True, True, True, True, True, backtest_fn=backtest_v24)
    # Just print summary
