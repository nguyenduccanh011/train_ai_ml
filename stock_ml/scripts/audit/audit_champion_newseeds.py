...
def comp(rid):
    ...
    r = cur.fetchone(); con.close(); return (float(r[0]), float(r[1])) if r and r[0] is not None else (None, None)
...
print(f"  2409 baseline: per-seed {res[2409]}  mean {statistics.mean([v for v in res[2409].values() if v]):.1f}")
print(f"  2417 champion: per-seed {res[2417]}  mean {statistics.mean([v for v in res[2417].values() if v]):.1f}")
g = statistics.mean([v for v in res[2417].values() if v]) - statistics.mean([v for v in res[2409].values() if v])
