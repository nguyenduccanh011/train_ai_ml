...
async def make_clone() -> int:
    ...
    async with Session() as s:
        ...
        eng = copy.deepcopy(eng); eng.update(GATE)   # the only change
        t = await repo.create(
...
def main():
    ...
    for sd in SEEDS:
        ...
        row = read(r.get("run_id")); comp = float(row[0]) if row and row[0] is not None else None
        seeds[sd] = comp
        print(f"  {NEW_NAME} seed={sd}: comp={comp} pnl={row[1]:.1f} pf={row[2]:.2f} mdd={row[3]:.3f} tr={row[4]}", flush=True)
