...
OV = {
    "trailing_struct_donch_win": 80,
    "trailing_struct_apply_overext": True,
...
async def make():
    ...
    async with S() as s:
        ...
        t = await repo.create(
            ...
            seed=b.seed,
            description="PnL-FRAME: structure-ride + max hold-continuation "
...
def main():
    ...
    for sd in SEEDS:
        ...
        print(
            f"  seed {sd}: comp={row[0]} PnL={row[1]:.1f} pf={row[2]:.2f} mdd={row[3]:.3f} tr={row[4]}",
            flush=True,
        )
    print(
