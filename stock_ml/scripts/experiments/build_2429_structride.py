...
async def make():
    ...
    async with S() as s:
        ...
        t = await repo.create(
            ...
            seed=b.seed, description="2429 + STRUCTURE-RIDE Donchian-80 trail (default+overext tiers exit "
...
def main():
    ...
    for sd in SEEDS:
        ...
        print(f"  seed {sd}: comp={seeds[sd]} pnl={row[1]:.1f} pf={row[2]:.2f} mdd={row[3]:.3f} tr={row[4]}", flush=True)
