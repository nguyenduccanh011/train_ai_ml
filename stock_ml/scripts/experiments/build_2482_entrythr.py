...
async def make(thr):
    ...
    async with S() as s:
        ...
        t = await repo.create(
            ...
            seed=b.seed,
            description=f"2482 structure-ride + LOOSER entry_threshold {thr} (more dip-buy "
...
def main():
    res = []
    for thr in THRS:
        tid, name = asyncio.run(make(thr))
        asyncio.run(async_engine.dispose())
        r = run_template_experiment(template_id=tid, seed=555)
        row = rd(r.get("run_id"))
        res.append((thr, tid, name, row))
        print(
            f"  et{thr} (id {tid}): comp={row[0]} pnl={row[1]:.1f} pf={row[2]:.2f} mdd={row[3]:.3f} tr={row[4]}",
            flush=True,
        )
    print(
    ...
    for thr, tid, name, row in res:
        dpnl = float(row[1]) - CHAMP["pnl"]
        dmdd = float(row[3]) - CHAMP["mdd"]
        dcomp = float(row[0]) - CHAMP["comp"]
        keep = dpnl > 2.0  # meaningful PnL gain (user accepts MDD)
        print(
            f"  et{thr}: Δpnl={dpnl:+.1f} Δmdd={dmdd:+.3f} Δcomp={dcomp:+.1f} -> {'KEEP (PnL-frame)' if keep else 'deactivate'}"
        )
        if not keep:
            deactivate(tid)
