...
for sd in SEEDS:
    ...
    print(
        "seed%d: " % sd + " ".join(f"{m}K{K}={res[(m, K)][sd][0]:.1f}" for m, K in CONF), flush=True
    )
con.close()

REG = {
    ("prio_eq", 16): (33.71, "k16prio"),
    ("size", 16): (57.42, "k16size(fixed)"),
    ("size", 25): (41.24, "k25size"),
    ("prio_eq", 25): (None, "(no k25prio row)"),
}
print("\n=== AUDIT: clean recompute vs registered leaderboard_nav ===")
print("config        | clean NAV(3-seed) | clean CAGR | registered NAV | verdict")
for mode, K in CONF:
    navs = [res[(mode, K)][s][0] for s in SEEDS]
    cn = sum(navs) / 3
    ...
    if reg is None:
        ...
        vd = (
            "OK (reproduces)"
            if 0.9 <= ratio <= 1.1
            else f"INFLATED reg={reg} is {reg / cn:.2f}x clean"
        )
    print(
        f"{mode:7s} K{K:2d} {nm:16s}| {cn:6.2f}            | {cagr * 100:5.1f}%    | {str(reg):>8s}       | {vd}"
    )
