"""BROAD batch 3 — build ON the confirmed winner ft_rs (entry_recov_rs, NAV frontier 64.87% mean).
Test the last untested RS feature variant (rs_vwap) + combine the RS feature win with the 2 best
target swaps (multi_horizon, mfe) to see if independent gains compound. Base = 3102 (ft_rs).
"""
...
BASE_TMPL = 3102  # ft_rs (entry_recov_rs)
SEEDS = [42]
# (name, feature_set override or None, target_config override or None)
VARIANTS = [
    ("b3_rsvwap", "entry_recov_rs_vwap", None),
    ("b3_rs_multih", None, {"type": "multi_horizon_return", "horizons": [5, 10, 20]}),
    ("b3_rs_mfe", None, {"type": "mfe_regression", "horizon": 20}),
    ("b3_rs_swing", None, {"type": "swing_value_regression", "horizon": 20}),
]
...
async def make_all() -> dict:
    ...
    async with Session() as s:
        ...
        for new_name, fset, tgt in VARIANTS:
            ...
            for sl in slots:
                if sl["slot_type"] == "entry":
                    ...
                    if tgt:
                        sl["target_config"] = copy.deepcopy(tgt)
            t = await repo.create(
                name=new_name,
                market=base.market,
                ...
                seed=base.seed,
                description=f"ft_rs base + {'feat=' + fset if fset else ''}{'target=' + tgt['type'] if tgt else ''} (batch3).",
                hypothesis="Do RS-feature win + best target swap compound / does rs_vwap beat rs?",
