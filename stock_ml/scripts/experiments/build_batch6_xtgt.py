"""BROAD batch 6 — swap the EXIT ML target (champion=velocity_exit_regression) on the double-RS
frontier (3115). Exit-target never swapped this campaign. Alternatives: risk_exit, trend_scanning,
downleg_depth, reward_risk. Engine exit rules (rgskip/velocity-trail) keep using exit_score; this
tests whether a different exit-head label improves the frontier or the velocity head is optimal.
"""
...
SEEDS = [42]
VARIANTS = [  # (name, new exit-slot target_config)
    ("b6_xrisk",    {"type": "risk_exit_regression", "horizon": 20}),
    ("b6_xtrend",   {"type": "trend_scanning_exit", "windows": [10, 20, 40]}),
    ("b6_xdownleg", {"type": "downleg_depth_regression", "max_span": 40}),
    ("b6_xrewrisk", {"type": "reward_risk_regression", "horizon": 20}),
]
        ...
        for new_name, xtgt in VARIANTS:
            ...
            for sl in slots:
                if sl["slot_type"] == "exit":
                    sl["target_config"] = copy.deepcopy(xtgt)
            t = await repo.create(
                ...
                description=f"double-RS + exit target -> {xtgt['type']} (batch6 exit-target swap).",
                hypothesis="Does a non-velocity exit ML target improve the double-RS frontier?",
