"""Extend the validated sizing win: conviction = simple z-BLEND of the top robust extension features
(dist20low, dma20, rsi14, ret20 — all +IC vs pnl every year, causal a-priori, NO model to avoid the
overfit that killed the learned version). Does a blend beat the single dma20 (+29%)? Fair mean-1 tilt,
signal-date features (conservative), priority-fill by score, K16, 3-seed. All causal -> walk-forward-clean.
"""
...
def run_sized(sim, K, k_conv, roundtrip=0.006, settle_lag=2, advance_fee=0.0008):
    ...
    cash = 1.0; pending = defaultdict(float); pt = 0.0; legs = []; exits = defaultdict(list); navs = []; exp = 0.0
    for di, dt in enumerate(cal):
        ...
        pos = sum(lv(l, dt) for l in legs); nav = cash + pt + pos; navs.append((dt, nav)); exp += (pos / nav if nav > 0 else 0)
    ns = pd.DataFrame(navs, columns=["date", "nav"]); nav = ns["nav"]; dd = nav / nav.cummax() - 1
    return dict(final=float(nav.iloc[-1]), maxdd=float(dd.min()), exposure=exp / len(ns))


# panel
_cx = duckdb.connect(MARKET, read_only=True)
_px = _cx.execute("SELECT symbol,date,high,low,close FROM ohlcv WHERE timeframe='1D' AND date>='2018-06-01' "
...
for sd in SEEDS:
    ...
    m = tr.merge(PANEL[["symbol", "date", "dist_ma20", "blend"]], left_on=["symbol", "sigd"], right_on=["symbol", "date"], how="left")
    m["ek"] = m["symbol"] + "|" + m["entry_date"].astype(str).str[:10]
    dma = dict(zip(m["ek"], m["dist_ma20"].fillna(0.0)))
    ...
    def mk(cm):
        ...
        for t in sim.trades:
            ...
            t["score"] = slook.get(ek, 0.0); t["conv"] = cm.get(ek, 0.0)
        return sim
    res["equal"][sd] = run_sized(mk({}), K, 0.0)["final"]
    res["dma20"][sd] = run_sized(mk(dma), K, 1.0)["final"]
    res["blend"][sd] = run_sized(mk(bld), K, 1.0)["final"]
    print(f"seed{sd}: equal={res['equal'][sd]:.2f} dma20={res['dma20'][sd]:.2f} blend={res['blend'][sd]:.2f}", flush=True)
con.close()
print("\n=== BLEND vs single-dma20 vs equal (fair K16, walk-forward-clean) ===")
for nm in ("dma20", "blend"):
    navs = [res[nm][s] for s in SEEDS]; d = [res[nm][s] - res["equal"][s] for s in SEEDS]
    signs = "".join("+" if x > 0 else "-" for x in d)
    print(f"{nm:6s}: mean={sum(navs)/3:.3f} navs={[f'{n:.2f}' for n in navs]} | Δvs-equal={[f'{x:+.2f}' for x in d]} signs={signs}")
