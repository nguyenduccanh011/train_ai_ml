    ...
    for di, dt in enumerate(cal):
        ...
        for leg in exits.get(dt, ()):
            ...
            if advance_fee is not None:
                ...
                ri = di + settle_lag
                (
                    pending.__setitem__(cal[ri], pending[cal[ri]] + proceeds)
                    if ri < len(cal)
                    else None
                )
            legs.remove(leg)
...
_px = _cx.execute(
    "SELECT symbol,date,high,low,close FROM ohlcv WHERE timeframe='1D' AND date>='2018-06-01' "
...
for sd in SEEDS:
    ...
    m = tr.merge(
        PANEL[["symbol", "date", "dist_ma20", "blend"]],
        left_on=["symbol", "sigd"],
    ...
    print(
        f"seed{sd}: equal={res['equal'][sd]:.2f} dma20={res['dma20'][sd]:.2f} blend={res['blend'][sd]:.2f}",
        flush=True,
    )
con.close()
...
for nm in ("dma20", "blend"):
    ...
    d = [res[nm][s] - res["equal"][s] for s in SEEDS]
    signs = "".join("+" if x > 0 else "-" for x in d)
    print(
        f"{nm:6s}: mean={sum(navs) / 3:.3f} navs={[f'{n:.2f}' for n in navs]} | Δvs-equal={[f'{x:+.2f}' for x in d]} signs={signs}"
    )
