"""Learner CAPACITY + ALGORITHM sweep on frontier x2_struct_to (t3185).

The champion entry head is EXTREMELY shallow/regularized (num_leaves=7, min_data_in_leaf=200,
lambda_l1=l2=1.0 -> near-linear). Every session probe used this frozen learner. If the RS+recovery
signal has INTERACTIONS (high-RS x low-vol x near-high), a 7-leaf tree can't capture them. Test
(a) more capacity (deeper/less-regularized LightGBM), (b) a different learner CLASS (MLP, wrapped in
StandardScaler) — the one predict lever untouched all session. Baseline (L2, 7-leaf) = frontier
...
BASE = 3185
BP = {'n_estimators': 300, 'learning_rate': 0.02, 'num_leaves': 7, 'min_data_in_leaf': 200,
      'feature_fraction': 0.6, 'bagging_fraction': 0.7, 'bagging_freq': 5, 'lambda_l1': 1.0, 'lambda_l2': 1.0}

# name -> (algorithm, params)
VARIANTS = {
    "cap_l15":  ("lightgbm", {**BP, "num_leaves": 15, "min_data_in_leaf": 100}),
    "cap_l31":  ("lightgbm", {**BP, "num_leaves": 31, "min_data_in_leaf": 50}),
    "cap_reg0": ("lightgbm", {**BP, "num_leaves": 15, "min_data_in_leaf": 100, "lambda_l1": 0.0, "lambda_l2": 0.0}),
    "algo_mlp": ("mlp", {"hidden_layer_sizes": [64, 32], "alpha": 1e-3, "max_iter": 300}),
}


def ensure_component(nm, algo, params):
    ...
    cur.execute(
        ...
        "VALUES (%s,'entry',%s,%s,%s,false,true,now(),now(),'ml') RETURNING id",
        (cn, algo, json.dumps(params), f"entry_plain {algo} {nm}"),
    )
    ...
    async with Session() as s:
        ...
        for nm in VARIANTS:
            ...
            t = await repo.create(
                ...
                description=f"frontier + entry learner {nm}",
                hypothesis="more capacity / different learner class extracts RS interactions",
...
comp_ids = {nm: ensure_component(nm, algo, params) for nm, (algo, params) in VARIANTS.items()}
