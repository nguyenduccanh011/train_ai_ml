...
for nm, tid in ids.items():
    try:
        r = run_template_experiment(template_id=tid, seed=42); rid = r.get("run_id"); nav = score_read(rid)
        res[nm] = nav
        print(f"{nm} seed42: NAV={nav[0]:.3f} CAGR={nav[1]*100:.2f}% DD={nav[2]*100:.2f}%", flush=True)
    except Exception as e:
        print(f"ERR {nm}: {type(e).__name__}: {str(e)[:260]}", flush=True); res[nm] = None

print("\n=== CO-ADAPTATION seed42 screen (ref: triple_barrier 48=27.14, 11=25.23) ===")
print("target | 48-feat | 11-feat | count-response")
for tk in TARGETS:
    n48 = res.get(f"ca_{tk}_48"); n11 = res.get(f"ca_{tk}_11")
    v48 = f"{n48[0]:.2f}" if n48 else "ERR"; v11 = f"{n11[0]:.2f}" if n11 else "ERR"
    resp = ""
    if n48 and n11:
