...
def run_v(sim, K, pri, k_conv, roundtrip=0.006, settle_lag=2, advance_fee=0.0008):
    ...
    for t in sim.trades:
        sz = (t.get("score", 0.0) - smu) / ssd; cz = (t.get("conv", 0.0) - mu) / sd
...
VARS = [("score/eq", "score", 0.0), ("blend/eq", "blend", 0.0), ("mix/eq", "mix", 0.0),
        ("mix/size", "mix", 1.0), ("score/size", "score", 1.0)]
...
for sd in SEEDS:
    ...
    m = tr.merge(PANEL[["symbol", "date", "blend"]], left_on=["symbol", "sigd"], right_on=["symbol", "date"], how="left")
    ...
    def mk():
        ...
        for t in sim.trades:
            ...
            t["score"] = slook.get(ek, 0.0); t["conv"] = bld.get(ek, 0.0)
        return sim
...
for nm, _, _ in VARS:
    ...
    print(f"{nm:11s}: mean={sum(navs)/3:.3f} navs={[f'{n:.2f}' for n in navs]} | Δvs-score/eq={[f'{x:+.2f}' for x in d]} {signs}")
print("CONVPRI_DONE")
