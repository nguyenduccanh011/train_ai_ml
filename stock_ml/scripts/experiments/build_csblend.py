"""#1 CROSS-SECTIONAL magnitude signal. The sizing/priority conviction blend uses GLOBAL z per feature.
The winning entry theme was cross-sectional RANK (RS), not raw levels. Rank each of the 4 extension
features cross-sectionally PER DAY (over the whole universe), average the ranks -> cs_blend (regime-
robust: 'most extended TODAY' not 'extended vs all history'). Test cs_blend vs the global-z blend on the
two monetizing levers (sizing + priority), 3-seed, walk-forward-clean. WIN = cs beats global-z 3/3.
"""
...
def run_v(sim, K, pri, k_conv, convkey, roundtrip=0.006, settle_lag=2, advance_fee=0.0008):
    """pri in {'score','mix'}; k_conv fair sizing tilt; convkey selects which conv field to use."""
    s_new = (roundtrip - FEE) / 2.0
    for t in sim.trades:
        t["net"] = (t["x_raw"] * (1.0 - s_new)) / (t["e_raw"] * (1.0 + s_new)) - 1.0 - FEE
        t["conv"] = t.get(convkey, 0.0)
    convs = [t["conv"] for t in sim.trades]
    ...
    for t in sim.trades:
        sz = (t.get("score", 0.0) - smu) / ssd; cz = (t["conv"] - mu) / sd
        t["_pri"] = sz if pri == "score" else sz + cz
...
# panel: global-z blend AND cross-sectional-rank blend
...
# cross-sectional rank blend: per-day percentile rank per feature, averaged
for col in BLEND:
    PANEL[col + "_r"] = PANEL.groupby("date")[col].rank(pct=True)
PANEL["cs_blend"] = PANEL[[c + "_r" for c in BLEND]].mean(axis=1)

con = psycopg2.connect(**PG)
# convkey: blend (global-z) vs cs_blend (cross-sectional)
VARS = [("gz score/size", "score", 1.0, "blend"), ("gz mix/size", "mix", 1.0, "blend"),
        ("cs score/size", "score", 1.0, "cs_blend"), ("cs mix/size", "mix", 1.0, "cs_blend"),
        ("cs blend/eq", "score", 0.0, "cs_blend"), ("equal", "score", 0.0, "blend")]
...
for sd in SEEDS:
    ...
    def mk():
        ...
        for t in sim.trades:
            ...
            t["score"] = slook.get(ek, 0.0); t["blend"] = gz.get(ek, 0.0); t["cs_blend"] = cs.get(ek, 0.5)
        return sim
    for nm, pri, kc, ck in VARS:
        res[nm][sd] = run_v(mk(), K, pri, kc, ck)["final"]
    print("seed%d: " % sd + " ".join(f"{nm}={res[nm][sd]:.2f}" for nm, *_ in VARS), flush=True)
con.close()

print("\n=== cross-sectional blend vs global-z blend (sizing+priority), sign 3/3 vs equal ===")
base = res["equal"]
for nm, *_ in VARS:
    navs = [res[nm][s] for s in SEEDS]; d = [res[nm][s] - base[s] for s in SEEDS]
    signs = "".join("+" if x > 0 else "-" for x in d)
    print(f"{nm:14s}: mean={sum(navs)/3:.3f} navs={[f'{n:.2f}' for n in navs]} | Δvs-eq={[f'{x:+.2f}' for x in d]} {signs}")
# head-to-head cs vs gz at mix/size
csm = [res["cs mix/size"][s] - res["gz mix/size"][s] for s in SEEDS]
print(f"\ncs-vs-gz @mix/size: {[f'{x:+.2f}' for x in csm]} {''.join('+' if x>0 else '-' for x in csm)}")
print("CSBLEND_DONE")
