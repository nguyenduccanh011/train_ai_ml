...
VARS = [
    ("gz score/size", "score", 1.0, "blend"),
    ("gz mix/size", "mix", 1.0, "blend"),
    ("cs score/size", "score", 1.0, "cs_blend"),
    ("cs mix/size", "mix", 1.0, "cs_blend"),
    ("cs blend/eq", "score", 0.0, "cs_blend"),
    ("equal", "score", 0.0, "blend"),
]
...
for sd in SEEDS:
    ...
    m = tr.merge(
        PANEL[["symbol", "date", "blend", "cs_blend"]],
        left_on=["symbol", "sigd"],
        right_on=["symbol", "date"],
        how="left",
    )
    m["ek"] = m["symbol"] + "|" + m["entry_date"].astype(str).str[:10]
    gz = dict(zip(m["ek"], m["blend"].fillna(0.0)))
    ...
    def mk():
        ...
        for t in sim.trades:
            ...
            t["blend"] = gz.get(ek, 0.0)
            t["cs_blend"] = cs.get(ek, 0.5)
        return sim
...
for nm, *_ in VARS:
    ...
    print(
        f"{nm:14s}: mean={sum(navs) / 3:.3f} navs={[f'{n:.2f}' for n in navs]} | Δvs-eq={[f'{x:+.2f}' for x in d]} {signs}"
    )
