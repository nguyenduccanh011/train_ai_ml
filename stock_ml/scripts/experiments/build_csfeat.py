...
def run_v(sim, K, convkey, k_conv=KCONV, roundtrip=0.006, advance_fee=0.0008):
    ...
    mu = statistics.mean(cv); sd = statistics.pstdev(cv) or 1.0
...
for s, g in _px.groupby("symbol"):
    ...
    g["dist20low"] = c / l.rolling(20).min() - 1; g["dist_ma20"] = c / ma20 - 1
...
for sd in SEEDS:
    ...
    m = tr.merge(PANEL[["symbol", "date"] + list(BLENDS)], left_on=["symbol", "sigd"], right_on=["symbol", "date"], how="left")
    ...
    def mk(nm):
        ...
        for t in sim.trades:
            ...
            t["score"] = slook.get(ek, 0.0); t[nm] = lut[nm].get(ek, 0.5)
        return sim
    for nm in BLENDS:
...
for nm in BLENDS:
    ...
    print(f"{nm:11s}: mean={sum(navs)/3:.3f} navs={[f'{n:.2f}' for n in navs]} | Δvs-cs4={[f'{x:+.2f}' for x in d]} {signs}{tag}")
print("CSFEAT_DONE")
