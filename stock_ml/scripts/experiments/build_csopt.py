...
VARS = [
    ("equal", None, "cs", 0.0),
    ("gz_mix_k1", "gz", "gz", 1.0),
    ("cs_mix_k05", "cs", "cs", 0.5),
    ("cs_mix_k1", "cs", "cs", 1.0),
    ("cs_mix_k15", "cs", "cs", 1.5),
    ("cs_mix_k2", "cs", "cs", 2.0),
    ("csPri_gzSize", "cs", "gz", 1.0),
    ("gzPri_csSize", "gz", "cs", 1.0),
]
...
for nm, *_ in VARS:
    ...
    print(
        f"{nm:13s}: mean={sum(navs) / 3:.3f} navs={[f'{n:.2f}' for n in navs]} | Δvs-eq={[f'{x:+.2f}' for x in d]} {signs}"
    )
print("CSOPT_DONE")
