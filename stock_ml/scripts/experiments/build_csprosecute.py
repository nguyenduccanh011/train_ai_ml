...
def run_v(sim, K, pri_key, size_key, k_conv, roundtrip=0.006, advance_fee=0.0008):
    ...
    def zof(key):
        v = [t.get(key, 0.0) for t in sim.trades]
        return statistics.mean(v), (statistics.pstdev(v) or 1.0)

    smu, ssd = zof("score")
    zk = {"gz": zof("gz"), "cs": zof("cs")}
    sm, ss = zk[size_key]
    raw = []
    for t in sim.trades:
    ...
    for t in sim.trades:
        ...
        if pri_key is None:
            ...
            pm, ps = zk[pri_key]
            t["_pri"] = sz + (t.get(pri_key, 0.0) - pm) / ps
...
for sd in SEEDS:
    ...
    m = tr.merge(
        PANEL[["symbol", "date", "gz", "cs"]],
        left_on=["symbol", "sigd"],
        right_on=["symbol", "date"],
        how="left",
    )
    m["ek"] = m["symbol"] + "|" + m["entry_date"].astype(str).str[:10]
    gzk = dict(zip(m["ek"], m["gz"].fillna(0.0)))
    csk = dict(zip(m["ek"], m["cs"].fillna(0.5)))
    slook = {
    ...
    ae = annual(run_v(mk(), K, None, "cs", 0.0))
    ac = annual(run_v(mk(), K, "cs", "cs", 1.5))
    for y in YEARS:
        if y in ae:
            ann["equal"][y].append(ae[y])
        if y in ac:
            ann["csk15"][y].append(ac[y])
    for f in FEES:
...
for y in YEARS:
    if ann["equal"][y] and ann["csk15"][y]:
        e = statistics.mean(ann["equal"][y])
        c = statistics.mean(ann["csk15"][y])
...
for f in FEES:
    e = statistics.mean(feeres[f]["equal"])
    c = statistics.mean(feeres[f]["csk15"])
