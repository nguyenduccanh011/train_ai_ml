...
FRONTIER = 3185; SEEDS = [42, 7, 99]; K = 16; KMAG = 1.5
...
def run_v(sim, K, kmag, kr, risk_only=False, roundtrip=0.006, advance_fee=0.0008):
    ...
    cv = [t.get("cs4", 0.5) for t in sim.trades]; mu = statistics.mean(cv); sd = statistics.pstdev(cv) or 1.0
    rv = [t.get("rrisk", 0.5) for t in sim.trades]; rmu = statistics.mean(rv); rsd = statistics.pstdev(rv) or 1.0
    sc = [t.get("score", 0.0) for t in sim.trades]; smu = statistics.mean(sc); ssd = statistics.pstdev(sc) or 1.0
    raw = []
    for t in sim.trades:
        mz = (t.get("cs4", 0.5) - mu) / sd
        rz = (t.get("rrisk", 0.5) - rmu) / rsd   # rrisk = inverse-vol rank (high => low vol => size up)
        wm = 1.0 if risk_only else min(max(1.0 + kmag * mz, 0.4), 1.8)
        wr = min(max(1.0 + kr * rz, 0.4), 1.8)
        t["_w"] = wm * wr; raw.append(t["_w"])
...
PANEL["rrisk"] = 1.0 - PANEL.groupby("date")["rvol20"].rank(pct=True)   # low vol => high rank => size up

con = psycopg2.connect(**PG)
VARS = [("cs4_only", 1.5, 0.0, False), ("cs4xrisk_k05", 1.5, 0.5, False),
        ("cs4xrisk_k10", 1.5, 1.0, False), ("risk_only_k15", 0.0, 1.5, True)]
...
for sd in SEEDS:
    ...
    m = tr.merge(PANEL[["symbol", "date", "cs4", "rrisk"]], left_on=["symbol", "sigd"], right_on=["symbol", "date"], how="left")
    m["ek"] = m["symbol"] + "|" + m["entry_date"].astype(str).str[:10]
    c4 = dict(zip(m["ek"], m["cs4"].fillna(0.5))); rr = dict(zip(m["ek"], m["rrisk"].fillna(0.5)))
    ...
    def mk():
        ...
        for t in sim.trades:
            ...
            t["score"] = slook.get(ek, 0.0); t["cs4"] = c4.get(ek, 0.5); t["rrisk"] = rr.get(ek, 0.5)
        return sim
    for nm, km, kr, ro in VARS:
        nav, dd = run_v(mk(), K, km, kr, ro); res[nm][sd] = (nav, dd)
    print("seed%d: " % sd + " ".join(f"{nm}={res[nm][sd][0]:.2f}(dd{res[nm][sd][1]*100:.1f})" for nm, *_ in VARS), flush=True)
con.close()

print("\n=== risk-tilt overlay on cs4 magnitude sizing (K16), sign 3/3 vs cs4_only ===")
base = res["cs4_only"]
for nm, *_ in VARS:
    navs = [res[nm][s][0] for s in SEEDS]; d = [res[nm][s][0] - base[s][0] for s in SEEDS]
    dd = statistics.mean([res[nm][s][1] for s in SEEDS]) * 100
    signs = "".join("+" if x > 0 else "-" for x in d)
    print(f"{nm:14s}: mean={sum(navs)/3:.3f} dd={dd:.1f}% | Δvs-cs4={[f'{x:+.2f}' for x in d]} {signs}")
print("CSRISK_DONE")
