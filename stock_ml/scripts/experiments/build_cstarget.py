...
BASE=3185
VARIANTS=[("cst_h10",{"type":"forward_cross_sectional","horizon":10}),("cst_h20",{"type":"forward_cross_sectional","horizon":20})]
async def make_all():
    ids={}
    ...
    async with Session() as s:
        ...
        for nm,tg in VARIANTS:
            ex=await repo.get_by_name(nm)
            if ex: print(f"exists {nm} {ex.id}"); ids[nm]=ex.id; continue
            slots=copy.deepcopy(bs)
            for sl in slots:
                if sl["slot_type"]=="entry": sl["target_config"]=copy.deepcopy(tg)
            t=await repo.create(name=nm,market=base.market,strategy=base.strategy,feature_set_id=base.feature_set_id,target_id=base.target_id,component_slots=slots,direction=base.direction,signal_mode=base.signal_mode,signal_threshold=base.signal_threshold,entry_threshold=base.entry_threshold,exit_threshold=base.exit_threshold,split_config=base.split_config,engine_config=copy.deepcopy(be),validation_config=base.validation_config,seed=base.seed,description=f"frontier + cross-sectional entry target h={tg['horizon']}",hypothesis="cs-rank target align với cs-features, regime-robust",universe_slug=base.universe_slug,model_mode=base.model_mode)
...
for nm,tid in ids.items():
    ...
    except Exception as e: print(f"  {nm}: ERROR {type(e).__name__}: {str(e)[:200]}",flush=True)
print("CSTARGET_DONE")
