...
BASE = 3185
VARIANTS = [
    ("cst_h10", {"type": "forward_cross_sectional", "horizon": 10}),
    ("cst_h20", {"type": "forward_cross_sectional", "horizon": 20}),
]
        ...
        for nm, tg in VARIANTS:
            ...
            for sl in slots:
                if sl["slot_type"] == "entry":
                    sl["target_config"] = copy.deepcopy(tg)
            t = await repo.create(
                name=nm,
                market=base.market,
                ...
                seed=base.seed,
                description=f"frontier + cross-sectional entry target h={tg['horizon']}",
                hypothesis="cs-rank target align với cs-features, regime-robust",
...
for nm, tid in ids.items():
    try:
        ...
        print(f"  {nm}: ERROR {type(e).__name__}: {str(e)[:200]}", flush=True)
print("CSTARGET_DONE")
