...
for sd in SEEDS:
    ...
    slook = {
        (x.symbol, str(pd.to_datetime(x.entry_date).date())): (
            x.score if pd.notna(x.score) else 0.0
        )
    ...
    print(
        f"seed{sd}: nodedup={res['nodedup'][sd][0]:.2f}(dd{res['nodedup'][sd][1] * 100:.1f}) "
        f"dedup={res['dedup'][sd][0]:.2f}(dd{res['dedup'][sd][1] * 100:.1f})",
        flush=True,
    )
con.close()
...
base = res["nodedup"]
navs = [res["dedup"][s][0] for s in SEEDS]
d = [res["dedup"][s][0] - base[s][0] for s in SEEDS]
dd = statistics.mean([res["dedup"][s][1] for s in SEEDS]) * 100
signs = "".join("+" if x > 0 else "-" for x in d)
print(
    f"nodedup: mean={sum(base[s][0] for s in SEEDS) / 3:.3f} dd={statistics.mean([base[s][1] for s in SEEDS]) * 100:.1f}%"
)
