...
FRONTIER = 3185; SEEDS = [42, 7, 99]; KSLOT = 16; KCONV = 1.5
...
def run_v(sim, kslot, ksize, k_conv, roundtrip=0.006, advance_fee=0.0008):
    ...
    cash = 1.0; pending = defaultdict(float); pt = 0.0; legs = []; exits = defaultdict(list); navs = []; exps = []
    for di, dt in enumerate(cal):
        ...
        for t in entries.get(dt, ()):
            if nopen >= kslot:
                break
            size = (nav_now / ksize) * t["w"]        # <-- sizing off ksize (<=kslot); cash caps => no leverage
            if cash + 1e-12 >= size:
        ...
        navs.append((dt, nav)); exps.append(pos / nav if nav > 0 else 0.0)
...
for sd in SEEDS:
    ...
    print("seed%d: " % sd + " ".join(f"Ks{ks}={res[ks][sd][0]:.1f}(dd{res[ks][sd][1]*100:.0f},ex{res[ks][sd][2]*100:.0f})" for ks in KSIZES), flush=True)
con.close()

print("\n=== deploy-idle-cash: sizing denom K_size (slot cap 16) vs K_size=16 base, sign 3/3 ===")
base = res[16]
for ks in KSIZES:
    navs = [res[ks][s][0] for s in SEEDS]; d = [res[ks][s][0] - base[s][0] for s in SEEDS]
    dd = statistics.mean([res[ks][s][1] for s in SEEDS]) * 100; ex = statistics.mean([res[ks][s][2] for s in SEEDS]) * 100
    signs = "".join("+" if x > 0 else "-" for x in d)
    tag = " (base)" if ks == 16 else ""
    print(f"K_size={ks:2d}: mean={sum(navs)/3:.3f} dd={dd:.1f}% exp={ex:.0f}% | Δ={[f'{x:+.2f}' for x in d]} {signs}{tag}")
print("EXPOSURE_DONE")
