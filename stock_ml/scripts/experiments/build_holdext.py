...
VARIANTS = {
    "he_d04w": {
        "signal_exit_skip_if_entry_distma20": 0.04,
        "signal_exit_skip_if_entry_distma20_winner_only": True,
    },
    "he_d08w": {
        "signal_exit_skip_if_entry_distma20": 0.08,
        "signal_exit_skip_if_entry_distma20_winner_only": True,
    },
}
...
async def make_all():
    ...
    async with Session() as s:
        ...
        for nm, ov in VARIANTS.items():
            ...
            if ex:
                print(f"exists {nm} {ex.id}", flush=True)
                ids[nm] = ex.id
                continue
            ec = copy.deepcopy(be)
            ec.update(ov)
            ...
            await s.commit()
            print(f"created {nm} {t.id}", flush=True)
            ids[nm] = t.id
    return ids
...
def nav_of(rid, con):
    tr = pd.read_sql(
        "SELECT symbol,entry_date,exit_date,entry_price,exit_price FROM run_trades "
        "WHERE run_id=%s AND exit_date IS NOT NULL AND entry_price IS NOT NULL "
        "AND exit_price IS NOT NULL",
        con,
        params=(rid,),
    )
    csv = WORK / f"_he_{rid.replace('/', '_')}.csv"
    tr.to_csv(csv, index=False)
    full = shuffle_stats(
        NavSim2(str(csv), "2020-01-01"), K=25, roundtrip=0.006, advance_fee=0.0008, n=20
    )
    f22 = shuffle_stats(
        NavSim2(str(csv), "2022-01-01"), K=25, roundtrip=0.006, advance_fee=0.0008, n=20
    )
...
for sd in SEEDS:
    for nm, tid in todo:
        try:
            ...
            print(
                f"seed{sd} {nm}(t{tid}): NAV={nav:.2f} DD={dd * 100:.1f}% f22={f22:.2f} tr={nt}",
                flush=True,
            )
...
for nm in VARIANTS:
    ...
    if navs:
        print(
            f"{nm}: NAVmean={sum(navs) / len(navs):.3f} Δ={[f'{x:+.2f}' for x in d]} {signs} | f22Δ={[f'{x:+.2f}' for x in df]} {signs22}"
        )
