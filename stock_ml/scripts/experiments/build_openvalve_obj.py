...
for sd in SEEDS:
    for nm, tid in TEMPLATES:
        ...
        tr[["symbol", "entry_date", "exit_date", "entry_price", "exit_price"]].to_csv(
            csv, index=False
        )
...
for nm, _ in TEMPLATES:
    ...
    print(
        f"{nm:9s}: mean={sum(navs) / 3:.3f} navs={[f'{n:.2f}' for n in navs]} | Δvs-base={[f'{x:+.2f}' for x in d]} {signs}"
    )
