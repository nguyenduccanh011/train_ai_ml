"""G3: exploit the EXIT-rule regime-skip family. Clone frontier xh_skip300w (t3064,
CAGR 0.651/NAV 26.24) and fill the un-swept gaps: skip-MA window > 300 (does the window keep
climbing past its 150->300 jump?), winner_only=False, and margin at MA300.

Engine-only knobs (rgskip / margin / winner_only) -> predictions cache-hit, backtest re-runs only.
Seed 42 first; NAV-score after via score_nav_leaderboard.py --run-like xg_.
...
BASE_TMPL = 3064  # xh_skip300w (frontier)
SEEDS = [42]
# (name, engine_config override on top of xh_skip300w)
VARIANTS = [
    ("xg_skip350w", {"signal_exit_skip_if_mkt_above_ma": 350}),
    ("xg_skip400w", {"signal_exit_skip_if_mkt_above_ma": 400}),
