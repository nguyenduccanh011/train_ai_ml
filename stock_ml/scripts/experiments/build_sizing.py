...
for sd in SEEDS:
    ...
    for kc in KCONV:
        ...
        print(
            f"seed{sd} k_conv={kc}: NAV={m['final']:.2f} CAGR={m['cagr'] * 100:.1f}% DD={m['maxdd'] * 100:.1f}%",
            flush=True,
        )
con.close()
...
for kc in KCONV:
    navs = [res[kc][s][0] for s in SEEDS]
    mean = sum(navs) / 3
    dds = [res[kc][s][2] for s in SEEDS]
    if kc == 0.0:
        print(
            f"k_conv=0.0 (equal): navs={[f'{n:.2f}' for n in navs]} mean={mean:.3f} DDmean={100 * sum(dds) / 3:.1f}%"
        )
        continue
    deltas = [res[kc][s][0] - base[s] for s in SEEDS]
    signs = "".join("+" if d > 0 else "-" for d in deltas)
    print(
        f"k_conv={kc}: navs={[f'{n:.2f}' for n in navs]} mean={mean:.3f} DDmean={100 * sum(dds) / 3:.1f}% | Δ={[f'{d:+.2f}' for d in deltas]} signs={signs}"
    )
print("SIZING_DONE")
