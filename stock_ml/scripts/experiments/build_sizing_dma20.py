...
DLOOK = {
    (r.symbol, str(r.date.date())): r.dist20low for r in DIST.itertuples() if pd.notna(r.dist20low)
}
...
def run_sized(sim, K, k_conv, roundtrip=0.006, settle_lag=2, advance_fee=0.0008):
    ...
    for di, dt in enumerate(calendar):
        ...
        exp_sum += pos / nav if nav > 0 else 0.0
...
for sd in SEEDS:
    ...
    for kc in KCONV:
        ...
        res[kc][sd] = m
        print(
            f"seed{sd} k={kc}: NAV={m['final']:.2f} DD={m['maxdd'] * 100:.1f}% exp={m['exposure'] * 100:.1f}%",
            flush=True,
        )
con.close()
print(
...
for kc in KCONV:
    navs = [res[kc][s]["final"] for s in SEEDS]
    exps = [res[kc][s]["exposure"] for s in SEEDS]
    mean = sum(navs) / 3
    exp = 100 * sum(exps) / 3
    if kc == 0.0:
        print(f"k=0 (equal): mean={mean:.3f} exp={exp:.1f}%  navs={[f'{n:.2f}' for n in navs]}")
        continue
    deltas = [res[kc][s]["final"] - base[s] for s in SEEDS]
    signs = "".join("+" if d > 0 else "-" for d in deltas)
    print(
        f"k={kc}: mean={mean:.3f} exp={exp:.1f}% | Δ={[f'{d:+.2f}' for d in deltas]} signs={signs}"
    )
print("SIZEDMA_DONE")
