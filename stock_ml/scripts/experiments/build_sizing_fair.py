"""FAIR conviction-sizing (user constraint: total size <= 1, same total capital as equal-weight —
isolate the CONVICTION edge from any exposure boost). Two guards vs the naive version:
 1. SYMMETRIC mean-1 tilt: w_i = clip(1 + k_conv*z_i, 1-c, 1+c) then RENORMALIZE the day's concurrent
    target weights so the intended book sums to 1 (redistribute, don't add exposure).
 2. Report avg EXPOSURE (invested/nav) per k_conv — must match equal-weight to be fair.
Priority-fill K16 base. Frontier 3-seed. TRUE win = beats equal-weight 3/3 AT MATCHED exposure.
"""
...
FRONTIER = 3185
SEEDS = [42, 7, 99]
KCONV = [0.0, 0.4, 0.8, 1.2]
K = 16


def run_sized_fair(sim, K, k_conv, roundtrip=0.006, settle_lag=2, advance_fee=0.0008):
    ...
    c = min(0.9, k_conv)   # symmetric clip half-width, mean-preserving
    for t in sim.trades:
        z = (t.get("score", 0.0) - mu) / sd
        t["w"] = min(max(1.0 + k_conv * z, 1.0 - c), 1.0 + c)   # symmetric around 1 -> mean~1
    ...
    for di, dt in enumerate(calendar):
        ...
        for t in entries.get(dt, ()):
            # FAIR: base slot = nav/K; weight tilts around 1 (mean-preserving) -> total exposure ~ equal.
            size = (nav_now / K) * t["w"]
            if cash + 1e-12 >= size and n_open < K:   # hard cap K positions (fair vs equal-weight K)
...
for sd in SEEDS:
    ...
    for kc in KCONV:
        m = run_sized_fair(sim, K=K, k_conv=kc)
        res[kc][sd] = m
        print(f"seed{sd} k={kc}: NAV={m['final']:.2f} DD={m['maxdd']*100:.1f}% exposure={m['exposure']*100:.1f}%", flush=True)
con.close()

print("\n=== FAIR conviction-sizing (mean-1 tilt, K cap, total<=1) vs equal — sign 3/3 + MATCHED exposure ===")
base = {s: res[0.0][s]["final"] for s in SEEDS}
for kc in KCONV:
    navs = [res[kc][s]["final"] for s in SEEDS]; mean = sum(navs) / 3
    exps = [res[kc][s]["exposure"] for s in SEEDS]; exp = 100 * sum(exps) / 3
    dds = [res[kc][s]["maxdd"] for s in SEEDS]; ddm = 100 * sum(dds) / 3
    if kc == 0.0:
        print(f"k=0.0 (equal): mean={mean:.3f} DD={ddm:.1f}% exposure={exp:.1f}%  navs={[f'{n:.2f}' for n in navs]}")
        continue
    deltas = [res[kc][s]["final"] - base[s] for s in SEEDS]
    signs = "".join("+" if d > 0 else "-" for d in deltas)
    print(f"k={kc}: mean={mean:.3f} (+{mean-res[0.0][SEEDS[0]]*0:.0f}) DD={ddm:.1f}% exposure={exp:.1f}% | Δ={[f'{d:+.2f}' for d in deltas]} signs={signs}")
print("SIZING_FAIR_DONE")
