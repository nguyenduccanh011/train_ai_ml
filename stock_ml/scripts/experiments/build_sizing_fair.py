...
for sd in SEEDS:
    ...
    for kc in KCONV:
        ...
        res[kc][sd] = m
        print(
            f"seed{sd} k={kc}: NAV={m['final']:.2f} DD={m['maxdd'] * 100:.1f}% exposure={m['exposure'] * 100:.1f}%",
            flush=True,
        )
con.close()
...
for kc in KCONV:
    ...
    exp = 100 * sum(exps) / 3
    dds = [res[kc][s]["maxdd"] for s in SEEDS]
    ddm = 100 * sum(dds) / 3
    if kc == 0.0:
        print(
            f"k=0.0 (equal): mean={mean:.3f} DD={ddm:.1f}% exposure={exp:.1f}%  navs={[f'{n:.2f}' for n in navs]}"
        )
        continue
    deltas = [res[kc][s]["final"] - base[s] for s in SEEDS]
    signs = "".join("+" if d > 0 else "-" for d in deltas)
    print(
        f"k={kc}: mean={mean:.3f} (+{mean - res[0.0][SEEDS[0]] * 0:.0f}) DD={ddm:.1f}% exposure={exp:.1f}% | Δ={[f'{d:+.2f}' for d in deltas]} signs={signs}"
    )
