...
BASE_TARGET = {"type": "action_oracle", "pct": 0.10, "min_fwd_leg": 0.10, "min_leg_bars": 3,
               "entry_min_ret_120": -0.10, "entry_confirm_pct": 0.06, "cut_struct_supp": 20}
    ...
    async with Session() as s:
        ...
        for sl in base.component_slots:
            if sl.slot_type == "entry":
                tc = dict(BASE_TARGET); tc["cut_struct_ma"] = cut_ma
                fs = ENTRY_FS
            else:
                tc = copy.deepcopy(sl.target_config); fs = sl.feature_set_name
...
def main():
    for new_name, cut_ma in SWEEP.items():
        ...
        if comps:
            ...
            print(f"== {new_name} (cut_struct_ma={cut_ma}): MEAN={mean:.1f} std={std:.1f} seeds={seeds}\n")
    print("== vs v8 -33.0 (pnl+22 pf1.58 mdd0.337 WR0.72) / v9 struct-feats -34.9 ; baselines 197.4/397.8")
