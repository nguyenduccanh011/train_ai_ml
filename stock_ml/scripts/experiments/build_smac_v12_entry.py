"""SMAC v12 — push the WINNING axis: ENTRY-label refinement on the v8 base.

Exit is exhausted (every flavor worse/flat); ALL gains came from the ENTRY label (regime
gate, confirmation). v12 refines the two entry-quality knobs on v8 (regime ENTER + confirm06,
peak exit, -33.0):
  conf08/conf10 = more reversal confirmation (confirm06 already beat confirm03 -> find optimum)
  mfl15         = require a >=15% up-leg (only the cleanest swings worth buying)
