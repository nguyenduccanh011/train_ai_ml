...
def run_split(sim_m, sim_r, K, Km, Kr, advance_fee=0.0008):
    ...
    ent_m = defaultdict(list)
    ent_r = defaultdict(list)
...
CSLK = {
    (r.symbol, str(r.date.date())): (r.cs4 if pd.notna(r.cs4) else 0.5) for r in PANEL.itertuples()
}
...
for sd in SEEDS:
    ...
    csvm = WORK / f"_sm_{sd}.csv"
    tm[["symbol", "entry_date", "exit_date", "entry_price", "exit_price"]].to_csv(csvm, index=False)
    csvr = WORK / f"_sr_{sd}.csv"
    trv[["symbol", "entry_date", "exit_date", "entry_price", "exit_price"]].to_csv(
        csvr, index=False
    )
    for Km, Kr in SPLITS:
        sm = build_sim(csvm, CSLK)
        prep(sm, KCONV)
        sr = build_sim(csvr, CSLK)
        prep(sr, KCONV)
        res[(Km, Kr)][sd] = run_split(sm, sr, K, Km, Kr)
    print(
        f"seed{sd}: "
        + " ".join(
            f"{Km}/{Kr}={res[(Km, Kr)][sd][0]:.2f}(dd{res[(Km, Kr)][sd][1] * 100:.1f})"
            for Km, Kr in SPLITS
        ),
        flush=True,
    )
...
base = res[(16, 0)]
for Km, Kr in SPLITS:
    navs = [res[(Km, Kr)][s][0] for s in SEEDS]
    d = [res[(Km, Kr)][s][0] - base[s][0] for s in SEEDS]
    dd = statistics.mean([res[(Km, Kr)][s][1] for s in SEEDS]) * 100
    signs = "".join("+" if x > 0 else "-" for x in d)
    tag = " (baseline)" if Kr == 0 else ""
    print(
        f"{Km}/{Kr}: mean={sum(navs) / 3:.3f} dd={dd:.1f}% | Δ={[f'{x:+.2f}' for x in d]} {signs}{tag}"
    )
