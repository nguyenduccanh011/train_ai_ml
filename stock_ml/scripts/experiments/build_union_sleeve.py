...
def run_book(csv, convmap, primap, K, k_conv, roundtrip=0.006, advance_fee=0.0008):
    ...
    for t in sim.trades:
        ...
        t["conv"] = convmap.get(ek, 0.5); t["pri_score"] = primap.get(ek, 0.0)
    ...
    for t in sim.trades:
        ...
        t["_pri"] = t["pri_score"] + (t["conv"] - mu) / sd   # within-source score-z (precomputed) + cs4 z
    ...
    for di, dt in enumerate(cal):
        ...
        for t in entries.get(dt, ()):
            ...
            if cash + 1e-12 >= size and nopen < K:
                ...
                cash -= size; legs.append(leg); exits[t["exit_date"]].append(leg); nopen += 1; held.add(s)
...
for sd in SEEDS:
    ...
    tm = load_trades(con, rm); tm["src"] = "m"
    trv = load_trades(con, rr); trv["src"] = "r"
    # within-source score-z for priority
    for df in (tm, trv):
        mu = df["score"].mean(); sd_ = df["score"].std() or 1.0
    ...
    def write_and_maps(df):
        ...
        cm = dict(zip(df2["ek"], df2["cs4"])); pm = dict(zip(df2["ek"], df2["pri_score"]))
        return df2, cm, pm
    # momentum-only book
    dm, cmm, pmm = write_and_maps(tm)
    csvm = WORK / f"_um_{sd}.csv"; dm[["symbol", "entry_date", "exit_date", "entry_price", "exit_price"]].to_csv(csvm, index=False)
    ...
    csvu = WORK / f"_uu_{sd}.csv"; du[["symbol", "entry_date", "exit_date", "entry_price", "exit_price"]].to_csv(csvu, index=False)
    res["union"][sd] = run_book(csvu, cmu, pmu, K, KCONV)
    print(f"seed{sd}: mom_only={res['mom_only'][sd][0]:.2f}(dd{res['mom_only'][sd][1]*100:.1f}) "
          f"union={res['union'][sd][0]:.2f}(dd{res['union'][sd][1]*100:.1f}) "
          f"[mom_tr={len(dm)} union_tr={len(du)}]", flush=True)
con.close()

print("\n=== union sleeve (mom+rev) under valve+cs-sizing vs mom-only, sign 3/3 ===")
base = res["mom_only"]
navs = [res["union"][s][0] for s in SEEDS]; d = [res["union"][s][0] - base[s][0] for s in SEEDS]
