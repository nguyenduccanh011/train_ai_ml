...
for nm, tid in TPLS.items():
    r = run_template_experiment(template_id=tid, seed=42); rids[nm] = r.get("run_id")
...
for nm in TPLS:
    ...
    if res is None:
        print(f"{nm:9s} | (no trades / invalid)"); continue
    rand, prio = res; rows.append((nm, rand, prio))
fr_prio = next((p for n, r, p in rows if n == "frontier"), 35.93)
for nm, rand, prio in rows:
    print(f"{nm:9s} | {rand:10.2f} | {prio:12.2f} | {prio-rand:+.2f} | {prio-fr_prio:+.2f}", flush=True)
con.close()
