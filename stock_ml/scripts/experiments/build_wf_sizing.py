...
for sd in SEEDS:
    ...
    rid = r.get("run_id")
    tr = pd.read_sql(
        "SELECT symbol,entry_date,exit_date,entry_price,exit_price,entry_signal_date,pnl_pct "
    ...
    m = tr.merge(PANEL, left_on=["symbol", "sigd"], right_on=["symbol", "date"], how="left").dropna(
        subset=FCOLS
    )
    m["yr"] = m["sigd"].dt.year
    ...
    tr[["symbol", "entry_date", "exit_date", "entry_price", "exit_price"]].dropna().to_csv(
        csv, index=False
    )
    ...
    print(
        f"seed{sd}: equal={res['equal'][sd]:.2f}  wf_learned={res['wf'][sd]:.2f}  dma20={res['dma20'][sd]:.2f}  (oos_n={len(oos)})",
        flush=True,
    )
con.close()
...
for nm in ("wf", "dma20"):
    ...
    print(
        f"{nm:10s}: mean={sum(navs) / 3:.3f}  navs={[f'{n:.2f}' for n in navs]} | Δ={[f'{x:+.2f}' for x in d]} signs={signs}"
    )
