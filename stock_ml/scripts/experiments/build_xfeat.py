"""Is the EXIT head (22 feat) UNDER-featured vs entry (48)? Test exit feature-count response on t3185:
 - xf_dir14  : exit_directional (14, fewer)
 - xf_top24  : exit_topclf (24, curated top-signals: divergence + macd dynamics + structure)
 - xf_peak37 : exit_peak_div (37, much richer)
If richer >= frontier(22) sign-consistent -> 22 was too few (fixable). If null/worse -> 22 enough or
the ~2x rule-mask on the exit head dominates. 3 arms x 3 seeds. Frontier 3-seed mean NAV 26.405/65.29%.
"""
...
BASE = 3185
VARIANTS = {"xf_dir14": "exit_directional", "xf_top24": "exit_topclf", "xf_peak37": "exit_peak_div"}
    ...
    async with Session() as s:
        ...
        for nm, fs in VARIANTS.items():
            ...
            t = await repo.create(
                ...
                description=f"frontier + EXIT feature set {fs} (exit feature-count test)",
                hypothesis="is exit 22-feat under-featured? richer exit set improves if so",
...
print("\n=== EXIT feature-count (14/24/37) vs frontier 22-feat (mean 26.405/65.29%) ===")
for nm in VARIANTS:
