...
tr = pd.read_sql("SELECT symbol, entry_signal_date, exit_date, exit_reason, pnl_pct "
                 "FROM run_trades WHERE run_id=%s AND exit_date IS NOT NULL "
                 "AND entry_signal_date IS NOT NULL", con, params=(FRONT,))
con.close()
tr["sigd"] = pd.to_datetime(tr["entry_signal_date"]); tr["exd"] = pd.to_datetime(tr["exit_date"])
...
for s, g in px.groupby("symbol"):
    g = g.sort_values("date").reset_index(drop=True); c, l = g["close"], g["low"]
...
conv_lk = {(r.symbol, r.date): (float(r.blend) if pd.notna(r.blend) else np.nan) for r in P.itertuples()}
...
def fwd(sym, exd, H):
    ...
    if kd is None:
        return np.nan
    b0, p0 = kd; arr = sym_close.get(sym)
...
hi = m[m["conv"] >= q[1]]; lo = m[m["conv"] <= q[0]]
for H in HORIZONS:
    col = f"fwd{H}"; sub = m[[ "conv", col]].dropna()
    ic = sub["conv"].corr(sub[col], method="spearman")
    print(f"  {H:3d}   | {m[col].mean()*100:+6.2f}% | {ic:+.4f}      | "
...
for rule in ["max_hold", "signal", "trailing_struct"]:
    ...
    if len(sub) < 50:
        print(f"  {rule}: n={len(sub)} (skip)"); continue
    qs = sub["conv"].quantile([1/3, 2/3]).values
    hh = sub[sub["conv"] >= qs[1]]; ll = sub[sub["conv"] <= qs[0]]
