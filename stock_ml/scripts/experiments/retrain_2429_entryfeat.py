...
BASE_TMPL = int(os.environ.get("BASE_TMPL", "2429"))   # set BASE_TMPL=2355 for the nopullback sandbox
BASE42 = float(os.environ.get("BASE42", "703.5"))      # the base model's seed-42 composite (for Δ)
ENTRY_FEAT = sys.argv[1] if len(sys.argv) > 1 else "entry_recov_flowcross"   # "-" = keep base feature
...
_ENTRY_TARGETS = {
    "tb_sl05":       {"type": "triple_barrier", "horizon": 30, "pt": 0.15, "sl": 0.05, "direction": "long"},
    "tb_sl06":       {"type": "triple_barrier", "horizon": 30, "pt": 0.15, "sl": 0.06, "direction": "long"},
    "tb_pt10_sl05":  {"type": "triple_barrier", "horizon": 30, "pt": 0.10, "sl": 0.05, "direction": "long"},
...
BASE_PERSEED = ({42: 719.1, 7: 714.5, 99: 714.5, 555: 719.6} if BASE_TMPL == 2429
                else {42: BASE42})
...
async def make_clone() -> int:
    ...
    async with Session() as s:
        ...
        for sl in base.component_slots:
            ...
            feat = (sl.feature_set_name if (sl.slot_type != "entry" or ENTRY_FEAT == "-")
                    else ENTRY_FEAT)
            slots.append({"slot_type": sl.slot_type, "ml_component_id": sl.ml_component_id,
                          "rule_component_id": sl.rule_component_id,
                          "feature_set_name": feat, "target_config": tc})
...
def read_composite(run_id: str):
    ...
    cur.execute("SELECT composite_score, total_pnl, pf, mdd_per_symbol, trades FROM leaderboard_runs "
...
def main():
    ...
    for sd in SEEDS:
        ...
        seeds[sd] = comp
        print(f"  {NEW_NAME} seed={sd}: comp={comp} (vs 2429 {base} Δ{d}) pnl={row[1]:.1f} "
              f"pf={row[2]:.2f} mdd={row[3]:.3f} tr={row[4]}", flush=True)
