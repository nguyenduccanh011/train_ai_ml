...
BASE_PERSEED = {42: 719.1, 7: 714.5, 99: 714.5, 555: 719.6} if BASE_TMPL == 2429 else {
    s: float(os.environ.get("BASE42", "0")) for s in SEEDS}
...
DYN_TARGET = {"type": "continuation_entry_regression", "horizon": 10, "penalty": 0.5, "trend_window": 50}
...
async def make_clone() -> int:
    ...
    async with Session() as s:
        ...
        eng["entry_ensemble3"] = {"target": copy.deepcopy(DYN_TARGET), "features": DYN_FEAT,
                                  "norm": "csrank", "z_threshold": PCT}
        t = await repo.create(
...
def main():
    ...
    for sd in SEEDS:
        ...
        seeds[sd] = comp
        print(f"  {NEW_NAME} seed={sd}: comp={comp} (vs {BASE_TMPL} {base} Δ{d}) pnl={row[1]:.1f} "
