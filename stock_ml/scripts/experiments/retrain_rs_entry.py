...
def main():
    ...
    for sd in SEEDS:
        ...
        b = BASE_PERSEED.get(sd); d = f"{comps[sd]-b:+.1f}" if (comps[sd] and b) else "?"
        print(f"  {NEW_NAME} seed={sd}: comp={comps[sd]} (vs 2632 {b} Δ{d}) pnl={row[1]:.1f} pf={row[2]:.2f} mdd={row[3]:.3f} tr={row[4]}", flush=True)
