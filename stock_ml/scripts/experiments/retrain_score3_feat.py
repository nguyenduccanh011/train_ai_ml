...
HOLD = {"signal_exit_hold_ext_atr": 1.3, "signal_exit_hold_ma": int(os.environ.get("HOLD_MA", "20")),
        "signal_exit_hold_min_score3_z": 0.5, "signal_exit_hold_profit_floor": -0.03}
...
NEW_NAME = os.environ.get("NEW_NAME", f"n2_2515_s3feat_{S3_FEAT.replace('entry_', '')}_ma{_ma}" + (f"_cv{_cv}" if _cv else ""))
...
def main():
    ...
    for sd in SEEDS:
        ...
        print(f"  {NEW_NAME} seed={sd}: comp={comps[sd]} pnl={row[1]:.1f} pf={row[2]:.2f} mdd={row[3]:.3f} tr={row[4]} (champ 2612 mean ~723.3)", flush=True)
