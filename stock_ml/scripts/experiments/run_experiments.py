...
def upsert_leaderboard_to_db(leaderboard_rows: list) -> None:
    """Upsert leaderboard rows to SQLite database (Phase 0-2 research pipeline).

    Args:
        leaderboard_rows: list of LeaderboardRow Pydantic models
    """
    if not DB_AVAILABLE:
        print("  [db] Database not available; skipping DB upsert")
        return
    ...
    try:
        ...
        session.commit()
        print(f"  [db] Upserted {len(leaderboard_rows)} rows to SQLite")
    except Exception as e:
        print(f"  [db] Warning: failed to upsert to database: {e}")
    finally:
        session.close()
...
def run_one_experiment(
    ...
    try:
        ...
        # Resolve symbols from universe config if present
        run_symbols = symbols
        if cfg.universe:
            from stock_ml.src.utils.config_loader import get_pipeline_symbols

            # Inject resolved universe version (Phase 1c)
            if cfg.universe.get("mode") == "db":
                slug = cfg.universe.get("slug")
                try:
                    ...
                    with Session(sync_engine) as session:
                        ...
                        u = repo.get_by_slug(slug)
                        if u:
                            cfg.universe["resolved_version"] = u.version
                            print(f"  [universe] Resolved {slug} → version {u.version}")
                except Exception as e:
                    print(f"  [universe] Warning: could not resolve version: {e}")

            print(
                f"  [universe] Resolving symbols from universe config: {cfg.universe.get('mode', 'unknown')}"
            )
            run_symbols = get_pipeline_symbols(
                symbols_arg="",
                market=cfg.market,
                universe_config=cfg.universe,
            )
            print(f"  [universe] Resolved to {len(run_symbols)} symbols")
        ...
        if n_seeds > 1:
            ...
            summary = run_experiment_multi_seed(
                cfg, data_root, run_symbols, out_dir, n_seeds=n_seeds
            )
        else:
            summary = run_experiment(cfg, data_root, run_symbols, out_dir)
...
def main():
    ...
    if args.rebuild_leaderboard and n_ok > 0:
        ...
        rows = rebuild_leaderboard(str(experiments_dir), str(leaderboard_dir))
        print(f"Leaderboard rebuilt at: {leaderboard_dir}")

        # Phase 0-2: Upsert to SQLite database
        upsert_leaderboard_to_db(rows)
