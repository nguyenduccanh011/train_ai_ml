...
FEATS = ["entry_recov_mp", "entry_recov_mp_lean", "entry_recov_newsig",
         "entry_lvup126_volstruct", "entry_lvup126_recov_shape", "entry_recov_eff"]
    ...
    async with S() as s:
        ...
        for fs in FEATS:
            ...
            t = await repo.create(
                ...
                seed=b.seed, description=f"head-blend champ + entry feature {fs} (iter-11 re-judge via blend).",
...
def main():
    ...
    for fs in FEATS:
        ...
        c = comp(r.get("run_id"))
        print(f"  {fs:30} seed42 comp={c[0]:.1f} mdd={c[1]:.3f}  (vs recov seed42 691.3)", flush=True)
    print("SCREEN_DONE")
