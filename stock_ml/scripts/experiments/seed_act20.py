...
async def main():
    ...
    async with S() as s:
        ...
        e_=next(x for x in base.component_slots if x.slot_type=="entry"); x_=next(x for x in base.component_slots if x.slot_type=="exit")
        tc=lambda sl:(json.loads(sl.target_config) if isinstance(sl.target_config,str) else copy.deepcopy(sl.target_config))
        slots=[{"slot_type":"entry","ml_component_id":e_.ml_component_id,"rule_component_id":None,"feature_set_name":e_.feature_set_name,"target_config":tc(e_)},
               {"slot_type":"exit","ml_component_id":x_.ml_component_id,"rule_component_id":None,"feature_set_name":x_.feature_set_name,"target_config":tc(x_)}]
