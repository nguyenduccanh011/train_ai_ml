...
VARIANTS = [
    ("n2_distexit", None, "Champion 1930 + distribution-aware exit feature set (exit_vol_dist2), no gate."),
    ("n2_distexit_g", "cons2", "Champion 1930 + exit_vol_dist2 + consolidation exit-gate (feature+gate)."),
]
...
async def main():
    ...
    async with S() as s:
        ...
        for name, gate, desc in VARIANTS:
            ...
            if ex:
                print(f"= {name} ({ex.id})"); ids[name] = ex.id; continue
