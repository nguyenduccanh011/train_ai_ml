...
GRID = [
    ("pen15", T(penalty=1.5)),
    ("pen20", T(penalty=2.0)),
    ("mfr15", T(min_fwd_rally=0.15)),
    ("mfr05", T(min_fwd_rally=0.05)),
    ("dip30", T(dip_window=30)),
    ("h15", T(horizon=15)),
]
...
async def main():
    ...
    async with S() as s:
        ...
        for tag, tgt in GRID:
            ...
            eng["entry_ensemble"] = {"target": tgt, "z_threshold": 0.9}
            t = await repo.create(
                name=name,
                market=base.market,
                ...
                seed=base.seed,
                description=f"reversal-target tune {tag}: {tgt}",
                hypothesis="sharper reversal target -> better orthogonal V-bottom adds",
