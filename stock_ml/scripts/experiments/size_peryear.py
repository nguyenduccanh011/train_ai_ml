...
tr = pd.read_sql(
    "SELECT symbol,entry_signal_date,pnl_pct FROM run_trades WHERE run_id=%s AND pnl_pct IS NOT NULL AND entry_signal_date IS NOT NULL",
    con,
    params=(FRONT,),
)
con.close()
...
px = cx.execute(
    "SELECT symbol,date,low,close FROM ohlcv WHERE timeframe='1D' AND date>='2018-06-01' ORDER BY symbol,date"
).fetchdf()
cx.close()
...
for yr in range(2020, 2027):
    ...
    print(
        f"{yr} | {len(sub):4d} | {ic1:+.3f}            | {ic2:+.3f}            | {sub.pnl_pct.mean() * 100:+.2f}%"
    )
print("PERYEAR_DONE")
