...
tr = pd.read_sql("SELECT symbol, entry_signal_date, pnl_pct FROM run_trades WHERE run_id=%s "
                 "AND pnl_pct IS NOT NULL AND entry_signal_date IS NOT NULL", con, params=(FRONT,))
sg = pd.read_sql("SELECT symbol, date, score FROM run_signals WHERE run_id=%s", con, params=(FRONT,))
con.close()
...
px = cx.execute(f"SELECT symbol,date,open,high,low,close,volume FROM ohlcv WHERE timeframe='1D' "
...
FCOLS = ["dist_ma10", "dist_ma20", "dist_ma50", "dist_ma100", "rsi14", "ret5", "ret20",
         "rvol20", "dist20low", "dist63high", "volz", "ma20_slope"]
m = tr.merge(F[["symbol", "date"] + FCOLS], left_on=["symbol", "d"], right_on=["symbol", "date"], how="left")
