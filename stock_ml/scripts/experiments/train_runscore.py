...
def causal_leg(close):
    n = len(close); d = 0; ei = 0; ep = close[0]; prev = 0; conf = {}
    for i in range(1, n):
        p = close[i]
        if d >= 0 and p > ep: ep = p; ei = i; d = 1
        elif d <= 0 and p < ep: ep = p; ei = i; d = -1
        elif d == 1 and p <= ep*(1-PCT):
            conf[i] = (ei, +1, ei-prev, abs(close[ei]/close[prev]-1) if close[prev] else None); prev = ei; d = -1; ep = p; ei = i
        elif d == -1 and p >= ep*(1+PCT):
            conf[i] = (ei, -1, None, None); prev = ei; d = 1; ep = p; ei = i
    active = np.zeros(n, int); tl = np.full(n, np.nan); ta = np.full(n, np.nan)
    last = 0; uls = []; uas = []; ctl = np.nan; cta = np.nan
    for i in range(n):
        if i in conf:
            pid, k, ul, ua = conf[i]; last = pid
            if k == +1 and ul: uls.append(ul); ctl = np.median(uls)
            if k == +1 and ua: uas.append(ua); cta = np.median(uas)
        active[i] = last; tl[i] = ctl; ta[i] = cta
...
FEATS = ["legage", "legamp", "cur_amp", "cur_age", "ext20", "ext50", "ret20", "ret60", "volr20", "accdist", "clpos"]


def build_panel(symbols):
    con = duckdb.connect(str(REPO / "market_data" / "market.duckdb"), read_only=True)
    ph = ",".join(["?"]*len(symbols))
    px = con.execute(f"SELECT symbol,date,close,high,low,volume FROM ohlcv WHERE timeframe='1D' AND symbol IN ({ph}) ORDER BY symbol,date", symbols).df()
    px["date"] = pd.to_datetime(px["date"]).dt.normalize()
    out = []
    for sym, g in px.groupby("symbol"):
        g = g.sort_values("date").reset_index(drop=True)
        cl = g.close.to_numpy(float); hi = g.high.to_numpy(float); lo = g.low.to_numpy(float); vol = g.volume.to_numpy(float)
        n = len(cl)
        if n < 120:
            continue
        lan, lamp, age, camp = causal_leg(cl)
        ma20 = pd.Series(cl).rolling(20).mean().to_numpy(); ma50 = pd.Series(cl).rolling(50).mean().to_numpy()
        vma20 = pd.Series(vol).rolling(20).mean().to_numpy()
        rng = np.where((hi-lo) <= 0, 1e-9, hi-lo); clpos = (cl-lo)/rng
        volr = vol/np.where(vma20 > 0, vma20, np.nan)
        accb = ((volr > 1.2) & (clpos > 0.55)).astype(float); distb = ((volr > 1.2) & (clpos < 0.45)).astype(float)
        ...
        df = pd.DataFrame(dict(
            symbol=sym, date=g.date.to_numpy(),
            legage=lan, legamp=lamp, cur_amp=camp, cur_age=age,
            ext20=cl/np.where(ma20 > 0, ma20, np.nan)-1, ext50=cl/np.where(ma50 > 0, ma50, np.nan)-1,
            ret20=ret20, ret60=ret60, volr20=np.nan_to_num(volr, nan=1.0),
            accdist=np.nan_to_num(adbal, nan=0.0), clpos=clpos, target=fwd))
        out.append(df)
...
def main():
    ...
    for ty in range(2020, 2027):
        ...
        tr = panel[(panel.date < cut) & (panel.date >= pd.Timestamp(year=ty-TRAIN_YEARS, month=1, day=1)) & panel.target.notna()].dropna(subset=FEATS)
        te = panel[panel.year == ty]
        if len(tr) < 500 or te.empty:
            continue
        m = lgb.LGBMRegressor(n_estimators=400, learning_rate=0.03, num_leaves=31, subsample=0.8,
                              colsample_bytree=0.8, min_child_samples=60, verbose=-1, random_state=42,
    ...
    res["runscore_z"] = res.groupby("year").runscore.transform(lambda s: (s - s.mean())/(s.std()+1e-9))
