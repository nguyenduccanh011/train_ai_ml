...
def trades(rid):
    return pd.read_sql("SELECT symbol, entry_signal_date, pnl_pct FROM run_trades WHERE run_id=%s", con, params=(rid,))


def signals(rid):
    return pd.read_sql("SELECT symbol, date, score FROM run_signals WHERE run_id=%s", con, params=(rid,))


ft = trades(rids["frontier"]); fs = signals(rids["frontier"])
...
for nm in TPLS:
    if nm == "frontier":
        continue
    vt = trades(rids[nm]); vs = signals(rids[nm])
    vset = set(zip(vt["symbol"], vt["entry_signal_date"]))
    inter = len(fset & vset); union = len(fset | vset)
    jac = inter / union if union else 0.0
    only_f = len(fset - vset); only_v = len(vset - fset)
    ...
    print(f"\n{nm}: {len(vt)} trades")
    print(f"  trade Jaccard overlap = {jac:.3f}  (shared {inter}, only-frontier {only_f}, only-variant {only_v})")
    print(f"  raw-score Spearman rank-corr = {rc:.4f}  (n_common={len(m)})")
    verdict = ("ABSORBED (trivial null — selection ~unchanged)" if jac > 0.95 and rc > 0.98
