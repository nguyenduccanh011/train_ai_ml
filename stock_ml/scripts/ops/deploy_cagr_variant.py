...
def read_row(run_id: str):
    ...
    cur = con.cursor()
    cur.execute(
        "SELECT composite_score, total_pnl, pf, mdd_per_symbol, trades, avg_hold FROM leaderboard_runs WHERE run_id=%s",
        (run_id,),
    )
    r = cur.fetchone()
    con.close()
...
def main():
    ...
    for sd in SEEDS:
        ...
        print(
            f"  {NEW_NAME} seed={sd}: composite={comps[sd]} pnl={row[1]:.1f} pf={row[2]:.2f} mdd={row[3]:.3f} avgHold={row[5]:.1f} tr={row[4]}",
            flush=True,
        )
    ...
    if cv:
        ...
        print(
            "== OFFLINE finite-capital CAGR (the real metric): ~77-82% / finalX ~1.7x vs baseline (capital_cagr_2429.py)"
        )
