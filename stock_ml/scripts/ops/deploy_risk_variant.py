...
def read_row(run_id: str):
    ...
    cur.execute("SELECT composite_score, total_pnl, pf, mdd_per_symbol, trades, sharpe FROM leaderboard_runs WHERE run_id=%s", (run_id,))
...
def main():
    ...
    for sd in SEEDS:
        ...
        print(f"  {NEW_NAME} seed={sd}: comp={comps[sd]} pnl={row[1]:.1f} pf={row[2]:.2f} mdd={row[3]:.3f} sharpe={row[5]:.2f} tr={row[4]}", flush=True)
    ...
    if cv:
        ...
        print(f"== vs champion 2429 composite-mean ~700.9 / mdd ~0.196 / pf ~4.8 (risk-optimized: lower MDD, higher PF, ~flat PnL)")
