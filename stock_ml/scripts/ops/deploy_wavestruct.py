...
def champ_baseline():
    ...
    con = psycopg2.connect(**PG); cur = con.cursor()
    cur.execute("SELECT run_seed, composite_score FROM leaderboard_runs WHERE run_name='n2_2515_volhold_rs8' AND superseded=false ORDER BY run_seed")
...
def main():
    ...
    for sd in SEEDS:
        ...
        print(f"  {NEW_NAME} seed={sd}: comp={comps[sd]} (champ {b}, Δ{dl}) "
              f"pnl={row[1]:.1f} pf={row[2]:.2f} mdd={row[3]:.3f} tr={row[4]}", flush=True)
    cv = [v for v in comps.values() if v is not None]
    bv = [float(base_lb[sd]) for sd in comps if base_lb.get(sd) is not None and comps[sd] is not None]
    if cv:
        print(f"\n== {NEW_NAME} (tmpl {new_id}): leaderboard MEAN={statistics.mean(cv):.2f} seeds={comps}")
        if bv:
            print(f"== champion 2643 MEAN={statistics.mean(bv):.2f}  -> Δ MEAN={statistics.mean(cv)-statistics.mean(bv):+.2f}")
