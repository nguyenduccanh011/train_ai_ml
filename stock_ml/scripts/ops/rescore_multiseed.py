...
def top_templates(k: int, explicit: list[int] | None):
    ...
    if explicit:
        ...
        cur.execute(
            "SELECT template_id, run_name, composite_score, run_seed FROM leaderboard_runs "
            "WHERE superseded=false ORDER BY composite_score DESC LIMIT %s",
            (k + 20,),
        )
...
def main():
    ...
    ap.add_argument(
        "--seeds", type=str, default="42,7,99", help="seeds to train (all fresh, deterministic)"
    )
    ...
    print(
        ...
        + ", ".join(f"{t[0]}:{t[1]}({t[2]:.1f}/s{t[3]})" for t in targets)
    )
    print(f"Seeds to train per config (ALL fresh, deterministic): {seed_list}")
    print(
        "NOTE: stored leaderboard composites are PRE-determinism-fix (non-reproducible); retraining all."
    )
    if not args.run:
    ...
    for tid, name, headline, hseed in targets:
        ...
        stat = dict(
            template_id=tid,
            run_name=name,
            headline_composite=float(headline),
            headline_seed=int(hseed),
            mean_composite=mean,
            std_composite=std,
            min_composite=min(comps),
            max_composite=max(comps),
            n_seeds=len(comps),
            seeds_json=json.dumps(seeds),
            mean_minus_headline=mean - float(headline),
        )
        upsert_stats(stat)
        results.append(stat)
        print(
            f"== {name}: MEAN={mean:.1f} (headline {headline:.1f}, {mean - float(headline):+.1f}) "
            f"std={std:.1f} seeds={seeds}",
            flush=True,
        )
    ...
    for s in sorted(results, key=lambda x: -x["mean_composite"]):
        ...
        print(
            f"{s['run_name'][:28]:28} {s['mean_composite']:6.1f} {s['std_composite']:5.1f} "
