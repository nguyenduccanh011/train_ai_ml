"""
Script to run predictions on all symbols and find buy/sell signals in the last 10 days
Using top 1 model: v22_exit_ablation_round42 (config 748af6fb17f4)
"""
...
def find_recent_signals(predictions_df, days=10):
    """Find buy/sell signals in last N days"""
    ...
    # Find buy signals (y_pred == 2 or 1 depending on target type)
...
def main():
    ...
    for i, symbol in enumerate(symbols, 1):
        ...
        try:
            ...
            print(f"[X] Error: {e}")
            if i == 1:  # Print full traceback for first error
                traceback.print_exc()
    ...
    if not results:
        ...
        print(f"\nFound {len(results)} stocks with signals:")
        print()
