...
def backtest_unified(y_pred, returns, df_test, feature_cols, y_pred_exit=None, **config):
    ...
    v33_tr_tier1_trigger = cfg.get(
        "v33_tr_tier1_trigger", 0.12
    )  # >= 12%: floor = tier1_keep * max_profit
    v33_tr_tier1_keep = cfg.get("v33_tr_tier1_keep", 0.40)
    v33_tr_tier2_trigger = cfg.get(
        "v33_tr_tier2_trigger", 0.20
    )  # >= 20%: floor = tier2_keep * max_profit
    v33_tr_tier2_keep = cfg.get("v33_tr_tier2_keep", 0.55)
    v33_tr_tier3_trigger = cfg.get(
        "v33_tr_tier3_trigger", 0.35
    )  # >= 35%: floor = tier3_keep * max_profit
    ...
    v39b_hap_trigger = cfg.get("v39b_hap_trigger")  # None = dùng v32_hap_pre_trigger
    ...
    ret_20d = ind["ret_20d"]
    ret_60d = ind["ret_60d"]
    ret_acceleration = ind["ret_acceleration"]
    days_since_low_10 = ind["days_since_low_10"]
    ...
    rule_signal = ind["rule_signal"]
    rule_consecutive = ind["rule_consecutive"]
    ...
    for i in range(1, n):
        ...
        if (
            ...
            if (
                ...
                or trend == "strong"
                and rs > 0
                or v26_relaxed_entry
                and trend == "strong"
                and rule_consecutive[i] >= 3
                or v35_single_bar_signal
                and rule_trigger_now
            ):
                pass
            elif prev_pred != 1:
            ...
            if ret_5d[i] > ret5_hot and not strong_breakout_context:
                if (
                    not (
                        v35_rule_override
                        and rule_trigger_now
                        and entry_score >= v35_rule_override_min_score
                    )
                    and not v35_hybrid_now
                ):
                    entry_alpha_ok = False
        ...
        if (
            ...
            if (
                ...
                and abs(ret_20d[i]) < 0.06
                and bb < 0.45
                and trend == "weak"
            ):
                entry_alpha_ok = False
                counters["chop_blocked"] += 1
        ...
        if v26_skip_choppy and new_position == 1 and position == 0 and entry_alpha_ok:
            if regime_cfg["choppy_regime"]:
                if not (v35_rule_override and rule_trigger_now) and not v35_hybrid_now:
                    entry_alpha_ok = False
                    counters["v26_choppy_skipped"] += 1

        # V27: selective choppy filter
        if (
            (not v26_skip_choppy)
            and v27_selective_choppy
            and new_position == 1
            and position == 0
            and entry_alpha_ok
        ):
            if regime_cfg["choppy_regime"]:
                ...
                quality_ok = (
                    breakout_entry
                    or (trend in ("strong", "moderate") and bs >= 3 and vs > 1.15 and vol_ok)
                    or (rule_consecutive[i] >= 3 and rs > 0 and vol_ok)
                )
                if not quality_ok:
                    entry_alpha_ok = False
        ...
        if v28_wave_acceleration_entry and new_position == 0 and position == 0:
            early_wave = (
                ...
                and ret_3d[i] > 0.02
                and ret_5d[i] < 0.05
                and ret_acceleration[i] > 0.005
            )
            accel_quality = (
                trend in ("strong", "moderate")
                and rs > 0
                and not np.isnan(sma20[i])
                and close[i] > sma20[i]
                and not np.isnan(avg_vol20[i])
                and volume[i] > 0.9 * avg_vol20[i]
            )
        ...
        if v30_peak_proximity_filter and new_position == 1 and position == 0 and not vshape_entry:
            ...
            already_rallied = ret_5d[i] > v30_peak_prox_rally10_min * 0.65 or (
                i >= 10
                and close[i] > 0
                and close[max(0, i - 10)] > 0
                and close[i] / close[max(0, i - 10)] - 1 > v30_peak_prox_rally10_min
            )
        ...
        if (
            ...
            ret10 = (
                (close[i] / close[max(0, i - 10)] - 1)
                if i >= 10 and close[max(0, i - 10)] > 0
                else 0
            )
            ret20_val = ret_20d[i]
        ...
        if v31_peak_chasing_guard and new_position == 1 and position == 0 and not vshape_entry:
            pcg_trigger = (
                ret_5d[i] > v31_pcg_ret5d_thresh and dist_sma20[i] * 100 > v31_pcg_dist_thresh
            )
            if pcg_trigger:
                if v31_pcg_action == "skip":
                    new_position = 0
                    counters["v31_pcg_skipped"] += 1
                else:  # half_size
        ...
        if new_position == 1 and position == 0:
            ...
            if v30_rally_position_scaling and not vshape_entry and not breakout_entry:
                ...
                if ret10_ps > v30_rps_tier2_rally:
                    new_position = 0
                    counters["v30_rps_blocked"] += 1
                elif ret10_ps > v30_rps_tier1_rally:
                    position_size *= 0.70
                    counters["v30_rps_scaled"] += 1
        ...
        if position == 1:
            ...
            # V38b: Stall-exit — giu N ngay nhung max profit qua thap, dang lo nho -> chot
                ...
                v38b_stall_exit
                and hold_days >= v38b_stall_min_hold
                and price_max_profit < v38b_stall_max_profit
                and price_cur_ret <= v38b_stall_pnl_thresh
            ):
                new_position = 0
                exit_reason = "v38b_stall_exit"
                counters["v38b_stall_exit"] += 1

            # V38c: HA-driven exit — bearish reversal HOAC late_wave + body_shrinking, dang lo
            elif (
                v38c_ha_exit
                and hold_days >= v38c_ha_min_hold
                and price_cur_ret < v38c_ha_pnl_thresh
            ):
                ha_bear = ha_arrays["ha_bearish_reversal_signal"][i] >= 0.5
                ha_late = (
                    ha_arrays["ha_late_wave"][i] >= 0.5 and ha_arrays["ha_body_shrinking"][i] >= 0.5
                )
                if ha_bear or ha_late:
                    new_position = 0
                    exit_reason = "v38c_ha_exit"
                    counters["v38c_ha_exit"] += 1

            # V38d: Rule co-pilot exit — co profit nhung rule_signal het hoat dong (close<MA20 hoac MACD<=0)
            elif (
                v38d_copilot_exit
                and hold_days >= v38d_copilot_min_hold
                and price_max_profit >= v38d_copilot_min_profit
            ):
                rule_off = (np.isnan(macd_hist[i]) or macd_hist[i] <= 0) or (
            ...
            # Cut loss at a tighter threshold within first N hold days
                ...
                if price_cur_ret <= v28_early_loss_cut_threshold:
                    new_position = 0
                    exit_reason = "v28_early_loss_cut"
            ...
            # V39b variant: require hold_days >= v39b_hap_min_hold + optional tighter trigger
                ...
                hap_trigger_eff = (
                    v39b_hap_trigger if v39b_hap_trigger is not None else v32_hap_pre_trigger
                )
                hap_price_ok = (
                    price_max_profit >= hap_trigger_eff and price_cur_ret <= v32_hap_pre_floor
                )
            ...
            # V30-C1: ATR-aware hard cap — replaces fixed floor when enabled
                ...
                atr_cap = max(
                    v30_atr_hc_floor, min(v30_atr_hc_ceiling, v30_atr_hc_mult * atr_ratio_now)
                )
            ...
            # === HARD CAP / SIGNAL HARD CAP ===
                ...
                if cfg["v22_adaptive_hard_cap"]:
                    if profile == "high_beta":
                        cap = max(
                            cfg["v22_hard_cap_floor_hb"],
                            cfg["v22_hard_cap_mult_hb"] * atr_ratio_now,
                        )
                    else:
                        cap = max(
                            cfg["v22_hard_cap_floor"], cfg["v22_hard_cap_mult_std"] * atr_ratio_now
                        )
                ...
                if trend == "weak":
                    ...
                    if v26_wider_hardcap:
                        cap = max(
                            hard_cap_moderate_floor * 1.3,
                            hard_cap_moderate_mult * 1.3 * atr_ratio_now,
                        )
                    elif patch_adaptive_hardcap:
                        cap = max(
                            0.10 if atr_ratio_now < 0.025 else hard_cap_moderate_floor,
                    ...
                    if v26_wider_hardcap:
                        cap = max(
                            hard_cap_strong_floor * 1.4, hard_cap_strong_mult * 1.4 * atr_ratio_now
                        )
            ...
            if new_position == 1:
                ...
                if v30_regime_aware_hardcap:
                    ...
                    rah_cap = (
                        v30_rah_choppy_cap
                        if choppy_now
                        else (v30_rah_trending_cap if trend == "strong" else -0.09)
                    )
                ...
                if v22_mode:
                    ...
                    do_fast_exit = False
                    ft = (
                        cfg["v22_fast_exit_threshold_hb"]
                        if profile == "high_beta"
                        else cfg["v22_fast_exit_threshold_std"]
                    )
                    mt = ft + 0.02

                    trend_healthy = (
                        cfg["v22_fast_exit_skip_strong"]
                        and strong_uptrend
                        and macd_line[i] > 0
                        and not np.isnan(sma20[i])
                        and close[i] > sma20[i] * 0.97
                    ...
                    vol_selling = (
                        cfg["v22_fast_exit_vol_confirm"]
                        and not np.isnan(avg_vol20[i])
                        and volume[i] > 1.3 * avg_vol20[i]
                        and close[i] < opn[i]
                    )
            ...
            if (
                new_position == 1
                and v31_hardcap_after_profit
                and price_max_profit >= v31_hap_profit_trigger
                and price_cur_ret <= v31_hap_floor
            ):
                new_position = 0
                exit_reason = "v31_hap_exit"
            ...
            if (
                new_position == 1
                and v32_weak_oversold_exit
                and trend == "weak"
                and hold_days >= v32_woe_hold_min
                and not np.isnan(dist_sma20[i])
                and dist_sma20[i] < v32_woe_dist_thresh
                and price_cur_ret >= v32_woe_min_profit
            ):
                new_position = 0
                exit_reason = "v32_weak_oversold"
            ...
            if (
                new_position == 1
                and v32_dynamic_hc_dist
                and not np.isnan(dist_sma20[i])
                and dist_sma20[i] < v32_dhc_dist_thresh
                and price_cur_ret <= v32_dhc_tight_cap
            ):
                new_position = 0
                exit_reason = "v32_dynamic_hc"
            ...
            if new_position == 1 and v32_profit_ratchet and price_max_profit >= v32_pr_trigger:
                pr_floor = price_max_profit * v32_pr_keep
                if price_cur_ret <= pr_floor:
                    new_position = 0
                    exit_reason = "v32_profit_ratchet"
            ...
            if new_position == 1 and v33_trailing_ratchet:
                ...
                if tr_floor is not None and price_cur_ret <= tr_floor:
                    new_position = 0
                    exit_reason = "v33_trailing_ratchet"
            ...
            if new_position == 1 and v33_trend_rev_exit:
                ...
                if (
                    price_max_profit >= v33_tre_min_profit
                    and hold_days >= v33_tre_hold_min
                    and v33_consec_below_ema8 >= 2
                    and not np.isnan(rsi14[i])
                    and rsi14[i] < v33_tre_rsi_thresh
                ):
                    new_position = 0
                    exit_reason = "v33_trend_rev_exit"
            ...
            if (
                new_position == 1
                and v29_profit_safety_net
                and price_max_profit >= v29_profit_safety_trigger
                and price_cur_ret < 0
            ):
                new_position = 0
                exit_reason = "v29_profit_safety"
                counters["v29_profit_safety"] += 1

            # V29-P9: Hardcap-after-peak — tighten hard cap once we've had a meaningful peak
            if (
                new_position == 1
                and v29_hardcap_after_peak
                and price_max_profit >= v29_hardcap_after_peak_trigger
                and price_cur_ret <= v29_hardcap_after_peak_floor
            ):
                new_position = 0
                exit_reason = "v29_hardcap_after_peak"
                counters["v29_hardcap_after_peak"] += 1

            # V29-P4: Reversal after peak — exit fast when ret_2d collapses after big run-up
            if (
                new_position == 1
                and v29_reversal_after_peak
                and price_max_profit >= v29_reversal_peak_trigger
                and ret_2d[i] <= v29_reversal_ret2_threshold
            ):
                new_position = 0
                exit_reason = "v29_reversal_after_peak"
                counters["v29_reversal_after_peak"] += 1

            # V29-P2: ATR velocity exit — exit on a fast 2-day decline > k*ATR after meaningful profit
            if (
                new_position == 1
                and v29_atr_velocity_exit
                and price_max_profit >= v29_atr_velocity_min_profit
                ...
                if two_d_drop >= atr_threshold:
                    new_position = 0
                    exit_reason = "v29_atr_velocity"
                    counters["v29_atr_velocity"] += 1

            # V30-B3: Chandelier trailing — ATR×k trailing from max_high when in profit
            if (
                new_position == 1
                and v30_chandelier_trail
                and price_max_profit >= v30_chand_profit_trigger
                and not np.isnan(atr14[i])
                and close[i] > 0
            ):
                ...
                if v22_mode:
                    ...
                    if price_max_profit >= pp_threshold_active:
                        ...
                        if price_below_sma10:
                            pp_pending_bars += 1
                            heavy_vol = (
                                not np.isnan(avg_vol20[i]) and volume[i] > 1.3 * avg_vol20[i]
                            )
                    ...
                    pp_threshold = (
                        peak_protect_strong_threshold
                        if strong_uptrend
                        else peak_protect_normal_threshold
                    )
            ...
            if new_position == 1 and max_profit >= PROFIT_LOCK_THRESHOLD:
                if cum_ret < PROFIT_LOCK_MIN and not strong_uptrend:
                    if (
                        patch_symbol_tuning
                        and regime_cfg.get("disable_profit_lock_in_strong")
                        and trend == "strong"
                    ):
                        pass
                    else:
            ...
            if v27_trend_persistence_hold and strong_uptrend and cum_ret > 0.03:
                trend_supported = (not np.isnan(sma20[i]) and close[i] >= sma20[i] * 0.995) and (
                    macd_hist[i] > -0.01
                )
                if trend_supported:
            ...
            if (
                new_position == 0
                and exit_reason == "signal"
                and v39a_signal_exit_min_hold > 0
                and hold_days < v39a_signal_exit_min_hold
            ):
                new_position = 1
                counters["v39a_min_hold_blocked"] += 1
            if new_position == 0 and exit_reason == "signal" and v39a_rule_confirm_exit:
                ...
                if not (macd_bearish_now and below_ma20_now) and min_mp_ok:
                    new_position = 1
                    counters["v39a_rule_confirm_blocked"] += 1
            ...
            if (
                new_position == 0
                and exit_reason == "signal"
                and v31_short_hold_exit_filter
                and hold_days < v31_shef_min_hold
                and cum_ret > v31_shef_min_pnl
            ):
                new_position = 1
                counters["v31_shef_blocked"] += 1
            ...
            if (
                new_position == 1
                and exit_reason == "signal"
                and v32_signal_weak_exit
                and trend == "weak"
                and not np.isnan(dist_sma20[i])
                and dist_sma20[i] < v32_swe_dist_thresh
            ):
                new_position = 0
                counters["v32_swe_pass"] += 1
            ...
            if (
                new_position == 0
                and v33_rsi_oversold_block
                and exit_reason in ("signal", "v32_hap_preempt", "v32_weak_oversold")
                and not np.isnan(rsi14[i])
                and rsi14[i] < v33_rob_rsi_thresh
                and price_cur_ret > -0.08
            ):  # chỉ block khi chưa lỗ quá nặng
                new_position = 1
                counters["v33_rob_blocked"] += 1
            ...
            if (
                new_position == 0
                and exit_reason == "signal"
                and v33_signal_confirm_exit
                and cum_ret > v33_sce_min_pnl
                and price_max_profit >= v33_sce_min_profit_seen
                and hold_days < v33_sce_max_hold
            ):
            ...
            if (
                new_position == 0
                and exit_reason == "signal"
                and v30_momentum_hold_override
                and cum_ret >= v30_mho_min_profit
                and not np.isnan(rsi14[i])
                and rsi14[i] < v30_mho_rsi_max
                and not np.isnan(sma20[i])
                and close[i] > sma20[i]
                and not np.isnan(ema8[i])
                and close[i] > ema8[i]
            ...
            if new_position == 0 and exit_reason == "signal" and v31_adaptive_defer:
                ema_ok = (
                    (not np.isnan(ema8[i]) and close[i] >= ema8[i])
                    if v31_ad_use_ema_confirm
                    else True
                )
            ...
            if (
                new_position == 0
                and exit_reason == "signal"
                and v39d_rule_exit_symbols
                and sym in v39d_rule_exit_symbols
            ):
                rule_exit_now = (not np.isnan(macd_hist[i]) and macd_hist[i] <= 0) or (
                    not np.isnan(sma20[i]) and close[i] < sma20[i]
                )
                if not rule_exit_now:
                    new_position = 1
                    counters["v39d_rule_exit_held"] += 1
            ...
            if new_position == 0 and exit_reason == "signal" and mod_h:
                ...
                if bearish_score < score_threshold:
                    new_position = 1
                    counters["confirmed_exit_blocked"] += 1
            ...
            if mod_i and new_position == 0 and exit_reason == "signal":
                ...
                if (
                    ...
                    new_position = 1
                    counters["trend_carry_saved"] += 1

            # Long-horizon carry
            if patch_long_horizon:
                if new_position == 0 and exit_reason in (
                    "signal",
                    "peak_protect_dist",
                    "peak_protect_ema",
                    "profit_lock",
                    ...
                    long_horizon_regime = (
                        ...
                        and not np.isnan(sma20[i])
                        and not np.isnan(sma50[i])
                        and not np.isnan(sma100[i])
                        and sma20[i] > sma50[i] > sma100[i]
                        and days_above_sma50[i] >= 20
                        and cum_ret > 0.15
                    )
                    if long_horizon_regime:
                        hard_breakdown = (close[i] < sma50[i] * 0.97) or (
                            i >= 3
                            and macd_hist[i] < 0
                            and macd_hist[i - 1] < 0
                            and macd_hist[i - 2] < 0
            ...
            if new_position == 1:
                ...
                if record_trades and entry_equity > 0:
                    ...
                    if v31_enriched_log:
                        ...
                        trade_rec["exit_rsi14"] = (
                            round(float(rsi14[i]), 2) if not np.isnan(rsi14[i]) else 50.0
                        )
                        trade_rec["exit_macd_hist"] = (
                            round(float(macd_hist[i]), 5) if not np.isnan(macd_hist[i]) else 0.0
                        )
                        trade_rec["price_max_profit_pct"] = round(price_max_profit * 100, 2)
                        trade_rec["exit_above_sma20"] = int(
                            not np.isnan(sma20[i]) and close[i] >= sma20[i]
                        )
                        trade_rec["exit_above_ema8"] = int(
                            not np.isnan(ema8[i]) and close[i] >= ema8[i]
                        )
                        trade_rec["exit_vol_ratio"] = (
                            round(float(volume[i] / avg_vol20[i]), 2)
                            if (not np.isnan(avg_vol20[i]) and avg_vol20[i] > 0)
                            else 1.0
                        )
