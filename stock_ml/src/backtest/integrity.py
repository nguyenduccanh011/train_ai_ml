...
from stock_ml.src.data.splitter import SplitWindow
...
def audit_report(
    ...
    if windows is not None:
        ...
        try:
            ...
            pass
    # entry_integrity / signal_coverage verify the signal->trade linkage. In portfolio
    # (top-K rebalance) mode entries are weight-driven, not signal-driven, so those two
    # checks don't apply; the leakage checks (windows, fill_offset) still do.
    checks.append(check_fill_offset(trades))
