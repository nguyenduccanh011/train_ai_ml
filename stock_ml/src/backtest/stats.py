...
def equity_stats(equity: pd.DataFrame, periods_per_year: int = 252) -> dict:
    """Performance metrics from a NAV/equity curve ([date, nav]).

    Used by the portfolio (time-stepped) execution path, where PnL is capital-weighted
    rather than an average of per-trade percentage returns.
    """
    if equity is None or equity.empty or "nav" not in equity.columns:
        return {"total_return": 0.0, "cagr": 0.0, "sharpe": 0.0, "max_drawdown": 0.0, "n_days": 0}
    nav = equity["nav"].astype(float).reset_index(drop=True)
    rets = nav.pct_change().dropna()
    total_return = float(nav.iloc[-1] / nav.iloc[0] - 1.0) if nav.iloc[0] else 0.0
    n = len(nav)
    cagr = (
        float((nav.iloc[-1] / nav.iloc[0]) ** (periods_per_year / max(n - 1, 1)) - 1.0)
        if nav.iloc[0] > 0 and n > 1
        else 0.0
    )
    std = float(rets.std(ddof=0))
    sharpe = float(rets.mean() / std * (periods_per_year**0.5)) if std > 0 else 0.0
