...
def _build_prediction_cache(
    ...
    *,
    ...
    model_cfg = get_model_config("v32")
    return _build_predictions(
        symbols,
        feature_set or model_cfg.get("feature_set", "leading_v3"),
        model_cfg.get("target", V32_TARGET),
        device,
...
def trades_to_v32_dataframe(trades: list[Trade | dict[str, Any]]) -> pd.DataFrame:
