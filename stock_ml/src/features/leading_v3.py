...
def _add_sector_relative_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for date in df["date"].unique():
        ...
        for sector in subset["sector"].unique():
            ...
            sector_return_20d_median = (
                sector_data["return_20d"].median() if "return_20d" in df.columns else 0
            )
            sector_momentum_median = (
                sector_data["momentum"].median() if "momentum" in df.columns else 0
            )
            sector_volume_median = sector_data["volume"].median()
            sector_volatility_median = (
                sector_data["realized_vol_10"].median() if "realized_vol_10" in df.columns else 0
            )
            ...
            df.loc[sector_symbol_mask, "volume_vs_sector"] = df.loc[
                sector_symbol_mask, "volume"
            ] / (sector_volume_median + 1e-10)
            ...
            if "sma_20_ratio" in df.columns:
                df.loc[sector_symbol_mask, "strength_vs_sector"] = df.loc[
                    sector_symbol_mask, "sma_20_ratio"
                ].rank(pct=True)
            ...
            if len(sector_data) > 10 and "returns" in df.columns:
                ...
                if sector_returns_std > 0:
                    df.loc[sector_symbol_mask, "beta_to_sector"] = (
                        df.loc[sector_symbol_mask, "returns"] - sector_returns_mean
                    ) / sector_returns_std
...
def _add_regime_interaction_features(
    ...
    market_index["market_volatility"] = (
        market_index["market_returns"].rolling(20, min_periods=1).std()
    )
    market_index["market_volatility_90th"] = (
        market_index["market_volatility"].rolling(200, min_periods=1).quantile(0.9)
    )

    df = df.merge(
        market_index[["date", "market_ma200", "market_volatility", "market_volatility_90th"]],
        on="date",
        how="left",
    )
    ...
    df["market_volatility_regime"] = (
        df["market_volatility"] > df["market_volatility_90th"]
    ).astype(float)
    ...
    if "sma_20_ratio" in df.columns:
        df["regime_interaction_strength"] = df["sma_20_ratio"] * (
            1 - df["market_volatility_regime"]
        )

    # Cleanup
    df = df.drop(
        columns=["market_close", "market_ma200", "market_volatility", "market_volatility_90th"],
        errors="ignore",
    )
...
def _add_liquidity_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["volume_20d_avg"] = df.groupby("symbol")["volume"].transform(
        lambda x: x.rolling(20, min_periods=1).mean()
    )
    df["volume_rank_20d"] = df.groupby("date")["volume_20d_avg"].transform(
