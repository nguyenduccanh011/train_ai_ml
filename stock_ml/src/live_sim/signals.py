...
from stock_ml.src.live_sim.config import LiveSimConfig
from stock_ml.src.models.baseline import BaselineModel
from stock_ml.src.signals.core import generate_signals_from_features
...
class FrozenSignalSet:
    ...
    scores: dict[str, float] = field(default_factory=dict)  # symbol → continuous alpha (T-1)
...
class SignalGenerator:
    ...
    def generate(
        ...
        # Use unified signal generation (also return continuous scores for sizing parity)
        filtered, scores = generate_signals_from_features(
            model=self.model,
            history_feat=history_feat,
            symbols=self.config.symbols,
            feature_cols=FEATURE_COLS,
            signal_threshold=getattr(self.config, "signal_threshold", 0.0),
            entry_threshold=getattr(self.config, "entry_threshold", None),
            exit_threshold=getattr(self.config, "exit_threshold", None),
