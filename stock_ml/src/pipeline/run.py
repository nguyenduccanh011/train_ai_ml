...
from stock_ml.src.backtest.engine import CostModel, EngineConfig
...
from stock_ml.src.tracking.mlflow_logger import data_fingerprint, get_git_commit
...
def build_default_config(
    ...
    **overrides,
    ...
    # EngineConfig no longer carries silent defaults for exit controls. This legacy
    # "default config" builder supplies the historical baseline EXPLICITLY here —
    # transparent at the call site, unlike a hidden dataclass fallback. The
    # template→backtest path (experiment.py) still fails loud if these are omitted.
    engine_kw.setdefault("max_hold_bars", 20)
    engine_kw.setdefault("hard_stop_pct", -0.08)
    cfg = RunConfig(
        data_root=data_root,
        symbols=symbols,
        out_dir=out_dir,
