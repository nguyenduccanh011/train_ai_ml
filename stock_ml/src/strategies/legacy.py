"""
Legacy backtest strategies: V17, V18, V19.1, V19.3.

These implementations are kept verbatim (logic unchanged) from the original
...
def backtest_v18(y_pred, returns, df_test, feature_cols,
    ...
    for i in range(1, n):
        ...
        if new_position == 1 and position == 0 and not vshape_entry:
            if ret_5d[i] > 0.05 and not strong_breakout_context:
                new_position = 0
            elif ret_5d[i] > 0.05 and strong_breakout_context:
        ...
        if new_position == 1 and position == 0:
            ...
            if dist_sma20[i] > 0.05 and position_size > 0.5:
                position_size = 0.5
            if ret_5d[i] > 0.05 and strong_breakout_context:
                position_size = min(position_size, 0.6)
...
def backtest_v19(y_pred, returns, df_test, feature_cols,
    ...
    def get_regime_adapter(i, trend):
        ...
        if choppy_regime:
            params["dp_floor"] += 0.004
            params["size_mult"] *= 0.75
            params["base_confirm_bars"] += 1
            params["exit_score_threshold"] += 0.4

        if atr_ratio > 0.045:
            params["size_mult"] *= 0.9
        if trend == "strong":
            params["dp_floor"] = max(0.012, params["dp_floor"] - 0.003)
            params["ret5_hot"] += 0.01
            params["exit_score_threshold"] += 0.2
...
def backtest_v19_4(y_pred, returns, df_test, feature_cols,
    ...
    def get_regime_adapter(i, trend):
        ...
        # VND/SSI: faster exit when losing, lower exit threshold
        if sym in ("VND", "SSI"):
            params["exit_score_threshold"] -= 0.3  # easier to exit
            params["hard_stop_override"] = 0.15  # tighter hard stop
            params["min_hold_override"] = 4 if trend != "strong" else 6

        # ACB: relax entry (V11 without filter was +59.8% vs V19.1 +26.2%)
        if sym == "ACB":
            params["dp_floor"] = max(0.008, params["dp_floor"] - 0.008)
            params["relax_entry"] = True

        # DGC/AAS: allow more entries in confirmed uptrend
        if sym in ("DGC", "AAS"):
            if trend == "strong":
                params["dp_floor"] = max(0.008, params["dp_floor"] - 0.006)
            params["relax_antichop"] = True  # relax anti-chop when SMA20>SMA50 confirmed
    ...
    for i in range(1, n):
        ...
        if new_position == 1 and position == 0 and not quick_reentry and not vshape_entry:
            ...
            # V19.4: ACB relaxed entry (V11 no filter: +59.8% vs V19.1: +26.2%)
            if regime_cfg.get("relax_entry", False):
        ...
        # Position sizing - V19.3: BINARY SIZING (eliminates weak 60-80% bucket)
        if new_position == 1 and position == 0:
            entry_score = sum([wp < 0.75, dp > 0.02, rs > 0, vs > 1.1, hl >= 2])
            atr_ratio = (atr14[i] / close[i]) if (close[i] > 0 and not np.isnan(atr14[i])) else 0.03

            # V19.3: Binary sizing - 3 tiers only
            if vshape_entry:
                position_size = 0.50  # V-shape = early, conservative
            elif trend == "strong" and entry_score >= 4:
                position_size = 0.95  # HIGH conviction
            elif trend == "strong" and entry_score >= 3:
                position_size = 0.90  # HIGH conviction
            elif trend == "moderate" and entry_score >= 3:
                position_size = 0.50  # MEDIUM conviction
            elif trend == "weak":
                position_size = 0.30  # LOW conviction
            else:
                position_size = 0.50  # MEDIUM default

            # V19.3: ATR-based cap (volatility protection)
            if atr_ratio > 0.055:
                position_size = min(position_size, 0.35)
            elif atr_ratio > 0.040:
                position_size = min(position_size, 0.50)

            # V19.3: Trend-based cap
            if trend == "weak":
                position_size = min(position_size, 0.40)
            elif trend == "moderate":
                position_size = min(position_size, 0.70)

            # Bearish candle reduction
            if close[i] <= opn[i] and not vshape_entry:
                position_size *= 0.75
        ...
        if position == 1:
            ...
            # 0) HARD STOP (V19.4: symbol-specific override)
            hard_stop_val = regime_cfg.get("hard_stop_override", HARD_STOP)
            if cum_ret <= -hard_stop_val:
                new_position = 0
                exit_reason = "hard_stop"

            # 0.5) V19.2: SIGNAL HARD CAP using price return (independent of position_size)
            elif price_cur_ret <= -SIGNAL_HARD_CAP:
                new_position = 0
                exit_reason = "signal_hard_cap"
                n_signal_hard_cap += 1
            ...
            # Zombie exit (V19.4: shorter zombie for weak trend)
            zombie_bars = 8 if trend == "weak" else ZOMBIE_BARS
            if new_position == 1 and hold_days >= zombie_bars and cum_ret < 0.01:
                if not strong_uptrend:
                    new_position = 0
                    exit_reason = "zombie_exit"
            ...
            if new_position == 0 and exit_reason == "signal":
                if mod_h:
                    ...
                    if cum_ret < -0.03:
                        score_threshold -= 0.4
                    # V19.2: TIME DECAY - reduce threshold when holding too long without profit
                    if hold_days > 15 and cum_ret < 0.03:
                        score_threshold *= 0.60
                        n_time_decay_exit += 1
                    elif hold_days > 10 and cum_ret < 0.01:
            ...
            if new_position == 0 and exit_reason == "signal":
                # V19.2: NO confirm bars when losing - exit immediately
                if cum_ret < 0:
                    confirm_bars = 0
                elif mod_d:
                    if cum_ret < 0:
            ...
            # Strong trend override (V19.2: only if meaningfully profitable)
            if new_position == 0 and exit_reason == "signal":
                if cum_ret > 0.03 and trend == "strong":
                    new_position = 1
