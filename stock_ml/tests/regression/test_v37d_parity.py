"""Regression: componentized v37d runner should match golden trades_v37d.csv."""
...
GOLDEN_CSV = GOLDEN_DIR / "trades_v37d.csv"
GOLDEN_META = GOLDEN_DIR / "trades_v37d.meta.json"
...
def test_v37d_matches_golden() -> None:
    ...
    model_cfg = get_model_config("v37d")
    prediction_cache = _build_predictions(
        symbols,
        meta["feature_set"],
        meta["target_config"],
        "cpu",
        model_type=model_cfg.get("model_type", meta["model_type"]),
    ...
    df_raw = trades_to_v37d_dataframe(trades).reset_index(drop=True)
