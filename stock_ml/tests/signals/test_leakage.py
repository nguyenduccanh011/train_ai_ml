...
def test_target_no_leakage_from_future(
    target_type: str, kwargs: dict, forward_window: int
) -> None:
...
def test_features_no_leakage_from_future() -> None:
    ...
    for col in feature_cols:
        ...
        valid = ~b_nan
        if not np.allclose(
            b[valid].astype(float), p[valid].astype(float), rtol=1e-9, atol=1e-9
        ):
            leaking_features.append(col)

    assert not leaking_features, (
        f"Features leak future data into past rows: {leaking_features}"
    )
