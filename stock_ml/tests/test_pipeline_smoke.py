...
from stock_ml.src.backtest.integrity import audit_report  # noqa: E402
from stock_ml.src.backtest.stats import aggregate_stats, per_day_stats, per_year_stats  # noqa: E402
...
from stock_ml.src.targets.forward import ForwardReturnTarget  # noqa: E402
...
# Burn-in buffer (calendar days) prepended before a window so rolling features are
# warm inside it — the way production computes features on the full continuous
# history. basic_v1's longest lookback is 20 bars (~28 calendar days); 70 is a safe
# margin.
_WARMUP_DAYS = 70


def _warm_split(symbols, win_start, split, end, *, horizon=5, seed=0):
    """Production-style train/test split with no NaN to drop.

    Generates ``_WARMUP_DAYS`` of history before ``win_start``, computes
    features + target on the full series, then slices to ``[win_start, split)`` /
    ``[split, end)`` by date. The burn-in prefix is excluded by the window's lower
    ...
    train = data[(data["date"] >= win_start) & (data["date"] < split)]
    test = data[(data["date"] >= split) & (data["date"] < end)]
    return train, test
...
def test_train_fold_regression():
    ...
    # Burn-in buffer warms the window; nothing is dropped (see _warm_split).
    train, test = _warm_split(["X"], "2020-01-01", "2020-04-01", "2020-06-30")
...
def test_train_fold_regression_reproducibility():
    ...
    # Burn-in buffer warms the window; nothing is dropped (see _warm_split).
    train, test = _warm_split(["X", "Y"], "2020-01-01", "2020-04-01", "2020-06-30", seed=42)
...
def test_regression_vectorized_faster_than_loop():
    ...
    # Burn-in buffer warms the window; nothing is dropped (see _warm_split).
    train, test = _warm_split(["X"], "2020-01-01", "2020-08-01", "2021-01-01", seed=42)
