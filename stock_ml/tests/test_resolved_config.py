...
def test_resolved_captures_all_227_engine_nodes() -> None:
    ...
    r = build_resolved_config(cfg, engine, universe=["FPT", "SSI"], universe_slug="vn_x", as_of="2024-01-01")

    assert r["schema"] == "resolved.json/1"
    assert len(r["engine"]) == 227, "resolved.json must pin every EngineConfig node, not just the set ones"
...
def test_data_is_declared_not_embedded() -> None:
    ...
    engine, _ = engine_config_from_dict(cfg.engine)
    r = build_resolved_config(cfg, engine, universe=["FPT", "SSI", "HPG"], universe_slug="s", as_of="2024-06-30")
