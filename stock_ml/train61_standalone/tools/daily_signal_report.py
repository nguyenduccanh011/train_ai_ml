...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--as-of", default="", help="Last cutoff date YYYY-MM-DD, default DB latest")
    ...
    parser.add_argument("--top", type=int, default=0, help="Limit predictions in summary, 0 keeps all")
...
def trades_as_of(payload: dict[str, Any] | None, model_id: str, cutoff: str) -> list[dict[str, Any]]:
    if not payload:
        return []
...
def position_rows(payloads: dict[str, dict[str, Any] | None], model_id: str, cutoff: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    open_positions = []
    new_entries = []
...
def main() -> int:
    ...
    write_csv(output_dir / f"next_session_predictions_{latest_date}.csv", latest["next_session_predictions"])
    write_csv(output_dir / f"watchlist_top_buy_proba_{latest_date}.csv", latest["watchlist_top_buy_proba"])
