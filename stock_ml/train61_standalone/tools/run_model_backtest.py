...
from src.pipeline.trainer import build_prediction_cache as build_standard_prediction_cache  # noqa: E402
...
def build_prediction_cache(model_id: str, model_cfg: dict[str, Any], cfg: Any, symbols: list[str]) -> list[dict[str, Any]]:
    ...
    for idx, symbol in enumerate(symbols, start=1):
        ...
        if model_type == "on_demand":
            ...
            cache_items.extend(server._build_prediction_cache_from_model(model_id, symbol, model_cfg))
        else:
            raise ValueError(
...
def compare_standard_cache(cfg: Any, symbols: list[str], live_items: list[dict[str, Any]]) -> dict[str, Any]:
    standard_items = build_standard_prediction_cache(cfg, symbols, device="cpu")
    active_year = int(cfg.split.last_test_year)
    standard_items = [item for item in standard_items if str(item.get("sym_test_df", pd.DataFrame()).get("timestamp", pd.Series(dtype=str)).min())[:4] == str(active_year)]
...
def summarize(result: Any, *, model_id: str, symbols: list[str], runtime_sec: float, cache_items: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "model_id": model_id,
        "symbols": len(symbols),
        "symbols_with_trades": int(trades_df["symbol"].nunique()) if not trades_df.empty and "symbol" in trades_df.columns else 0,
...
def main() -> int:
    ...
    parity = compare_standard_cache(cfg, symbols, cache_items) if args.compare_standard_cache else None
    ...
    if not closed_trades.empty and "exit_date" in closed_trades.columns:
        closed_trades = closed_trades[closed_trades["exit_date"].astype(str).str.strip() != ""].copy()
